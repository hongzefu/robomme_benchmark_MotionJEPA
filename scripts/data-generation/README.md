...
**worst 片段合集**（仅模式②，两个）：逐帧误差 = 网格 mask 对 GT 的**漏标臂像素 +
误涂物体像素**（刻意不含误涂背景——整格涂红盖到背景是网格化的必然代价，不算错）。

- `worst_per_task.mp4`：**每任务最差第一名**，一任务恰一段（16 段），按峰值误差降序；
- `worst_top5.mp4`：**全局最差 top-5 段**（`--top-worst` 可改）——段语义、互不重叠：
  按误差降序扫描峰值帧，与已选段（同 episode）重叠的跳过取下一名，只有误差 > 0 的帧
  不足时段数才少于 N（早期版本按帧取 top-20 再合并，最差帧扎堆时缩水到 3 段，已废弃）。

每段带前后 `--context-seconds`（默认 1 秒）上下文；**两个合集开头都有清单卡**，逐条
列出各段的任务/episode/最差帧号/误差像素数；段前有中文标题卡、峰值帧画黄框；另落
`worst.json` 机读排名（两张表）。模式①没有 GT ⇒ 无误差面板、无 worst 合集（显式传
worst 参数会报错）。
