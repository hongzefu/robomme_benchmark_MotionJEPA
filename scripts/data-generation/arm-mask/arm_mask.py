...
只有三个整数，且都是形态学的最小配置（结构元固定为 3×3 全 1 方形，即八邻域）：开运算 1 次、
...
# 像素表（颜色表）的定义与拟合归 gt-data，这里只做只读推理，绝不复制第二份判别口径
_GT_DATA_DIR = Path(__file__).resolve().parent.parent / "gt-data"
if str(_GT_DATA_DIR) not in sys.path:
    sys.path.insert(0, str(_GT_DATA_DIR))

from color_model import CLASS_ARM  # noqa: E402


# 涂红用的纯红：被标定的像素涂 (255, 0, 0)，其余像素逐位不动
RED = (255, 0, 0)
