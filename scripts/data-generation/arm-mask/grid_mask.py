...
- **占比分子**：格内被像素级最终 mask（`arm_mask.arm_masks_for_episode`
...
- **阈值全局统一**：`min_pixels` 是唯一阈值，对全部任务、全部 episode、全部格子
  位置一体生效。本模块与上层入口都**不提供** per-task / per-episode / per-cell 的
  覆盖通道——这是口径不是省事，用户原话「阈值是全局全task/episode全8*8位置必须
  统一的！」。
- `min_pixels` **刻意没有默认值**：拍板值 K=13 只写在 `make_mask.GRID_MIN_PIXELS`
  一处，库层给默认值等于偷偷立第二个口径。

## ⚠ 两条必须记住的性质

1. **网格 mask 打破了像素链路「只减不增」的单调性**。四条形态学规则每条都只让
   ...
   不设闸门。不要拿像素口径「刚性原则」的结论来推网格 mask 的性质。
...
格级加时间平滑，注意 `arm_mask` 第④条的白名单交**兜不住格级**的非单调性。

⚠ **时间维未对齐**：Wan VAE 还有时间维 4× 压缩（首帧单独成组），本模块只对齐空间
8×，网格 mask 是**逐帧独立**的。接生成链路时不要把「一格 = 一个 latent 位置」这句
外推到时间维。
"""
