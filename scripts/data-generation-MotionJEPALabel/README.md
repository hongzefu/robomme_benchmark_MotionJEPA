...
⚠ **窗口 ≥2 的口径对账（实测 4/19 条不重合）**：原版后续窗口的 `idx1` 是固定的 **bin**，
窗口 1 可能已把它挪到别的槽位，而我们钉死的是**槽位对**。Button 8 条全部重合（其 NN 图是
0↔3、1↔2 的完美配对），Video 有 4 条偏离。**事件本身 19/19 严格落在原版可达空间内**，
偏离的只是后 30 帧露出的第二次 swap 换了哪一对 —— 逐条量化在
`later_windows_follow_native_nn` 字段；要求整条 clip 都原版可达就按该字段筛，得 15/19 条。
