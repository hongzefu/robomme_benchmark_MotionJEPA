...
先 11 组汇总，再每组逐条：`ep | seed | T | demo | 段数 | 窗口 demo+exec=合计 | Δ32 | Δ8 | 段序列`，两个视频任务再加一列 `swap 起止帧（发起者↔搭档）`。段序列写「短标 帧数」，`‖` 标 demo→exec 的切换点。下面的表由 `window_timeline.py tables --write` 自动生成并写入两条标记之间，手改会被 `check_doc_links.py` 判 `window_tables=FAIL`。
