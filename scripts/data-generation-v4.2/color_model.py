...
### 这条规则的来历：它是「归一化似然 argmax + 混合支撑否决」的退化形态

v4.1 / v4.2 的写法是：每列除以本列像素总量得 `P(颜色 | 类)`，取 argmax，再叠一条
「凡混合列计数 > 0 一律改判混合」的否决。2026-08-12 在标定集（16297 色）上实测发现
**三段判决全部退化成上表的支撑判据，逐位一致**——那 145 亿个计数值一个都没进判决。
于是直接把规则写成它的真实形态，删掉似然与 argmax。

两段的成因不同，**必须分开记**：

- **判臂那一段是恒等式**，永远成立。证明：(⇐) `N₁=0` 蕴含 `N₀=0`，故 `P(c|背景) =
  P(c|混合) = 0`，而 `N₂>0` 给出 `P(c|臂)>0`，argmax 取臂；且 `N₁=0` 不触发否决。
  (⇒) 判臂要求未被否决即 `N₁=0`，且 argmax 取臂要求 `P(c|臂)>0` 即 `N₂>0`。∎
- **背景 vs 混合那一段没有构造保证**，只是本批数据实测成立。旧口径下判背景的条件是
  `N₀/T₀ > N₁/T₁`，即该色上物体像素占比小于 `1 − T₀/T₁ = 2.37%`（实测
  `T₀/T₁ = 0.9762989`）；16297 个色**没有一个**违反，所以两种写法在本批数据上逐位相同。
  换一批数据可能不同，但**下游只消费「臂」，背景/混合的区分仅用于出图**，不影响产物。

推论（现在是定义的直接结果，不再需要引理）：判臂色在标定集上的非臂像素恒为 0，故
「颜色阶段精确率上界 = 1.000」是恒等式而非经验数字；颜色阶段的漏标代价恰为臂∩混合
共享色上的臂像素（实测 126 色 / 3112 万臂像素 / 9.0789%）。规则等价性与「计数值大小
不影响判决」由 `tests/lightweight/test_arm_mask_v4_2.py` 钉死。
