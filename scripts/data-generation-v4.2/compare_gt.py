...
每个任务出一张两栏大图（每栏 8 帧 ×「原图 | 红遮罩 | 误差图」三联），同时把两栏的
全帧指标写进 metrics.json。GT 只出现在评分与 GT 栏，不参与预测栏的产出。

⚠ **刚性红线闸门**：汇总 16 个任务预测栏的 `false_object_pixels`，非 0 则点名任务并以
非零退出码退出（与 `render_outputs.py` 共用 `enforce_no_false_object`）。
...
from render_outputs import _error_image, _grid, enforce_no_false_object  # noqa: E402
...
def process_task(
    ...
    variants: dict[str, list[np.ndarray]] = {
        "pred": arm_masks_for_episode(predicted, demo_flags, params),
        "ground_truth": [gt == CLASS_ARM for gt in truth],
    }
    ...
    grids = [_panel_grid(frames, variants[key], truth, picks) for key in COLUMN_KEYS]
    height, width = grids[0].shape[:2]
    cell = width // 3

    columns = len(COLUMN_KEYS)
    canvas_w = PAD * 2 + width * columns + GAP * (columns - 1)
    ...
    xs = [PAD + i * (width + GAP) for i in range(columns)]
    ...
    out = Path(out_dir) / f"{task}_compare.png"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"两栏对比：{len(paths)} 个任务 × {episode_name}，并行 {args.workers}")
    ...
    for record in records:
        print(
            f"{record['task']}: 误标物体 {record['pred']['false_object_pixels']} px"
            f" / 召回 {record['pred']['机械臂召回']}"
            f"（GT 栏自校验误标 {record['ground_truth']['false_object_pixels']} px）"
        )
    print(f"共 {len(records)} 张 → {out_dir}")

    # GT 栏按构造恒为 0，不进闸门；闸门只管预测栏
    return enforce_no_false_object(
        f"{episode_name} 逐任务",
        [(record["task"], record["pred"]["false_object_pixels"]) for record in records],
    )
