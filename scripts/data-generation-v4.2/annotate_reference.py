#!/usr/bin/env python3
"""官方旧数据标注入口：对 RoboMME 官方 h5 产出 K=13 网格 mask sidecar。

## 链路定位（用户 2026-08-14 拍板）

v4.2 生成新数据（v4seg-16env-val20ep，val split）**只为标定颜色表**；正式产出是对
**官方旧数据**的标注——`Yinpei/robomme_data_h5` 本地备份（`/data/hongzefu/
robomme_data_h5`，train split，只读参考源，本入口绝不写入）。官方数据**没有 GT
segmentation**，这正是颜色表方案的存在理由：颜色表在自生数据（有 GT）上标定，
推理链（查表 → 四条形态学规则 → K=13 网格）在官方数据上纯前向运行。

范围：每任务前 10 个 episode（ep0-9），共 16 任务 × 10 ep = 80,853 帧（实测）。
官方数据是 train split（MoveCube ep0-4 seed 与 env_metadata/train 逐位一致），
颜色表拟合在 val split——**零 seed 重叠**，本标注是干净的跨 split 泛化。

## 产物（独立 sidecar，官方 h5 保持只读）

`<out-dir>/arm_grid_mask_<Task>.h5` ×16，每 episode 三层全存（用户拍板）：
`arm_grid_mask` (T,32,32) bool【正式产物，K=13】、`arm_cell_counts` (T,32,32)
uint8【换 K 免重跑】、`arm_mask_px` (T,256,256) bool【像素层审计留痕——无 GT 环境
下唯一客观存档，任意 K/涂红帧可由它重建】；另存 `is_video_demo`/`is_completed`
（下游 MotionJEPA build_data_raw_from_h5 切段与 exec 截断必需——刻意**不存**
exec_len，`first_completed + 2` 的截断公式留在下游，防两仓两口径）。主键对齐：
(task, episode_<i>, timestep 序) ↔ MotionJEPA 侧 `<Task>_ep<i>`。

## 金丝雀（迁移风险监测，产物全部落盘后才判，FAIL 退出码 1）

颜色表拟合在自生 val 数据上，跨到官方渲染数据的迁移质量靠两类闸门：
- **未见颜色率**（诊断量，方向安全——未见 ⇒ 不判臂 ⇒ 只会漏标）：相对 v4seg
  评估集同任务基线的倍率 + 绝对值**双判据**（单看倍率会被零基线任务放大成无穷，
  单看绝对值会误杀 RouteStick——其 2% 未见率来自运行时动态创建的路线曲线，是
  任务固有属性，v4seg 基线同为 2.2%）。
- **结构闸门**（真正测「臂丢没丢」）：空像素 mask 帧占比（画面顶部机械臂基座恒
  可见，实测 ep0 全任务最小 mask 614 px，空帧应恒为 0）+ 逐任务判臂率下限。

⚠ **无 GT 的根本盲点**：「颜色在标定集只在臂上见过、在官方数据却落在物体上」
无法量化（官方数据没有 GT 当尺子），只能靠预览图目视——`unseen`/`min` 帧型是
主要观察窗。像素口径的刚性红线（误标物体恒 0）在官方数据上**无法复验**，其保证
仅来自 v4seg 评估集的实测外推。

## 用法（在仓库根）

    # 全量正式（16 任务 × ep0-9，带预览与回读校验，约 2-3 分钟）
...
# ⚠ 刻意不 import enforce_no_false_object：刚性红线在官方数据上没有 GT 可验，
# 网格口径本身也不适用该闸门（见 README 四、五节）。

# 用户 2026-08-14 拍板：全局统一 K=13（全任务/episode/格子位置一体生效）。
# 刻意不提供 CLI 开关——要换 K 就改这里 + 改 README 记录，走 commit。
GRID_MIN_PIXELS = 13

SIDECAR_SCHEMA_VERSION = "ref-arm-grid-mask-v1"
DEFAULT_REFERENCE_ROOT = "/data/hongzefu/robomme_data_h5"
DEFAULT_BASELINE_JSON = SCRIPT_DIR / "outputs" / "json" / "validation_val_ep10-19.json"
...
def iter_reference_frames(
    ...
    """官方数据版帧迭代器：按 timestep 数字序产出 (名, front_rgb, is_video_demo, is_completed)。

    与 `color_model.iter_episode_frames` 的两点差异都是刻意的：
    1. **不读 obs/front_camera_segmentation**——官方数据没有 GT，读了必 KeyError；
...
def annotate_episode_arrays(
    ...
    for name, rgb, is_demo, is_completed in iter_reference_frames(episode_group):
        names.append(name)
        rgbs.append(rgb)
    ...
    # 主键自检：timestep 编号必须恰为 0..T-1（官方数据 160/160 实测成立，破了就停）
...
def load_unseen_baseline(path: Path) -> dict[str, float]:
    """从 v4seg 评估集 validation JSON 读逐任务未见颜色占比当基线。"""
...
def render_episode_previews(
    ...
    # 惰性 import：只有出图才拉 PIL / grid_sweep（连带 render_outputs 等），
    # 主标注链路与测试隔离面都保持窄
    from PIL import Image, ImageDraw, ImageFont

    from grid_sweep import GRID_OVERLAY_ALPHA  # noqa: F401  确认同口径常量存在
    from grid_sweep import TILE_SCALE_AGENT, _diff_image, _grid_overlay, _nearest
    ...
    baseline_text = (
        f"（v4seg 同任务基线 {baseline_rate:.4%}，倍率 "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--episodes", default="0-9", help="官方数据 episode（用户口径：前 10 个）")
    parser.add_argument("--model", default=str(SCRIPT_DIR / "outputs" / "color_model.npz"))
    parser.add_argument(
        "--out-dir",
        default=str(SCRIPT_DIR / "outputs" / "ref_arm_grid_mask_ep0-9"),
        help="sidecar 输出目录（每任务一个 h5）",
    )
    parser.add_argument(
        "--json",
        default=str(SCRIPT_DIR / "outputs" / "json" / "ref_annotate_ep0-9.json"),
    )
    parser.add_argument("--preview-dir", default=None, help="预览图目录；不给则不出图")
    ...
    all_paths = sorted(reference_root.glob("record_dataset_*.h5"))
    if not all_paths:
        raise SystemExit(f"参考数据目录没有 record_dataset_*.h5：{reference_root}")
    if args.tasks != "all":
    ...
    print(
        f"官方数据标注：{len(paths)} 个任务 × episodes {sorted(episodes)}，"
        f"K={GRID_MIN_PIXELS}（{GRID_MIN_PIXELS}/{cell_size * cell_size} = "
        f"{GRID_MIN_PIXELS / cell_size**2:.1%}），并行 {args.workers}"
    )
    ...
    payload = {
        "参数": {
            ...
            "口径": "官方数据 = train split；颜色表拟合在自生 val split（零 seed 重叠，"
            "跨 split 泛化）；官方数据无 GT segmentation，像素刚性红线不可复验",
        ...
        "逐任务": {
            ...
            | {
                ...
                "v4seg基线未见率": baseline.get(item["task"]) if baseline else None,
            }
            for item in results
        },
