...
`env_code` 是任务在 16 任务规范序（`scripts/legacy/data-generation/validate_generated_dataset_contract.py`
...
```bash
uv run python scripts/legacy/data-generation-newSeed/utils/compare_with_metadata.py \
  --env VideoUnmaskSwap,VideoUnmask,ButtonUnmaskSwap,ButtonUnmask \
  --output-dir scripts/data-generation-newSeed/outputs/full
```
...
```bash
uv run python scripts/legacy/data-generation-newSeed/utils/calibrate_parallelism.py \
...
3. **实践含义**：用本脚本无 seed 重跑，**无法逐字复现 train 的 seed 集合**，
   ...
   `scripts/legacy/data-generation/generate_dataset.py` 读死 seed；本目录适用于**产生新数据**。
