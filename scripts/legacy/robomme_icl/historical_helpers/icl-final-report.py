"""正式 v3 全部阶段结束后，独立核对证据并排他写出 VERIFIED_ROOT.json 与 VERIFIED_ROOT.md。"""
...
def read_phases(report: Path) -> dict[str, dict]:
    ...
    require(main.rstrip().endswith("EXIT_CODE=0"), "主脚本尚未成功结束，暂不发布 VERIFIED_ROOT")
...
def render_markdown(result: dict) -> str:
    ...
    lines += ["", "采样窗口按 UTC 阶段时间与 GPU 日志的主机时区对齐。利用率为样本均值；原始间隔统计保存在 VERIFIED_ROOT.json，不把名义500ms当作硬件计数器分辨率。", "",
    ...
    lines += ["", "原始证据：[run_context.json](run_context.json)、[phase_timings.tsv](phase_timings.tsv)、[main.log](main.log)、[gpu_samples.csv](gpu_samples.csv)。VERIFIED_ROOT.json保留原始纳秒时间、ICL_TIMING原文、所有控制文件摘要与本汇总工具全文。", ""]
    return "\n".join(lines)
...
def main(argv=None) -> int:
    ...
    require(not (report / "VERIFIED_ROOT.json").exists() and not (report / "VERIFIED_ROOT.md").exists(), "VERIFIED_ROOT已存在，拒绝覆盖")
    ...
    result = {
        ...
        "evidence": evidence, "report_tool": {**file_evidence(Path(__file__)), "source": Path(__file__).read_text(encoding="utf-8")},
    }
    ...
    with (report / "VERIFIED_ROOT.json").open("x", encoding="utf-8") as stream:
        json.dump(result, stream, ensure_ascii=False, sort_keys=True, indent=2, allow_nan=False)
        stream.write("\n")
    with (report / "VERIFIED_ROOT.md").open("x", encoding="utf-8") as stream:
        stream.write(markdown)
    print(f"最终汇总已完成：{report / 'VERIFIED_ROOT.json'}；{report / 'VERIFIED_ROOT.md'}")
    return 0
