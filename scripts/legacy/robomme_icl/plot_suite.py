...
本工具只读取传入套件的配置快照和 EpisodeSpec，不读取旧版 metadata。
空任务或空难度明确标注无样本，输出图片与逐图来源哈希均保留在仓库内。
运行入口：uv run python scripts/legacy/robomme_icl/plot_suite.py --suite <认证套件> --output <新输出目录>。
"""
