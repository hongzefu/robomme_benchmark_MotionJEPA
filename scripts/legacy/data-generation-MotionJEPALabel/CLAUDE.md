...
| 变体 | 第一次 swap 换的槽位对 | 本链路第二次换的槽位对 | 原版规则会换的槽位对 | |
| --- | --- | --- | --- | :-: |
...
**Button 8 条全部重合**，因为它的最近邻关系恰好是**两两配对**（槽位 0 与 3 互认、1 与 2
互认）—— 无论第一次 swap 把 bin 挪到哪个槽位，配对关系都对称，第二次换的槽位对恒为 (1,2)。
Video 侧的最近邻关系不总是配对（ep99 实测是「0 认 3、1 认 0、2 认 3、3 认 0」），才会分岔。
...
`slot[b]=b`；某次 swap 交换 bin (u,v) 即 `slot[u], slot[v] = slot[v], slot[u]`。
**某次 swap 实际移动的槽位对** = 这两个 bin 在交换**之前**各自所处的槽位。
...
- **窗口 i≥2**：从 Phase 0 原始跑复算出该窗口原本移动的槽位对，变体里注入「**此刻占据
  这两个槽位的那两个 bin**」。于是窗口 i≥2 的 teleport 起止位置、被锁定旁观 bin 的
...
`clip_worker.py` 的 rollout 骨架照抄 `scripts/legacy/data-generation-newSeed/generate_dataset_newseed.py`，除三处：
...
# P0b 计划表（不生成数据）
uv run python scripts/legacy/data-generation-MotionJEPALabel/clip_plan.py
...
uv run python scripts/legacy/data-generation-MotionJEPALabel/make_clip_labels.py --regression
# P1 smoke A：非 train 单源（test:3 的 Video 侧 2~3 条）
uv run python scripts/legacy/data-generation-MotionJEPALabel/generate_swap_clips.py \
...
uv run python scripts/legacy/data-generation-MotionJEPALabel/merge_clip_h5.py \
  --input-dir scripts/data-generation-MotionJEPALabel/outputs/event1 --delete-source
# P4 标签 + 验收 + 简图
uv run python scripts/legacy/data-generation-MotionJEPALabel/make_clip_labels.py \
  --input-dir scripts/data-generation-MotionJEPALabel/outputs/event1
uv run python scripts/legacy/data-generation-MotionJEPALabel/verify_clips.py \
  --gen-dir scripts/data-generation-MotionJEPALabel/outputs/event1
uv run python scripts/legacy/data-generation-MotionJEPALabel/draw_clip_diagrams.py \
  --gen-dir scripts/data-generation-MotionJEPALabel/outputs/event1
