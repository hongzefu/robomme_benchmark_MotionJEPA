...
成品报告在 [`reports/`](../../patternlock-routestick-params/reports/)：一份总览（含 policy 侧 eval 的成功率分析与图）+ 八份分源明细。
...
- **test**：本机从来没有 test 种子域的 h5，一律按 test metadata 的死 seed 实跑生成。
  - PatternLock / RouteStick 用 `scripts/legacy/data-generation-newSeed/generate_dataset_newseed.py --layout test`
...
```bash
# 1. 离线复算四个源
uv run python scripts/legacy/patternlock-routestick-params/derive_episode_params.py

# 2. val 时长（原版 h5，只读）
uv run python scripts/legacy/patternlock-routestick-params/extract_move_durations.py \
...
uv run python scripts/legacy/patternlock-routestick-params/cross_check.py

# 4. test 实跑（tmux 后台，双卡，约 10 分钟；日志用 Monitor 等，别 sleep 轮询）
tmux new-session -d -s plrs-test \
  "set -o pipefail; PYTHONUNBUFFERED=1 uv run python scripts/legacy/data-generation-newSeed/generate_dataset_newseed.py \
...
uv run python scripts/legacy/patternlock-routestick-params/build_report.py \
...
```bash
# val：现成 h5，只读
uv run python scripts/legacy/patternlock-routestick-params/extract_task_params.py \
...
tmux new-session -d -s bp-test \
  "set -o pipefail; PYTHONUNBUFFERED=1 uv run python scripts/legacy/patternlock-routestick-params/run_test_fixed_seed.py \
...
```bash
uv run python scripts/legacy/patternlock-routestick-params/plot_eval_success.py
uv run python scripts/legacy/patternlock-routestick-params/build_report.py \
  --durations scripts/patternlock-routestick-params/outputs/durations_val.json \
  --durations scripts/patternlock-routestick-params/outputs/durations_test.json
```
