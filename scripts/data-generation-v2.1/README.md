# RoboMME 数据生成 v2.1：2D flow ground truth + masked-rgb-v2 遮蔽图

本目录是 `scripts/data-generation-v2/` 的演进版本，**只有遮蔽图口径一处不同**：flow 链路、
契约校验、参考对拍、报告格式全部逐字沿用 v2。整条链路仍然做同一件事：
...
| 方面 | v2（`masked-rgb-v1`） | v2.1（`masked-rgb-v2`） |
|---|---|---|
...
| `setup/` 字段 | `masked_rgb_{schema_version,paint_color,painted,kept}` | 另加 `masked_rgb_black_luminance_max` 与 `masked_rgb_black_exempt/` |
| 报告字段 | `masked_rgb_{seconds,frames}` | 另加 `masked_rgb_{painted,black_exempt}_pixels` |

代码差别集中在 `masked_rgb.py`；`record_wrapper_v2.py` / `generate_dataset.py` 只多接了两个
像素计数与几处文案，其余文件与 v2 逐字相同（`record_wrapper_v2.py` 沿用原文件名与原类名，
因为那三个 override 的机制一个字没变，改名只会制造无谓的 diff）。**`v2` 目录冻结不动**，
旧产物按 `masked-rgb-v1` 的口径解读。
...
| 遮蔽图字段 | 无 | `obs/front_rgb_masked` 与 `setup/masked_rgb_*` |
...
没动，flow 全部挂在 `super()` 调用之后。v2.1 继承了这一点：改的只是 `close()` 之后那段后处理里
「哪些像素涂棕」，`step()` 与父类照旧一个字没动。

## 遮蔽口径（`masked-rgb-v2`）

**涂色白名单只有一类**：机器人 articulation 下的全部 link。其余一律原样保留——桌面、地面
`ground`、全部任务物体、以及任何没认出来的东西。坚持白名单而不是「除了 XX 都涂」，是因为
黑名单遇到新出现的未知物体会默认涂掉它、静默毁掉数据；白名单最多是少涂一块，肉眼一看就知道。

**唯一的例外是逐像素的黑色豁免**：`panda_leftfinger` / `panda_rightfinger` 两个 link 内，
三通道均值 `≤ 100` 的像素不涂。这就是「gripper 头部的黑色接触面」，也是全画面唯一留下的
机器人部位。
...
材质，而 ManiSkill 的 segmentation 是 **link 级**的——seg id 这一层根本切不开。

阈值 `100` 的依据是在 16 任务既有产物上实测的手指像素亮度分布：**暗簇 38–59、亮簇 142–231，
中间 60–141 完全是空档**，阈值落在空档正中，往两边挪 40 个灰度级结果都不变。判定用整数和
`R+G+B ≤ 3×阈值` 算，与「均值 ≤ 阈值」严格等价。

黑色豁免**只作用于手指 link**：手掌 `panda_hand` 与腕部相机支架上也有黑色方块，那些不属于
夹爪接触面，按口径一起涂掉。`panda_stick`（RouteStick / PatternLock）没有手指，豁免集为空，
整个机器人连那根棍一起涂掉——棍与手掌同属 `panda_hand` 一个 link，本来也分不开。

**验收信号**：报告里的 `masked_rgb_black_exempt_pixels`。手指在 256×256 里只占几十个像素，
光看涂色总数看不出豁免有没有生效；这个计数恒为 0 就说明阈值或手指 seg id 解析出了问题。

权威 schema 见 [doc/h5_data_format.md](../../doc/h5_data_format.md) 的「纯色棕遮蔽图
（`masked-rgb-v2`）」一节。
...
| `masked_rgb.py` | 纯色棕遮蔽图：涂色对象枚举（白名单）、黑色指尖逐像素豁免、单帧涂色、h5 写入 |
...
```bash
env CUDA_VISIBLE_DEVICES=0 uv run --locked scripts/data-generation-v2.1/generate_dataset.py \
  --output-dir artifacts/generated/v21mask-smoke \
...
```bash
env CUDA_VISIBLE_DEVICES=0 uv run --locked scripts/data-generation-v2.1/generate_dataset.py \
  --output-dir artifacts/generated/flow-16env-v21mask \
...
# 基准
env CUDA_VISIBLE_DEVICES=0 uv run --locked scripts/data-generation-v2.1/generate_dataset.py \
  --output-dir artifacts/generated/v21mask-16env-baseline \
  --env all --episodes 1 --workers 1 --gpus 0 \
  --reference-root /data/hongzefu/robomme_data_h5 \
  --no-flow --no-masked-rgb --no-reference-validation

# 对拍
uv run --no-sync python scripts/data-generation-v2.1/verify_joint_action_bitexact.py \
  --baseline artifacts/generated/v21mask-16env-baseline \
  --candidates artifacts/generated/v21mask-16env
```
...
```bash
uv run --no-sync python scripts/data-generation-v2.1/verify_flow_math.py \
...
```bash
uv run --no-sync python scripts/data-generation-v2.1/fetch_reference_h5.py --tasks all
```
...
scripts/data-generation-v2.1/reports/
