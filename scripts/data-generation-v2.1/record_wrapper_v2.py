...
class RobommeRecordWrapperV2(RobommeRecordWrapper):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # masked 图全部在 close() 里算，不占 step() 一丝一毫，这几个量只为报告可观测。
        # 两个像素计数是 v2.1 新增的验收信号：黑色指尖是逐像素豁免出来的，
        # 豁免像素恒为 0 就说明阈值或手指 seg id 解析出了问题，只看涂色总数发现不了。
        self.masked_rgb_seconds = 0.0
        self.masked_rgb_frames = 0
        self.masked_rgb_painted_pixels = 0
        self.masked_rgb_black_exempt_pixels = 0
        ...
        if write_flow or write_masked:
            with h5py.File(dataset_path, "a") as handle:
                ...
                if write_masked:
                    ...
                    self.masked_rgb_painted_pixels = painter.painted_pixel_total
                    self.masked_rgb_black_exempt_pixels = painter.black_exempt_pixel_total
