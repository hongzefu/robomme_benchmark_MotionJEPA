...
ENCODING = {
    ...
    "crf": 18,
    "threads": 1,
    "layout": "base_rgb+wrist_rgb",
    "demonstration_border_rgb": [255, 0, 0],
...
def _probe(path, *, frames, fps, width, height):
    ...
    result = subprocess.run(
        [
            ...
            "v:0",
            "-show_entries",
            "stream=width,height,codec_name,pix_fmt,nb_read_frames,avg_frame_rate,r_frame_rate",
            "-of",
            "json",
            str(path),
        ],
...
def _read_sidecar(path, identity):
    ...
    if (
        not isinstance(value.get("video_sha256"), str)
        or len(value["video_sha256"]) != 64
    ):
...
def _verify_existing(video_path, metadata, identity):
    ...
    _probe(
        video_path,
        **{key: identity[key] for key in ("frames", "fps", "width", "height")},
    )
...
def export_video(h5_path, video_path) -> dict:
    ...
    frames = [
        frame for frame in record.frames if frame["info"].get("deliver_frame", False)
    ]
    if not frames:
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(
            prefix=f".{target.stem}-", suffix=".mp4", dir=target.parent, delete=False
        ) as stream:
        ...
        with tempfile.TemporaryFile(dir=target.parent) as errors:
            ...
            try:
                ...
                errors.seek(0)
                raise RuntimeError(
                    f"FFmpeg 编码失败：{errors.read().decode(errors='replace')}"
                ) from exc
            if code != 0:
                errors.seek(0)
                raise RuntimeError(
                    f"FFmpeg 编码退出 {code}：{errors.read().decode(errors='replace')}"
                )
        verification = _probe(
            temporary_video, frames=len(frames), fps=fps, width=width, height=height
        )
        result = {
            "schema_version": 1,
            **identity,
            "source_h5": str(source),
            "video_sha256": _sha256(temporary_video),
