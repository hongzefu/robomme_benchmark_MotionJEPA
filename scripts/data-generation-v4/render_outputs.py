...
对验证集 episode 逐帧跑「颜色表分类 → 四条形态学规则」得到机械臂 mask，涂成纯红，
再拿 ground truth segmentation 当尺子把结果量化。**GT 只出现在评分环节，不参与产出**。
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        action="store_false",
        default=True,
        help="关掉共享色否决（纯 argmax），用于与默认口径做对照",
    )
