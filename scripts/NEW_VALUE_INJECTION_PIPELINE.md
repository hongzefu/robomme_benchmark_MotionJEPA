...
跑前分布还有一套更细的 2D 图（每组两张绝对坐标图：全部物体的初始位置叠加、全部随机事件画进桌面坐标，共 22 张）与按 11 组分难度的「事件 / 取值域 / 分配 / 结果分布」表，见 [injection-before-2d/NEW_VALUE_DISTRIBUTION_BEFORE.md](injection-before-2d/NEW_VALUE_DISTRIBUTION_BEFORE.md)。
