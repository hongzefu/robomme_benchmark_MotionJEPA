...
跑前分布还有一套更细的 2D 图（只画实跑范围前 30 条，每组三种：初始位置、随机事件、单个 episode，共 77 张，放在 `injection-before-2d/figures/`）与按 11 组分难度、各分「初始化」「事件」两张的「事件 / 取值域 / 分配 / 结果分布」表，见 [injection-before-2d/NEW_VALUE_DISTRIBUTION_BEFORE.md](injection-before-2d/NEW_VALUE_DISTRIBUTION_BEFORE.md)。
