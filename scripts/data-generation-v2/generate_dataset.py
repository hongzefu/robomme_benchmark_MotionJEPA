#!/usr/bin/env python3
"""RoboMME 数据生成的唯一入口（v2：带 2D flow ground truth）。
...
def _args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="RoboMME 数据生成与校验（v2，带 2D flow ground truth）；最新的完整报告始终写到 scripts/data-generation-v2/reports/ 下的 generation_report.json 与 generation_report.md")
