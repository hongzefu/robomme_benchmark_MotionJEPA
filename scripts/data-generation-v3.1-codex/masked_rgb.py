...
v3.1 保留 v3 的字段名与 H5 结构，方便现有训练与 flow 回放代码继续读取。生产输入只有本
episode 的 ``front_rgb`` 与 ``front_depth``，不读取 segmentation，也不根据任务、对象、机器人
或 link 名称做判断。``cv_arm_removal.py`` 先找边缘进入的低饱和机械臂候选，再用 RGB-D 时序证据
从候选中扣除任务物体；普通双指夹爪只额外扣除稳定轨迹中的黑色指尖，白色指身仍删除，stick
不做黑色豁免。未命中掩码的原图像素逐位不变，因此桌面不会像 v2 那样整块被涂成纯色。
"""
