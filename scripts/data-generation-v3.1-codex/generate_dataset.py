#!/usr/bin/env python3
"""RoboMME 数据生成的唯一入口（v3.1-codex：flow + 纯 RGB-D 物体保护去机械臂图）。

一次完整调用依次做四件事：**并行生成 → 按任务合并 → 契约校验与参考对拍 → 写报告**。

相对 v1 的差别集中在以下几处，都是「只增」：

- 换用 `record_wrapper_v3.RobommeRecordWrapperV3`（薄子类，父类 RecordWrapper 一字未改）；
- 新增 `--reference-root`：v1 把官方参考路径硬编码成仓库内 `data/robomme_data_h5` 且没有开关，
  参考数据在仓库外时根本跑不起来；
- 新增 `--flow` / `--no-flow`：控制是否采集 flow。关掉时行为与父类完全一致，专供自对拍基线；
- 新增 `--masked-rgb` / `--no-masked-rgb`：用 episode 的原始 RGB-D 保护任务物体并删除机械臂、
  腕部和夹爪非黑色部分；普通双指夹爪仅保留稳定轨迹对应的黑色指尖，stick 不做黑色豁免，同时
  保留 mask 外的桌面原像素；
...
def _worker(job: EpisodeJob) -> dict[str, Any]:
    ...
    try:
        ...
        # v3 链路用带 flow 与纯 CV 去臂图采集的薄子类；两个开关都关时行为与父类完全一致
        from record_wrapper_v3 import RobommeRecordWrapperV3
        ...
        record_env = RobommeRecordWrapperV3(
            base_env,
...
def _args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="RoboMME 数据生成与校验（v3.1-codex：flow + 纯 RGB-D 物体保护去机械臂图）；最新完整报告始终写到 scripts/data-generation-v3.1-codex/reports/ 下")
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="采集并写入 2D flow ground truth（默认开启）",
    )
    parser.add_argument(
        "--no-flow",
        dest="record_flow",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="写入 obs/front_rgb_masked（默认开启）：仅用 RGB-D 通用 CV 规则保护任务物体并删除机械臂/"
        "夹爪非黑色部分；普通双指夹爪只保留黑色指尖，stick 不豁免，再用 episode 时序背景补回遮挡"
        "区域，未命中区域（包括桌面）逐位保留",
    )
    parser.add_argument(
        "--no-masked-rgb",
