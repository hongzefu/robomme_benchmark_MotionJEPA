...
def render_markdown(report: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# 数据生成报告（v3.1-codex：flow + 纯 RGB-D 物体保护去机械臂图）",
        "",
        "## 运行来源信息",
        "",
...
def _args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="对已存在的产物做只读复核，并把最新的完整报告写到 scripts/data-generation-v3.1-codex/reports/ 下")
