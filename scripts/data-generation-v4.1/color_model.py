...
v4.1 的口径比 v4 更弱（用户 2026-08-12 拍板）：GT 不再提供背景 / 物体 / 机械臂的
...
- **推理**（`ColorModel.classify`）：逐像素查表，按**各列各自归一化的似然**取 argmax
  （类先验不参与），默认再叠一条混合支撑否决。之后的形态学、连通域、时间平滑全都
  只看这个标签图，再不碰 GT。
...
- `veto_shared` 默认开：argmax 判臂后，凡混合列计数 > 0 的颜色一律改判混合——
...
class ColorModel:
    ...
    def classify(self, rgb: np.ndarray, veto_shared: bool = True) -> np.ndarray:
        ...
        判别是**归一化似然 argmax**：每列先除以本列像素总量得 `P(颜色 | 类)`，再取
        最大者。类先验（各类像素量之比）刻意不参与——混合列里物体占多少是未知的，
        任何用到计数绝对量对比的规则都在偷用先验。列序 (背景, 混合, 臂) 使 `argmax`
        平手时偏向非臂，判臂必须严格胜出。**不做「混合 − 背景」减法**（那等于变相
        恢复物体分布，违背先验未知的设定）。

        `veto_shared` 打开时多一条**混合支撑否决**：某颜色只要在混合分布（背景∪物体
        场景）里出现过（混合列计数 > 0），就一律不判机械臂，改判混合。理由与 v4 的
        共享色否决同源：无臂场景里出现过的颜色无法排除属于物体，而「绝不误标物体、
        允许少量机械臂没被标进去」要求往保守方向倒。门限仍是「出现过 / 没出现过」，
        即 0，不引入新阈值。

        （v4 三类表时代的实测背景：纯 argmax 的物体误标全是被夹爪按压的白色按钮顶面、
