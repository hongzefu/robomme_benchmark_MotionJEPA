...
`compare_veto.py` 只给最终结果，看不出中间发生了什么。本入口把同一帧在
**颜色判决 → 四条形态学规则**上的每一步都画出来，veto / noveto 各一行，并把每一步
**删掉的像素单独标红**——四条规则的设计原则是「每一条都只会让标定区域变小或持平」，
这张图就是它的逐格证据。

每个任务出两张：

- `<Task>_walkthrough.png`：**逐阶段走查**。上下两行 = noveto / veto，每行七格：
  ...
  再加底部一条参照带：GT 三类图 / GT 臂 / 红遮罩 / 误差图 / 两口径最终 mask 的差异。

- `<Task>_pixels.png`：**逐像素举例**。在原图上点几个代表性像素（真臂、真物体、
  被否决翻转的共享色、noveto 误标点），逐个列出它的 24 位颜色、三列归一化似然、
  两种口径下的判决与 GT 真身——把「查表 argmax + 否决」这条规则落到具体数字上。

选帧规则：优先取该 episode 里 **noveto 误标物体像素最多的那一帧**（最能说明否决在挡
什么）；整段都没有误标时退化成取臂像素最多的一帧。选中的帧号写在图注里。
...
用法：
    ...
    uv run --no-sync python scripts/data-generation-v4.1/segmentation_walkthrough.py \\
      ...
      --episode 10 --out scripts/data-generation-v4.1/outputs/walkthrough
"""
...
def _pick_frame(
    ...
    """选走查帧：noveto 误标物体最多的一帧；整段无误标则取臂像素最多的一帧。"""
...
def _render_walkthrough(
    ...
    height = PAD * 2 + TITLE_H + row_h * 2 + GAP * 2 + LEGEND_H + row_h + GAP
    ...
    for row_index, (key, label) in enumerate(
        ...
        tiles: list[tuple[str, np.ndarray, str]] = [
            ...
            (
                "颜色判决（查表 argmax）",
                _label_image(predicted[key]),
                f"判臂 {int((predicted[key] == CLASS_ARM).sum())} px",
            ),
        ]
        previous: np.ndarray | None = None
        for stage_index, name in enumerate(STAGE_NAMES):
            mask = stages[key][stage_index]
            kept = int(mask.sum())
            # ⚠ 增量必须带符号：③ 时间平滑是多数表决，**会补像素**（本帧不在候选、
            # 前后帧在），实测 InsertPeg 走查帧就是 +4。写死成「−」会打出「−−4」。
            delta = ""
            if previous is not None:
            ...
            previous = mask
        used = _paste_row(canvas, draw, tiles, x0, y, head=row_index == 0)
        block_y = y + (HEAD_H if row_index == 0 else 0)
        draw.multiline_text(
            (PAD, block_y + TILE // 2 - 24),
            label,
            font=_font(19),
            fill=(120, 235, 140) if key == "veto" else (250, 170, 90),
            align="center",
            spacing=6,
        )
        y += used + GAP
    ...
    final_noveto = stages["noveto"][-1]
    final_veto = stages["veto"][-1]
    only_noveto = final_noveto & ~final_veto
    diff = np.zeros((*final_veto.shape, 3), np.uint8)
    diff[final_veto] = KEEP
    diff[only_noveto] = DROP
    reference: list[tuple[str, np.ndarray, str]] = [
        ...
        (
            "veto 红遮罩（产物）",
            apply_red_mask(rgb, final_veto),
            f"标定 {int(final_veto.sum())} px",
        ),
        (
            "veto 误差图",
            _error_image(final_veto, gt),
            f"误标物体 {int((final_veto & (gt == CLASS_OBJECT)).sum())} px",
        ),
        (
            "noveto 误差图",
            _error_image(final_noveto, gt),
            f"误标物体 {int((final_noveto & (gt == CLASS_OBJECT)).sum())} px",
        ),
        (
            "两口径差异",
            diff,
            f"noveto 多标 {int(only_noveto.sum())} px",
        ),
    ]
...
def _sample_pixels(
    ...
    noveto: np.ndarray,
    veto: np.ndarray,
    final_noveto: np.ndarray,
    final_veto: np.ndarray,
) -> list[dict[str, Any]]:
    """挑一组能讲清规则的代表性像素，每类最多两个，按「解释力」排序。

    刻意不随机采样——要的是四种典型情形各一个：两口径都判臂的真臂、被否决翻转的共享色
    真臂（否决的代价）、noveto 误标的物体（否决在挡什么）、以及背景与未见色各一个。
    """
    packed = pack_rgb(rgb)
    ...
    take(is_arm & final_veto, "真臂 · 两口径都标到", 2)
    take(is_arm & (noveto == CLASS_ARM) & (veto == CLASS_MIX), "真臂 · 被否决翻转（漏标代价）", 2)
    take(is_object & final_noveto, "真物体 · noveto 误标（否决挡下的）", 2)
    take(is_arm & (noveto == CLASS_UNKNOWN), "真臂 · 未见色 UNKNOWN")
    take(is_object & (veto == CLASS_MIX) & (noveto == CLASS_MIX), "真物体 · 两口径都判混合")
        ...
        detailed.append(
            {
                **item,
                ...
                "noveto": int(noveto[y, x]),
                "veto": int(veto[y, x]),
                "gt": int(gt[y, x]),
                "final_noveto": bool(final_noveto[y, x]),
                "final_veto": bool(final_veto[y, x]),
            }
        )
    return detailed
...
def _render_pixels(
    ...
    for index, item in enumerate(samples):
        ...
        values = [
            ...
            DECISION_NAMES[item["noveto"]],
            DECISION_NAMES[item["veto"]],
            GT_NAMES[item["gt"]],
            item["note"],
        ]
        x = x0
        for (title, column_w), value in zip(columns, values):
            if value is None:
                ...
                color = FG
                if title == "veto" and item["veto"] != item["noveto"]:
                    color = (255, 120, 120)
                if title == "这个像素说明什么":
                    color = DIM
    ...
    for offset, (line, color) in enumerate(
        (
            ...
            ("③ 混合支撑否决（veto，默认开）：argmax 判臂后，凡混合列计数 > 0（= 该色在无臂场景里出现过）一律改判混合。", (255, 150, 150)),
            ("　 门限就是「出现过 / 没出现过」，不引入任何新阈值；上表 veto 列标红的行就是被这一条翻掉的。", (255, 150, 150)),
...
def process_task(
    ...
    predicted = {
        key: [model.classify(rgb, veto) for _, rgb, _, _ in frames]
        for key, veto in (("noveto", False), ("veto", True))
    }
    stages = {
        key: stagewise_masks(labels, demo_flags, params)
        for key, labels in predicted.items()
    }

    index, false_object = _pick_frame(stages["noveto"][-1], truth)
    rgb = frames[index][1]
    _render_walkthrough(
        task,
        episode_name,
        ...
        rgb,
        truth[index],
        {key: value[index] for key, value in predicted.items()},
        {key: [stage[index] for stage in value] for key, value in stages.items()},
    ...
    samples = _sample_pixels(
        ...
        truth[index],
        model,
        predicted["noveto"][index],
        predicted["veto"][index],
        stages["noveto"][-1][index],
        stages["veto"][-1][index],
    )
    _render_pixels(
    ...
    return {
        ...
        "frames": len(frames),
        "走查帧": index,
        "该帧 noveto 误标物体": false_object,
        "该帧各阶段像素数": {
            key: {
                name: int(stage[index].sum())
                for name, stage in zip(STAGE_NAMES, value)
            }
            for key, value in stages.items()
        },
        "该帧最终标定": {
            key: int(value[-1][index].sum()) for key, value in stages.items()
        },
        "逐像素举例": samples,
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    for record in records:
        print(
            ...
            f"noveto 标定 {record['该帧最终标定']['noveto']} px / "
            f"veto 标定 {record['该帧最终标定']['veto']} px，"
            f"该帧 noveto 误标物体 {record['该帧 noveto 误标物体']} px"
        )
