#!/usr/bin/env python3
"""通用 computer vision 规则的机械臂删除（红色遮罩版）核心模块。

本模块回答一件事：**只靠通用图像规则**（不碰任何仿真真值），在静止相机的 RoboMME
`front_rgb` 序列里逐帧找出机械臂（含夹爪）与其阴影的像素。找到的像素由调用方涂成
纯红遮罩——本模块只产 mask，不做任何填充（用户口径：删除的像素用红色遮罩，不填充
其他像素）。
