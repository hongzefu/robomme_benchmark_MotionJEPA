...
def build_length_section(lengths: dict[str, list[dict[str, Any]]]) -> list[str]:
    ...
    lines += [
        ...
        "**帧路 N=8**（红点）、**motion 窗口**（demo 蓝 / exec 绿，按 33 帧全宽画；相邻窗口只错开 "
        "16 帧、重叠一半，同一行会粘连，所以按 3 行轮流堆叠——同一行内起点差 48 > 33，互不接触，"
        "可以逐个数清）、"
        "**帧路 N=32**（紫色细竖线）。段短于 33 帧铺不出窗口，画成橙色虚线空框。"
        "右侧标 T、`demo窗+exec窗`、两个 Δ。",
        "",
        f"> **交互版**（难度档切换、悬停看 subgoal 原文与每个窗口的区间、横轴跨档固定，另含 eval 成功率视图）："
        f"[采样窗口与 eval 成功率](https://claude.ai/code/artifact/093a467c-d567-466b-b57e-e4fdee2bcac0)",
        "",
    ]
    for task in keys:
