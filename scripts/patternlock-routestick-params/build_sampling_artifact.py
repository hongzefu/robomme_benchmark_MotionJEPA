...
<div class="wrap">
  <header style="display:flex;flex-direction:column;gap:9px">
    ...
    <div class="tabs" id="bands"><span class="lbl">长度档</span></div>
  </header>
...
// 长度档：每个 (任务, 难度) 里按整条长度 T 排序取的三条代表集
const BANDS = ["全部", "最短", "中位", "最长"];
let curDiff = "all", curBand = "全部";
...
function render(){
  ...
  const keep = r => (curDiff === "all" || r.diff === curDiff) &&
                    (curBand === "全部" || r.band === curBand);
  for (const suite of ["Imitation", "Counting"]){
    const tasks = Object.keys(DATA).filter(t => SUITES[t] === suite);
    const rows = tasks.flatMap(t => DATA[t].filter(keep).map(r => [t, r]));
  ...
  if (!shown){
    ...
    empty.className = "suite";
    empty.innerHTML = `<span class="dt">当前筛选没有匹配的 episode</span>`;
...
function buildTabs(host, items, initial, onPick){
  for (const item of items){
    const b = document.createElement("button");
    b.className = "tab"; b.textContent = item;
    b.setAttribute("aria-pressed", String(item === initial));
    b.onclick = () => {
      for (const t of host.querySelectorAll(".tab")) t.setAttribute("aria-pressed", "false");
...
buildTabs(tabs, DIFFS, curDiff, v => { curDiff = v; });
buildTabs(bands, BANDS, curBand, v => { curBand = v; });
for (const [key, label] of [["axis", "采样窗口数轴"], ["eval", "eval 成功率"]]){
  ...
  b.onclick = () => {
    ...
    tabs.hidden = !isAxis;
    document.getElementById("bands").hidden = !isAxis;
