#!/usr/bin/env python3
"""RoboMME 数据生成的唯一入口（v3-codex：flow + 纯 CV 去机械臂图）。
...
- 新增 `--masked-rgb` / `--no-masked-rgb`：用 episode 的原始 RGB-D 通过通用 CV 规则删除
  机械臂、腕部和夹爪，同时保留 mask 外的桌面原像素；
...
def _args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="RoboMME 数据生成与校验（v3-codex：flow + 纯 CV 去机械臂图）；最新完整报告始终写到 scripts/data-generation-v3-codex/reports/ 下")
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="写入 obs/front_rgb_masked（默认开启）：仅用 RGB-D 通用 CV 规则检测并删除机械臂/夹爪，"
        "用 episode 时序背景补回遮挡区域，未命中区域（包括桌面）逐位保留",
    )
    parser.add_argument(
        "--no-masked-rgb",
        dest="record_masked_rgb",
