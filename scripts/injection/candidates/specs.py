...
# 观测通道只复制已产生的提案；不参与随机流、旧统计和旧规格。
_OBSERVER: ContextVar[Callable[[dict[str, Any]], None] | None] = ContextVar("candidate_observer", default=None)
_GROUP: ContextVar[tuple[str, str] | None] = ContextVar("candidate_group", default=None)
_PROPOSAL: ContextVar[dict[str, Any] | None] = ContextVar("candidate_proposal", default=None)


def _observe_proposal(episode: int, trial: int, object_id: str | None, proposal: Any) -> None:
    if _OBSERVER.get() is None:
        return
    task, difficulty = _GROUP.get()
    _PROPOSAL.set(dict(task=task, difficulty=difficulty, episode=episode,
                       object_id=object_id, trial=trial + 1, proposal=copy.deepcopy(proposal)))


def _observe_outcome(reason: str | None, detail: Any = None) -> None:
    observer = _OBSERVER.get()
    if observer is not None:
        proposal = _PROPOSAL.get()
        if proposal is None:
            raise SpecGenerationError("观测结果缺少对应提案")
        observer(copy.deepcopy({**proposal, "reason": reason, "detail": detail}))
        _PROPOSAL.set(None)
...
def _count_rejection(stats: dict[str, Any], rejection: bc.CollisionRejection | None, *, geometry: bool = False) -> None:
    if geometry:
        _observe_outcome("geometry")
        stats["rejected_geometry"] += 1
        return
    if rejection is None:
        return
    _observe_outcome(rejection.reason, rejection.as_dict())
    ...
    **不锁在原细分层格里**。格宽只有 0.0036 米，被按钮或孔板压住时同格重抽 256 次
    ...
    for order_index, (color_name, color_idx) in enumerate(tasks):
        ...
        for trial in range(MAX_CANDIDATES):
            ...
            _observe_proposal(episode, trial, f"cube_{color_name}_{color_idx}", {"xy": [x, y], "yaw_rad": yaw})
            if any(obb2d_intersect(*obstacle, *candidate) for obstacle in obstacles):
                _observe_outcome("geometry")
                stats["rejected_geometry"] += 1
                continue
            _observe_outcome(None)
            found = (x, y, yaw)
            break
        if found is None:
            return None
...
def _unmask_group(difficulty: str, gc: GroupContract, config: dict[str, Any], parameters: dict[str, Any], positions: dict[str, Any], seed: int, *, blocks: int = 1) -> GroupResult:
    ...
    for episode in range(GROUP_SIZE * blocks):
        ...
        for trial in range(MAX_CANDIDATES):
            ...
            _observe_proposal(episode, trial, None, {"theta_rad": theta, "bins": bins})
            states = {}
...
def _repick_group(difficulty: str, gc: GroupContract, config: dict[str, Any], parameters: dict[str, Any], positions: dict[str, Any], seed: int, *, blocks: int = 1) -> GroupResult:
    ...
    for episode in range(GROUP_SIZE * blocks):
        ...
        for trial in range(MAX_CANDIDATES):
            ...
            _observe_proposal(episode, trial, None, {"theta_rad": theta, "button_xy": button_xy, "cubes": cubes})
...
def build_group(task: str, difficulty: str, sampling: dict[str, Any], contract: Contract, seed: int = DEFAULT_SEED, *, blocks: int = 1, observer: Callable[[dict[str, Any]], None] | None = None) -> GroupResult:
    """在独立上下文中采集提案，异常时也复原通道，不影响其他组。"""
    tokens = (_OBSERVER.set(observer), _GROUP.set((task, difficulty)), _PROPOSAL.set(None))
    try:
        return _build_group(task, difficulty, sampling, contract, seed, blocks=blocks)
    finally:
        _PROPOSAL.reset(tokens[2])
        _GROUP.reset(tokens[1])
        _OBSERVER.reset(tokens[0])


def _build_group(task: str, difficulty: str, sampling: dict[str, Any], contract: Contract, seed: int = DEFAULT_SEED, *, blocks: int = 1) -> GroupResult:
