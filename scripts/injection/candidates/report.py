...
from .contract import Contract, ContractError, GroupContract, load_contract  # noqa: E402
...
DEFAULT_RUN_ID = "20260912-contract-v3-10"  # 14 组的冻结规格（与 06 逐条相同，OLD_GROUPS_EQUIVALENCE compared=1400 differences=0）
BEGIN = "<!-- AUTO:EVENT_TABLES BEGIN -->"
END = "<!-- AUTO:EVENT_TABLES END -->"

from ..rollout.windows import GROUPS  # noqa: E402  # 14 组的唯一真源（纯标准库模块）
COARSE_BINS = 10
...
def load_sampling(root: Path) -> dict[str, Any]:
    ...
    return load_candidates(root / "candidates/candidates.jsonl", repo_root=REPO_ROOT)[0]["sampling_config"]
...
def load_group(root: Path, task: str, difficulty: str) -> list[dict[str, Any]]:
    from .io import load_candidates
    _, rows = load_candidates(root / "candidates/candidates.jsonl", repo_root=REPO_ROOT)
    return [r["spec"] for r in rows if (r["task"], r["difficulty"]) == (task, difficulty)]
...
def render_all(root: Path, sampling: dict[str, Any], contract: Contract) -> tuple[str, int]:
    ...
    for task, difficulty in GROUPS:
        if f"{task}/{difficulty}" not in header["group_provenance"]:
            continue
        records = [r["spec"] for r in candidates if (r["task"], r["difficulty"]) == (task, difficulty)]
        text, n = render_group_table(task, difficulty, records, sampling, contract)
        blocks.append(text)
        total += n
    return "\n" + "\n\n".join(blocks) + "\n", total
...
def load_contract_for(root: Path, override: str | None = None) -> Contract:
    """表格取值域引用保留的契约文件，散列必须与候选 header 一致。"""
    from .io import load_candidates
    header, _ = load_candidates(root / "candidates/candidates.jsonl", repo_root=REPO_ROOT)
    path = Path(override) if override else REPO_ROOT / header["contract"]
    contract = load_contract(path)
    if contract.sha256 != header["contract_sha256"]:
        raise ContractError("表格契约与候选身份记录不一致")
    return contract


def check(run_id=DEFAULT_RUN_ID, artifacts_root=None, contract_path=None):
    root = (Path(artifacts_root) if artifacts_root else REPO_ROOT / "artifacts/injection") / run_id
    block, rows = render_all(root, load_sampling(root), load_contract_for(root, contract_path))
    _, current, _ = split_region((root / "candidates/DISTRIBUTION.md").read_text(encoding="utf-8"))
    drift = sum(line[:1] in "+-" for line in difflib.ndiff(current.splitlines(), block.splitlines()))
    return drift == 0, rows, drift


def generate(root: Path):
    from .io import load_candidates
    from ..rollout.state import atomic_text
    header, _ = load_candidates(root / "candidates/candidates.jsonl", repo_root=REPO_ROOT)
    block, rows = render_all(root, load_sampling(root), load_contract_for(root))
    lines = ["# 候选分布", "", f"运行：`{header['run_id']}`，共 {header['candidates']} 条候选。",
             "", "## 取值域、分配与结果", "",
             "统计表覆盖每组全部候选；连续量按每 100 条独立 block 分层，位置重试和整体布局重试的计数单位不同。",
             "候选能通过静态筛查不等于可完成任务，实跑结果见下一阶段的结果表。", "",
             BEGIN + block + END, "", "## 三类图的读法", "",
    ...
    for task, difficulty in GROUPS:
        ...
        if key not in header["group_provenance"]:
            continue
        base = f"figures/{key}"
        pages = " ".join(f"[{i}]({base}/3_episodes_p{i}.png)" for i in range(1, 6))
        lines.append(f"| {key} | [图 1]({base}/1_positions.png) | [图 2]({base}/2_events.png) | {pages} |")
    atomic_text(root / "candidates/DISTRIBUTION.md", "\n".join(lines) + "\n")
    atomic_text(root / "candidates/logs/event_tables.md", block)
    print(f"DISTRIBUTION=PASS groups={header['groups']} rows={rows}", flush=True)
    return block


def main():
    from ..rollout.state import run_root
    parser = argparse.ArgumentParser(description="候选分布报告与事件表零漂移核验")
    parser.add_argument("--run-id", required=True)
    parser.add_argument("--check", action="store_true")
    args = parser.parse_args()
    if args.check:
        ok, rows, drift = check(args.run_id)
        print(f"EVENT_TABLES={'PASS' if ok else 'FAIL'} rows={rows} drift={drift}")
