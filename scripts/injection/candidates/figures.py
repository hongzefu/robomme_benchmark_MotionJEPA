...
**视角与回放视频对齐**：回放视频左起第一格是 ``base_camera``（eye (0.3, 0, 0.4) → target (0, 0, −0.2)，从机器人一侧
俯视），画面右 = 世界 +y，画面上 = 世界 −x（远离机器人），机器人在画面下方。本脚本全部图都按这个视角画：
绘图坐标 (u, v) = (y, −x)，这是绕 z 轴 −90° 的纯旋转（行列式 +1），所以方块朝向与顺／逆时针弧向都不需要改。
2026-09-11 用 BinFill/easy ep0 与 VideoUnmaskSwap/easy ep0 的视频首帧逐物体对过。
...
from ..rollout.windows import GROUPS  # noqa: E402  # 14 组的唯一真源
...
# 几何常量（与 scripts/injection/candidates/specs.py 一致，这里照抄数值不 import）
CUBE_HALF = 0.02
...
def view(xy) -> tuple[float, float]:
    """世界 (x, y) → 视频视角绘图坐标 (u, v) = (y, −x)：画面右 = +y，画面上 = −x。"""
    return (xy[1], -xy[0])


def square(ax, xy, side: float, angle_deg: float, **kw) -> None:
    u, v = view(xy)
    # 纯旋转 −90°，正方形转 90° 形状不变，朝向角照用
    ax.add_patch(Rectangle((u - side / 2, v - side / 2), side, side, angle=angle_deg, rotation_point="center", **kw))


def circle(ax, xy, radius: float, **kw) -> None:
    ax.add_patch(Circle(view(xy), radius, **kw))


def region_box(ax, x_lo, x_hi, y_lo, y_hi, label: str, color="#455a64") -> None:
    """合法区按世界 x/y 范围给，画到视角坐标：u∈[y_lo,y_hi]，v∈[−x_hi,−x_lo]；标签放框的左上角。"""
    ax.add_patch(Rectangle((y_lo, -x_hi), y_hi - y_lo, x_hi - x_lo, fill=False, linestyle="--",
                           linewidth=1.4, edgecolor=color, zorder=1))
    ax.text(y_lo, -x_lo, label, fontsize=10, color=color, va="bottom", ha="left", zorder=9,
            bbox=dict(facecolor="white", alpha=0.85, edgecolor="none", pad=1.5))


def style_axes(ax, title: str, xlim, ylim) -> None:
    """xlim/ylim 仍按世界 x/y 给（episode_limits 的口径），内部换成视角坐标。"""
    ...
    ax.set_xlabel("世界 y / 米（画面右 = +y）", fontsize=11)
    ax.set_ylabel("世界 x / 米（画面上 = −x，远离机器人）", fontsize=11)
    ax.text(0.5, 0.01, "▼ 机器人在此侧（+x）", transform=ax.transAxes, ha="center", va="bottom", fontsize=9, color="#78909c")
...
def set_view_limits(ax, task: str) -> None:
    xl, yl = episode_limits(task)
    ax.set_xlim(*yl)
    ax.set_ylim(-xl[1], -xl[0])
...
def ep_text(ax, xy, episode: int, color="white", size=7) -> None:
    u, v = view(xy)
    ax.text(u, v, str(episode), fontsize=size, ha="center", va="center", color=color, fontweight="bold", zorder=8)
...
def draw_anchor_rings(ax, anchors_cfg: dict[str, Any], layout_types: list[str], n_obj: int, offset_limit: float, label=True) -> None:
    ...
    for lt in layout_types:
        for i in range(n_obj):
            ...
            u, v = view(anchor)
            ax.plot(u, v, "k+", markersize=14, markeredgewidth=2, zorder=7)
            if label:
                ax.text(u, v + 0.012, f"{lt} 锚点 {i}", fontsize=9, ha="center", zorder=7)


# ── 单条 episode 的俯视图（图 3 每格、图 1／图 2 的叠加共用）───────────────────
def draw_binfill(ax, r, labels=True, alpha=ALPHA):
    layout = r["layout"]
    circle(ax, layout["button_xy"], BUTTON_BASE_R, color=BUTTON_COLOR, alpha=alpha, zorder=3)
    board = layout["board"]
...
def draw_routestick(ax, r, labels=True, alpha=ALPHA):
    ...
    for index, (u, v) in enumerate(points):
        if index % 2 == 0:
            ax.add_patch(Circle((u, v), 0.014, facecolor="#cfd8dc", edgecolor="#37474f", alpha=alpha, zorder=3))
            if labels:
                ax.text(u, v, str(index), fontsize=8, ha="center", va="center", zorder=6)
        else:
            ax.add_patch(Circle((u, v), 0.015, facecolor=r["layout"]["obstacle_rgb"][index // 2], edgecolor="#212121", alpha=alpha, zorder=3))
...
def draw_video(ax, r, task: str, labels=True, alpha=ALPHA):
    ...
    positions = {item["object_id"]: view(item["xy"]) for item in items}
...
def plot_positions(task, difficulty, records, positions_cfg, out: Path) -> None:
    ...
    if task == "BinFill":
        ...
        for episode, r in enumerate(records):
            ...
            circle(axes[0], layout["button_xy"], BUTTON_BASE_R, fill=False, edgecolor=BUTTON_COLOR, linewidth=1.6, zorder=6)
            ...
            # 面板 1：按钮 + 孔板，标 ep 号
            circle(axes[1], layout["button_xy"], BUTTON_BASE_R, color=BUTTON_COLOR, alpha=0.45, zorder=3)
        ...
        for episode, r in enumerate(records):
            ...
            for index, (u, v) in enumerate(points):
                if index % 2 == 0:
                    axes[0].plot(u, v, "o", color="#607d8b", markeredgecolor="#263238", markersize=9, alpha=ALPHA, zorder=3)
                else:
                    axes[0].plot(u, v, "^", color=r["layout"]["obstacle_rgb"][index // 2], markeredgecolor="#212121", markersize=10, alpha=ALPHA, zorder=3)
        ...
        for ax in axes:
            for deg in (-30, 30):
                pts = [view(p) for p in routestick_points(deg)]
                ax.plot([p[0] for p in pts], [p[1] for p in pts], "--", color="#455a64", linewidth=1.4, zorder=1)
        for index, (u, v) in enumerate(view(p) for p in routestick_points(0.0)):
            axes[0].text(u, v + 0.03, f"格点 {index}", fontsize=10, color="#37474f", ha="center", va="bottom")
        style_axes(axes[0], f"① 9 个格点 × {n} 条 = {9 * n} 个（整排绕世界原点转 -30°～30°，虚线 = 两端极限；偶数格点 = 节点，奇数 = 障碍柱）", xl, yl)
        ...
        for episode, r in enumerate(records):
            ...
            if not is_unmask:
                circle(axes[0], r["layout"]["button_xy"], BUTTON_BASE_R, fill=False, edgecolor=BUTTON_COLOR, linewidth=1.4, zorder=6)
                circle(axes[1], r["layout"]["button_xy"], BUTTON_BASE_R, color=BUTTON_COLOR, alpha=0.45, zorder=3)
    ...
    if task == "BinFill":
        ...
        for episode, r in enumerate(records):
            ...
            for ax in (axes[0], axes[1 + starts.index(start)]):
                ...
                ep_text(ax, (x0, y0 - 0.02), episode, color="#1b5e20", size=8)
        ...
        for episode, r in enumerate(records):
            ...
            for order, pair in enumerate(r["actions"]["swap_pairs"]):
                ...
                for ax in (axes[0], axes[1 + order]):
                    ...
                    iu, iv = positions[pair["initiator"]]
                    ax.text(iu, iv, str(episode), fontsize=8, ha="center", va="center", color=SWAP_COLORS[order], fontweight="bold", zorder=8)
            for ax in axes:
                if is_unmask:
...
def card_lines(task: str, r: dict[str, Any]) -> list[str]:
    ...
    # 封套规范序列化会排序字典键；文字展示按规格显式颜色顺序还原旧文本，不改输入 spec。
    obj = dict(obj)
    if task == "BinFill":
        for name in ("spawn_count", "target_count"):
            obj[name] = {color: obj[name][color] for color in obj["colors_present"]}
    elif task == "VideoUnmaskSwap":
        obj["hidden"] = {color: obj["hidden"][color] for color in obj["color_order"]}
    if task == "BinFill":
        return [
...
def plot_episode_pages(task, difficulty, records, out_dir: Path) -> list[Path]:
    ...
    for page in range(EPISODE_PAGES):
        ...
        for k in range(EPISODES_PER_PAGE):
            ...
            set_view_limits(ax, task)
            ax.set_aspect("equal")
            ax.set_xticks([])
            ax.set_yticks([])
            ax.text(0.5, 0.01, "▼ 机器人在此侧", transform=ax.transAxes, ha="center", va="bottom", fontsize=9, color="#78909c")
        ...
        fig.suptitle(f"图 3 · {task} / {difficulty} 单个 episode 的情况 第 {page + 1}/{EPISODE_PAGES} 页（ep{first}～ep{last}；"
                     "视角同回放视频：画面右 = +y、画面上 = −x；格下文字 = 该条全部随机事件与位姿；黑边方块 = 被抓／目标，箭头 = 动作）", fontsize=16)
...
# ── 入口 ────────────────────────────────────────────────────────────────────
def generate(root: Path, out_root: Path | None = None) -> dict[str, Any]:
    """从候选封套读取旧规格；图中每组前 30 条的口径与旧实现一致。"""
    from .io import load_candidates
    header, candidates = load_candidates(root / "candidates/candidates.jsonl", repo_root=REPO_ROOT)
    out_root = out_root or root / "candidates/figures"
    out_root.mkdir(parents=True, exist_ok=True)
    font = use_cjk_font()
    sampling = header["sampling_config"]
    groups = [(t, d) for t, d in GROUPS if f"{t}/{d}" in header["group_provenance"]]
    entries: list[dict[str, Any]] = []
    files: list[Path] = []
    for task, difficulty in groups:
        records = sorted([r["spec"] for r in candidates if (r["task"], r["difficulty"]) == (task, difficulty)], key=lambda item: item["episode"])[:PLOT_EPISODES]
    ...
    try:
        ...
        pass
    payload = {"run_id": header["run_id"], "phase": "before", "episodes": PLOT_EPISODES, "font": font, "dpi": DPI,
               "groups": entries, "sizes": sizes}
    (root / "candidates/logs/figures_manifest.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    if not ok:
        raise ValueError("跑前图数目或尺寸不完整")
    return payload


def main():
    from ..rollout.state import run_root
    parser = argparse.ArgumentParser(description="从候选封套生成三类跑前图")
    parser.add_argument("--run-id", required=True)
    args = parser.parse_args()
    generate(run_root(args.run_id))
    return 0
