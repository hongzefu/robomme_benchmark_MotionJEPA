...
from ..candidates.figures import SWAP_COLORS, use_cjk_font  # noqa: E402
...
def generate(data: dict[str, Any], out_root: Path) -> dict[str, Any]:
    """保持旧出图顺序、字体与像素，只改数据和输出来源。"""
    out_root.mkdir(parents=True, exist_ok=True)
    groups = [(t, d) for t, d in GROUPS if f"{t}/{d}" in data["groups"]]
    font = use_cjk_font()
    ...
    (out_root.parent / "logs/windows_manifest.json").write_text(json.dumps(
    ...
    if not ok:
        raise ValueError("数轴图片数量或尺寸不完整")
    return {"files": [str(f) for f in files], "sizes": sizes}


def main():
    from .state import RunStore, run_root
    from .windows import generate_windows
    parser = argparse.ArgumentParser(description="从实跑结果出数轴图")
    parser.add_argument("--run-id", required=True)
    args = parser.parse_args()
    store = RunStore(run_root(args.run_id))
    with store.locked():
