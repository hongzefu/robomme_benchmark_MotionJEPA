...
# 本文件位于 scripts/injection/rollout/，向上三级是仓库根。
...
class EnvCheckJob:
    ...
    #: 从候选 header 经统一校验得到的任务配置，随作业深拷贝，不再读取外部可变文件。
...
def check_one(job: EnvCheckJob) -> dict[str, Any]:
    ...
    try:
        ...
        if job.sampling_config is not None:
            kwargs["sampling_config"] = copy.deepcopy(job.sampling_config)
        if job.record:
...
def run_env_check(
    ...
    *,
    gpus: Sequence[str],
    workers_per_gpu: int,
    sampling_config: Mapping[str, dict[str, Any]] | None,
    out_dir: Path,
    repo_root: Path,
    timeout_s: float = 120.0,
    max_tasks_per_child: int = 8,
    submit: Callable[[EnvCheckJob], dict[str, Any]] | None = None,
    cached_results: Mapping[tuple[str, str, int], dict[str, Any]] | None = None,
    on_terminal: Callable[[EnvCheckJob, dict[str, Any]], None] | None = None,
) -> dict[str, Any]:
    ...
    if sampling_config is not None and not isinstance(sampling_config, Mapping):
        raise ValueError("reset 只接受从候选 header 校验得到的配置对象，不接受外部配置路径")
    ...
    def make_job(state: _GroupState, episode: int) -> EnvCheckJob:
        ...
        return EnvCheckJob(
            ...
            sampling_config=copy.deepcopy(sampling_config[plan.task]) if sampling_config is not None else None,
    ...
    try:
        while True:
            ...
            for index, (state, job) in enumerate(batch):
                key = (job.task, job.difficulty, job.episode)
                if key in (cached_results or {}):
                    harvested.append((key, state, job, cached_results[key]))
                    continue
...
def run_reset(store, *, groups=None, gpus="0", tier=1, limit=None, only_unused=False, purpose="delivery"):
    """从所属运行的候选快照启动，重复终态复用；unused 冻结范围不按配额截断。"""
    from collections import Counter
    from scripts import generate_dataset_newseed as gen
    from .state import StateError, candidate_key, result_key, write_json, file_sha
    from .run import choose_groups, new_call, terminal_cache, normalize

    header, candidates, results = store.load()
    index = {candidate_key(r): r for r in candidates}
    selected = choose_groups(header, groups)
    if limit is not None and (purpose != "smoke" or limit < 1):
        raise StateError("reset limit 只用于正数 smoke 范围")
    unused_path = store.logs / "unused_scope.json"
    original_primary = sorted(list(result_key(r)) for r in results if r["role"] == "primary")
    if only_unused:
        if store.root.name != "20260912-contract-v3-10" or groups or limit:
            raise StateError("only-unused 仅对原运行十的全部冻结 unused 使用")
        if unused_path.exists():
            frozen = json.loads(unused_path.read_text())
            if frozen["identity_sha256"] != header["identity_sha256"]:
                raise StateError("unused 冻结范围身份不一致")
        else:
            keys = [list(candidate_key(r)) for r in candidates if r["role"] == "unused"]
            if len(keys) != 838:
                raise StateError(f"首次 unused 范围应为 838 条，实际 {len(keys)}")
            frozen = {"keys": keys, "identity_sha256": header["identity_sha256"], "primary": original_primary}
            write_json(unused_path, frozen)
        pool_keys = {tuple(k) for k in frozen["keys"]}
    else:
        finished = set(store.completed_groups())
        if purpose != "smoke":
            selected = [g for g in selected if "/".join(g) not in finished]
        pool_keys = {candidate_key(r) for r in candidates if r["split"] == "test" and (r["task"], r["difficulty"]) in selected}
        if not selected:
            print("RESET_IDEMPOTENT=PASS rerun=0 duplicates=0", flush=True)
            return {"executed": 0, "reused": sum(r["kind"] == "reset" for r in results), "passed": True}
    config_groups = {(g["task"], g["difficulty"]): g for g in header["delivery_config_snapshot"]["groups"]}
    documents, plans = {}, []
    for task, difficulty in selected:
        keys = sorted(k for k in pool_keys if k[:2] == (task, difficulty))
        if not keys:
            continue
        document = {"episodes": [copy.deepcopy(index[k]["spec"]) for k in keys]}
        documents[(task, difficulty)] = document
        need = len(keys) if only_unused else min(header["delivery_config_snapshot"]["extra_candidates"], limit or 10**9)
        plans.append(EnvCheckPlan(task, difficulty, start=config_groups[(task, difficulty)]["run_episodes"], need=need, limit=limit))
    if not plans:
        raise StateError("reset 范围为空")
    cache = terminal_cache(store, "reset")
    # 正式迁入的旧终态也属于可复用证据，不能因没有新调用日志而重跑。
    for row in results:
        if row["kind"] == "reset" and candidate_key(row) not in cache:
            cache[candidate_key(row)] = copy.deepcopy(row)
    call = new_call(store, "reset", sorted(pool_keys), {"gpus": gpus, "tier": tier, "limit": limit,
                                                       "only_unused": only_unused, "purpose": purpose})
    fresh = []
    with (call / "episode_results.jsonl").open("a", encoding="utf-8") as sink:
        def terminal(job, result):
            key = (job.task, job.difficulty, job.episode)
            if key in cache:
                raise StateError("已经有终态的 reset 被重复派发")
            sink.write(json.dumps(result, ensure_ascii=False, allow_nan=False) + "\n")
            sink.flush()
            os.fsync(sink.fileno())
            cache[key] = result
            fresh.append(key)
            print(f"reset 完成 {job.task}/{job.difficulty}/ep{job.episode} ok={result['ok']}", flush=True)
        payload = run_env_check(documents, plans, gpus=gpus.split(","), workers_per_gpu=tier,
            sampling_config=gen.validate_sampling_config(header["sampling_config"], REPO_ROOT),
            out_dir=call / "batches", repo_root=REPO_ROOT, timeout_s=120,
            cached_results=cache, on_terminal=terminal)
    observed = []
    for path in sorted((call / "batches").glob("*/*.jsonl")):
        for row in path.read_text().splitlines():
            observed.append(json.loads(row))
    # 调度器的实际处理范围含尾批在飞余量，不能按 need 截断。
    observed_keys = [candidate_key(r) for r in observed]
    if len(observed_keys) != len(set(observed_keys)):
        raise StateError("reset 批次出现重复结果")
    normalized = [normalize(store, cache[key], "reset", index) for key in observed_keys]
    store.merge(normalized)
    completed = set(store.completed_groups())
    if not only_unused and purpose != "smoke":
        completed.update(key for key, group in payload["groups"].items() if group["shortfall"] == 0)
    payload["completed_groups"] = sorted(completed)
    payload["executed"] = len(fresh)
    payload["reused"] = len(observed_keys) - len(fresh)
    write_json(call / "summary.json", payload)
    store.publish(store.load()[2], completed_groups=sorted(completed))
    audit = store.audit()
    if only_unused:
        if set(observed_keys) != pool_keys:
            raise StateError("unused 没有逐一遍历冻结集合")
        current = store.load()[2]
        primary = sorted(list(result_key(r)) for r in current if r["role"] == "primary")
        if primary != frozen["primary"] or audit["unused"] != 0:
            raise StateError("unused 补查改变原 primary 或仍有遗漏")
        passed = sum(cache[k]["ok"] for k in pool_keys)
        payload["passed"] = True
        payload["unused_check"] = {"checked": len(pool_keys), "passed": passed, "failed": len(pool_keys) - passed,
                                   "unused_left": audit["unused"], "primary_changed": 0}
        print(f"UNUSED_RESET=PASS checked={len(pool_keys)} passed={passed} failed={len(pool_keys)-passed} unused_left=0 primary_changed=0")
    elif not payload["passed"]:
        write_json(store.logs / "reset_check_result.json", payload)
        raise StateError("reset 配额不足，保留全部终态")
    write_json(store.logs / ("unused_result.json" if only_unused else "reset_check_result.json"), payload)
    if not fresh:
        print("RESET_IDEMPOTENT=PASS rerun=0 duplicates=0", flush=True)
    return payload
...
def main():
    ...
    from .run import recover
    parser = argparse.ArgumentParser(description="候选 reset 核验与运行十 unused 全量补查")
    ...
    parser.add_argument("--only-unused", action="store_true")
    args = parser.parse_args()
    label = args.label or ("smoke" if args.purpose == "smoke" else None)
    store = RunStore(run_root(args.run_id), label)
    with store.locked():
        recover(store)
        run_reset(store, groups=args.groups, gpus=args.gpus, tier=args.tier, limit=args.limit,
                  only_unused=args.only_unused, purpose=args.purpose)
