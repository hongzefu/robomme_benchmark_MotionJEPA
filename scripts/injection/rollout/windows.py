...
DEFAULT_ROLLOUT_RUN_ID = "20260912-contract-v3-10"
TIMELINE_JSON = REPO_ROOT / "artifacts/injection" / DEFAULT_ROLLOUT_RUN_ID / "rollout/logs/windows_timeline.json"
DOC = TIMELINE_JSON.parent.parent / "WINDOWS.md"
...
def load_specs(run_id: str, task: str, difficulty: str) -> dict[int, dict[str, Any]]:
    """从所属运行唯一候选封套投影规格。"""
    from ..candidates.io import load_candidates
    _, rows = load_candidates(REPO_ROOT / "artifacts/injection" / run_id / "candidates/candidates.jsonl", repo_root=REPO_ROOT)
    return {r["episode"]: swap_spec(r["spec"]) for r in rows if (r["task"], r["difficulty"]) == (task, difficulty)}


def swap_spec(record):
    """恢复旧 load_specs 的交换字段投影，连续区间推导仍用原算法。"""
    pairs = [{"initiator": p["initiator"], "partner": p["partner"]} for p in record["actions"]["swap_pairs"]]
    count = int(record["objects"]["n_swaps"])
    if count != len(pairs):
        raise ValueError("n_swaps 与 swap_pairs 数量不一致")
    return {"n_swaps": count, "pairs": pairs}
...
def extract_results(store) -> dict[str, Any]:
    """从唯一结果表的当前有效路径读取 HDF5，保留旧数据层算法与剔除规则。"""
    from .state import candidate_key
    header, candidates, rows = store.load()
    run_ids = [header["run_id"]]
    sources = [store.state]
    latest = {}
    for original in rows:
        ...
        if key in latest:
            raise ValueError("数轴输入存在重复键")
        latest[key] = row
    projected = {candidate_key(r): r["spec"] for r in candidates}
    active_groups = {key[:2] for key in latest}
    groups: dict[str, list[dict[str, Any]]] = {f"{task}/{difficulty}": [] for task, difficulty in GROUPS if (task, difficulty) in active_groups}
    ...
    for (task, difficulty, episode), row in sorted(latest.items()):
        ...
        if task in SWAP_TASKS:
            annotate_swaps(task, entry, swap_spec(projected[(task, difficulty, episode)]), h5_path)
...
def generate_windows(store):
    ...
    from . import window_figures
    data = extract_results(store)
    if data["skipped"]:
        raise StateError(f"数轴读取跳过 {len(data['skipped'])} 条，拒绝交付")
    if data["episodes"] + len(data["excluded_slow"]) != data["episodes_before_exclusion"]:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="从唯一结果表生成采样窗口数据、图和报告")
