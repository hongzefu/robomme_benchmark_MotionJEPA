...
| 3 | **难度序 easy < medium < hard < xhard1 < xhard2 < xhard3 < xhard4**；现在的 xhard 改名 **xhard4** 并作为最难档，**档名 `xhard` 废弃、代码与配置里不再出现**（`VALID_DIFFICULTIES` 去掉 `xhard`，`config_xhard`/decision 子树 `xhard` 键/`XHARD_*` 常量/`spec_kind` 一律改为 `xhard4` 命名，v5-01 的 xhard 规格随 M3 作废）；**四档的取值区间互不重叠**，并尽量与 hard 不重叠；xhard4 的数值可以为此重排（xhard 不对拍，M3） | 用户 2026-09-26「改为xhard123 xhard现在xhard改为xhard4！原来的纯xhard废弃 不要再使用 容易混淆 / 每个难度要有区分 不能有重叠交集合」 |
...
| 7 | **均匀化算法 xhard 与新档共用**；**16 个环境的 xhard 全部在 V6 重新抽签、不做任何 xhard 对拍**（用户 2026-09-26：xhard 是为了迭代的，只对拍原三档 easy/medium/hard） | 用户原话；M3 已定 |
...
| 9 | **只做一次对拍、一次生成**：V1 = 16 × 9 原三档与 `13e5151` 逐位比（唯一闸门）；生成 = 一次多 worker 运行，13×3 新档 + 16 个 xhard = **55 格**，每格「10 候选 + index 0/3/6 三局正式」= 550 候选、165 正式局（M4 已定 (a)） | V5 口径 12；M3/M4 已定 |
...
| 编号 | 问题 | 选项 | 建议 | 结论（用户答复） |
|---|---|---|---|---|
| M1（**已定：2.3～2.12 各表数值照用；机制型字段沿用 xhard**，用户 2026-09-26） | 新档是否**沿用 xhard 的全部机制只内插数值**（口径 5）。这意味着 BinFill xhard1 就是杂乱布局、PH xhard1 就是 HSV 任意色、VUS xhard1 就带外环同步交换、PickXtimes xhard1 就是均匀无偏置 + 0.08 间距 | (a) 是。<br>(b) 否：某些机制（如外环交换、杂乱布局）也按档分级引入，需逐环境再定 | (a) | |
| M2（**已定：不加**，用户 2026-09-26「m2不动」） | MoveCube 要不要也加三档（原版无梯度） | (a) 不加，只推 xhard。<br>(b) 加：xhard1/2/3 = 在现状与 U 之间按桌心半径 / 离基座环带内插三档，xhard = U | (a) | |
| M3（**已定：不对拍 xhard**，用户 2026-09-26「不要做任何对拍xhard xhard是为了迭代的 只需要对拍之前的easy medium hard」） | 未被改动的 10 个环境（加 InsertPeg、StopCube 共 12 个）的 xhard 是否必须逐位复现 v5-01 | **取消 X0 回注闸门**：16 个环境的 xhard 在 V6 全部重新抽签（seed 偏移 6e6），不再复制 v5-01 的行，`v4_specs` 一份快照一条 `seed_rule` 的问题随之消失；唯一硬闸门仍是 V1（原三档 16×9 与 `13e5151` 逐位比） | — | 已定 |
| M4（**已定：(a)**，用户 2026-09-26「m4 a」） | 每格规模 | (a) 沿用「10 候选 + 3 正式」，抽签尝试上限按环境 30～60。<br>(b) 新档减到「6 候选 + 2 正式」 | (a) | |
| M5（**已定：(b) 不动，目视无区别**，用户 2026-09-26） | Unmask 内环 xhard 里「bin_3 恒为空容器」（藏 cube 只在 bin_0..2）要不要一并随机化 | (a) 随机化（`randperm(4)[:pick]`，xhard 流原地移位）。<br>(b) 不动 | (a) | |
| M6（**已定：S5 + 整条重排 + G 连通**，用户 2026-09-26） | **均匀化算法选型**（口径 6 的实现） | (a) S5：可行候选中参与次数最少者优先、平局均匀抽、禁止立即撤销、整条极差 >1 重排 ≤20 次；VUS/BUS 另以 G 连通作 reset 接受条件。<br>(b) S1n：可行候选里均匀抽 + 禁止立即撤销（用户提议的形式；跨局均匀，局内极差 2.63，VR 全员参与只 50%）。<br>(c) S1：可行候选里均匀抽，不加任何约束 | (a) | |
| M7（**已定：(a)**，用户 2026-09-26） | 外环在 V5 路径约束下局内做不到均匀 | (a) 接受「跨局均匀 + O4 尽量均衡」，报告逐局未参与数。<br>(b) 放宽「路径全程可见」（零可行搭档槽 31.7% → 15.0%），交换会出画。<br>(c) 外环改沿环切向成对放置（改布局）。<br>(d) 外环不做均匀化，只做内环 | (a) | |
| M8 | MoveCube 统一区域 | **已定**：圆环，圆心 (−0.06, 0)，内孔 0.12、外径 0.20，推距上限 0.30 保留（2.6） | — | 用户 2026-09-25「movecube这个同意 就这么做 固化到plan内」 |
| M10（**已定：(a)**，用户 2026-09-26） | PickHighlight「按干扰数量」在 xhard 不重冻时无法加码（干扰均值 hard=xhard=3） | (a) 只按 pick 与总块加码，干扰均值全程 3（2.9 现表）。<br>(b) 重冻 PH xhard：新增 decision 键直接抽干扰数（如 3/4/5/6），pick 不变 | (a) | |
...
| M15 | BinFill xhard3/4、PickXtimes xhard3/4、PickHighlight xhard4 按 1.7 表会超评估 1301 步（原 xhard 已有局超） | (a) 接受（评估侧本来就截断，与 xhard 现状同）。<br>(b) 压回 1301：BinFill 6/6/7/7？无法四档不重叠 → 改 5/6/7/8 并接受与 hard 重叠 1 档；PickX [6,7]/[8,9]/[10,11]/[12,12]；PH pick 4/5/6/6 总块 7/8/9/10 | (a) | |
...
### 1.7 四档定稿表（2026-09-26，用户口径：xhard1<xhard2<xhard3<xhard4，区间互不重叠；本表取代 2.3～2.12 各表的数值，机制仍按各节）

| 环境 | 梯度维度 | hard（冻结） | xhard1 | xhard2 | xhard3 | xhard4（原 xhard，数值重排） | 备注 |
|---|---|---|---|---|---|---|---|
| BinFill | 投入块数（总块 12） | [3,5] | 6 | 7 | 8 | 9 | 原 xhard [5,7] 已有 1/3 局超评估 1301 步（无演示段，全部计步）；8/9 块估约 1500～1900 步，见「步数说明」 |
| PickXtimes | 次数 / 干扰块 | [4,5] / 0 | [6,7] / 1 | [8,9] / 2 | [10,12] / 3 | [13,15] / 3 | 原 xhard [6,15]；xhard3/4 超 1301（原 xhard 2/3 局已超）；干扰颜色表若 ≥4 色则 xhard4 取 4 |
| SwingXtimes | 轮数 / 干扰块 | 3 / 0 | [4,5] / 1 | [6,7] / 2 | [8,9] / 3 | [10,11] / 3 | 原 xhard [4,10]；11 轮估约 1200 步 |
| PickHighlight | pick / 总块 | 3 / 6 | 4 / 7 | 5 / 8 | 6 / 9 | 7 / 10 | 原 xhard [5,7]/[8,10]；干扰均值全程 3（M10）；总块 10 的 reset 成功率 73%；pick 7 估约 1400 步 |
| VideoUnmask | 干扰容器 / pick | 0（15 容器）/ 2 | 8 / 2 | 10 / 3 | 13 / 3 | 15 / 3 | pick 上限 3（M5 (b)：可藏 cube 的容器只有 3 个），靠干扰数区分 |
| ButtonUnmask | 干扰容器 / pick | 0 / 2 | 8 / 2 | 10 / 3 | 12 / 3 | 14 / 3 | 同上 |
| VideoUnmaskSwap | swap / pick / 外环干扰 | [2,3] / 2 / 0 | [4,5] / 2 / 4 | [6,7] / 3 / 6 | [8,9] / 3 / 8 | [10,12] / 3 / 10 | 原 xhard swap [8,12] |
| ButtonUnmaskSwap | swap / pick / 外环干扰 | [2,3] / 2 / 0 | 4 / 2 / 4 | 5 / 3 / 6 | [6,7] / 3 / 8 | [8,9] / 3 / 10 | 原 xhard [6,8] |
| VideoRepick | 块数 / swap / repick | 15 静态 / 0 / [1,3]（另一种任务，不比较，M9） | 4 / [3,4] / 2 | 5 / [5,6] / 3 | 6 / [7,8] / 4 | 7 / [9,12] / [5,6] | 原 xhard 6 / [8,12] / [4,6]；7 块的 reset 成功率需实测（6 块 0.59） |
| PatternLock | 节点数（5×5，不重访） | [4,8] | [9,12] | [13,16] | [17,20] | [21,25] | 原 xhard 25；非演示步 ≤ 约 850 |
| RouteStick | 段数 L | [4,7] | [8,10] | [11,13] | [14,16] | [17,21] | 原 xhard [15,21]；非演示步 = 50·L ≤ 1050 |
| VideoPlaceButton | 放台次数（五档都放回原位） | 2（放桌面） | 1 块 3 次（多放 1 次无关台） | 1 块 4 次（前后各多 1 次） | 2 块 5 次（一块 3 次一块 2 次） | 2 块 6 次（各 3 次） | 原 xhard 2 块 4 次；额外放台段要在 xhard4 模板里实现（2.10） |
| VideoPlaceOrder | 总放台次数（五档都放回原位） | 1 块 v∈[2,4]（2～4） | 2 块 (2,3)=5 | 2 块 (3,3)=6 | 2 块 (3,4)=7 | 2 块 (4,4)=8 | 原 xhard 2 块各 v∈[2,4]（4～8）；每块访问数按档定值抽（哪块多访问随机） |
| MoveCube | 不加档（M2） | 原三档同值 | — | — | — | 圆环 U（2.6） | 只有 xhard4 |
| InsertPeg / StopCube | 不加档 | 原三档同值 | — | — | — | 原 xhard 改名 xhard4 | 数值不动 |

**步数说明**：评估侧 1301 步只数执行段（无演示段的环境全部计步）。BinFill xhard3/4、PickXtimes xhard3/4、PickHighlight xhard4 按上表会超过 1301（原 xhard 本来就有局超），其余环境全部在 1301 内。要不要为这三个环境把上限压回 1301（例如 BinFill 上限 7、PickX 上限 12、PH pick 上限 6，但那样四档就要压缩成更窄的不重叠区间），待用户定（M15）。

### 1.6 副本实现与实测状态（2026-09-25 晚，用户「可以先开始做代码的改动但是不要修改原文件以副本的形式保留下来」）

四个 git worktree 副本（主仓库源码一字未动），各自分支**不 push**（用户 2026-09-26「四个副本分支不要 push」）：
...
| `/data/hongzefu/v6-draft/vp` | `v6-draft-vp` / `9b51421` | 2.10 三种放回策略（`return_to_origin` / `return_last_only` / `native_random_goal_site`=不放回）与 (k2,不放回) 落点（M11：goal_site 中心 ±0.12 m 的 1 cm 网格上由近及远取第一个满足「离方块初始位 ≥0.07、离目标台 ≥0.08、离按钮 ≥0.10、离已选落点 ≥0.07」的点，不抽随机数，找不到抛可重试 SceneGenerationError）；VPO 新键 `xhard.visit_count_range`（默认 [2,4]，与 V5 同抽，缺键自动补）；`vqa_options` drop 候选加 goal 落点；`scripts/parity/v6_tier_monotone.py` + `test_v6_tier_monotone.py` | 本机真演示：VPB (k1,放回) 3/3、(k2,不放回) 3/3、(k2,只放回末块) 4/4、xhard 对照 4/4；VPO (k1,v[2,4],放回) 3/3、(k2,v[2,3],不放回) 3/3、(k2,v[2,4],不放回) 3/3、(k2,只放回末块) 4/6、xhard 3/4（失败皆 reset 阶段 Target 4 放不下，xhard 默认同样失败）；VPB/VPO xhard 同 seed h5 SHA 与基线 4/4 相同；PatternLock 三档 619/893/1249（xhard 1641）、RouteStick 933/1167/1367（xhard 1833）步，逐档递增且低于 xhard，PatternLock DFS 预算未耗尽；规格回放 7 组合 × 3 seed mismatch 0；全量 pytest 47F/12E 对基线 46F/12E，多出的 1 条是 `test_snapshot_matches_source`（`visit_count_range` 对 V5 快照，预期） | ① 首版落点不避障导致 2 局失败（方块初始位离 goal 中心 3.9 cm），已改避障网格；② VPB xhard2/xhard3/xhard 的 pick-place 段数与步数几乎相同（6 段、约 1550 步），难度差只在终点是桌面还是原位（M11 问题）；③ `validate_demo_plan` 的 `!= "xhard"` 需在合入时改族判断（pipeline 副本已做） |
...
GL 实测（只 `srun --overlap` 连现有占位 job 61890467/61890468，不开新 job）：MoveCube 圆环 U 144 局 120/144（2.6）；Swap S5 真机回放 292 局（2.2）；**13 环境 × 3 新档 × 6 局 + xhard 对照 3 局 = 273 局**（`plan-probes/tiers/`，用 `sampling_config` 覆盖 xhard 数值模拟新档，守卫只比键结构、无需 patch；driver 的 `max_tasks_per_child=3` 在 spawn 上下文换代点挂死两次，去掉后 12 worker 续跑完成）。新档 273 局结果（`tiers/report.md`、`tables.md`、`tiers_summary.png`）：

| 项 | 结果 |
|---|---|
| 演示级成功率（成功 / reset 通过局） | 39 格中 37 格 100%；例外 BinFill xhard1 5/6、xhard3 4/6（DatasetGenerationError，xhard 对照 2/3 同类） |
| reset 失败（均为既有拒绝率） | VR 新档 2/3/1 局（≈60%，与审计 0.625 一致）；VPB xhard1 2、xhard2 1（对照 2/3）；VPO 3/5/5（对照 2/3，Target 4 放不下）；PH、VUS 对照各 1 |
| 步数 | 新档总步数全部低于各自 xhard 对照（PL 824/1166/1326 对 1720，RS 1316/1473/1718 对 2220，VR 885/1200/1419 对 1625，PickX 994/1204/1577 对 1955）；超评估 1301 非演示步的只有 PickX xhard2 1/6、xhard3 5/6（xhard 对照 2/3，V5 现状）；无 fail_safe 5000 |
| 旋钮单调 | 11 个有独立旋钮的环境逐档严格上升（BinFill 4.2/4.5/5.2→5.3、PickX 6.3/7.5/9.5→10、SwingX 3.3/4.7/6.3→6.3、PH pick 4/4.8/5.5→6、VU 8/10/13→15、BU 7/9/12→14、VUS 4.3/6.2/7.5→10、BUS 3.5/4.5/5.5→7.3、VR 3.8/5.7/7.2→10、PL 10/15/18.7→25、RS 9.5/11.5/13.7→18）；VP 两环境用 (k2,全放回) 代测上界，真正三档以 vp 副本实现为准（M11） |
| 代测/自定 | VUS 次数挂 `native.parameters.configs`；VUS/BUS 外环 cube 数 = 干扰一半；VPO xhard2 的 v 上界 3 靠进程内补丁 |
