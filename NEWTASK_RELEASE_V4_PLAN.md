...
| 10 | **正式验收单 worker、判据只认 A40**（`mplib` 的 RRT 用墙钟预算；本机 sm_89 与 A40 sm_86 产物不同）。**例外：V1 原值回归只在本机跑**——对照改为本机改动前基线（`13e5151`）与改动后逐位比，不再对 A40 的 V3 留档；**步 7 的 48 条全量也在本机跑**（K4） | V3 实测；用户 2026-09-22「原值回归只需要在本机器跑」；第五节 |
...
| G2 | **已定（2026-09-22，实测见 `docs/validation/newtask-v4/20260922-step0-g2-capacity.md`）**：VideoUnmask / ButtonUnmask **区域不动、`min_gap_factor` 0.75、容器数 N=8**（两环境全部放下率 ≥99%，相邻容器最小边距约 15 mm）；VideoRepick clutter **方块数 N=6**（8~12 次交换至少扫到一次旁观块约 41.5%，D5 修好后≈抽签拒绝率）。⚠ 实测推翻了 2.7② 的基线：hard `bin=15` 真实只放下 4~8 个（均值约 5.5），因 `spawn_random_bin` 把 `min_gap` 加了两次 | 2.7 / 2.8 / 2.13 |
| G3 | **MoveCube `corner_bias` 已定 0.5**（2026-09-22，本机每档 8 条演示：0→8/8、0.25→7/8、0.5→6/8、0.75→6/8、1.0→5/8，每档每种 way 均有成功）；V6 每组合样本量见 J8（5 条） | 2.17 / 第二部分五 |
...
| J5 | PickXtimes 边角偏置 **corner_bias=0.5，推全部 3 个有色方块**（不只推目标块） | 2.4 |
...
| K1 | PatternLock xhard 节点数**改为 `[20,24]`**（原 `[20,25]`：收窄到 25 时 1000 次预算内只 4/10 真得到 25 节点、其余静默用错长路径，违反口径 14；20～24 各 10/10）（用户 2026-09-23「改为24」） | 2.19 |
...
新值一律落新增的 `xhard` 档，原三档不动（口径 7）。三件事：

1. **十三个环境新建 `config_xhard`**；`VideoUnmaskSwap` / `VideoRepick` / `RouteStick` 已有该档，
   **直接覆盖成 V4 新值**，不保留旧值、不另起档名、不扩 `VALID_DIFFICULTIES` 白名单。
   每个环境从哪一档派生，见 2.1。
2. **`StopCube` / `MoveCube` / `InsertPeg` 没有难度分档**——类里无 `configs` / `config_*`，
   `self.difficulty` 算出来后全文件无消费点，参数都是全局常量。这三个取
   `configs = {easy: X, medium: X, hard: X, xhard: 新值}`，`X` = 现有全局常量原值。
   **三档同值**保证"不管传哪档行为都与现状一致"——现状本就如此，所以这个写法不改变任何既有行为。
3. **抽签段入口显式传 `difficulty="xhard"`**：`--difficulty` 那个三位循环配额只管 easy/medium/hard，
   xhard 只经每条规格的 `difficulty` 字段进入。

由此，`NATIVE_REGRESSION`（V1）的判据是**原三档产物零差异**——不是"分析漂移是否可接受"，
任何非零差异都是明确的 bug。
...
| **VideoUnmaskSwap** | hard `{bin 4, swap [2,3], pick [2,2]}` | **已有 xhard** `{bin 4, swap [4,5], pick [2,2]}` | **覆盖成新值**（A7） |
...
| **VideoRepick** | **medium** `{cube 3, swap [2,3]}` | **已有 xhard** `{cube 3, swap [4,5]}`，恰好就是 medium 派生 | **覆盖成新值**（A7） |
...
| **RouteStick** | hard `{length [4,7], backtrack True}` | **已有 xhard** `{length [8,10], backtrack True}` | **覆盖成新值**（A7） |
...
两类：`VideoRepick.decision.num_repeats_range` 有键但**没有任何消费点**（`__init__` 实际读的是
...
**要做**：速度最快档、number `[6,15]`。**本环境没有难度分档**（无 `configs`，`difficulty` 无消费点），
...
- **速度与判定阈值必须一起看**（C4）：停止窗口宽度恒等于 `move_interval`，
...
**① 干扰容器怎么放**：做成**额外容器**，放在现 region 之外、相机可见处，
...
**③ swap 速度 ×1.5 的取整**（B4）：`SWAP_WINDOW_STEPS = round(50/1.5) = 33`，
...
**步数预算上两个 Swap 环境不对称**：`ButtonUnmaskSwap` 所有任务 `demonstration=False`
⇒ 观看交换那段**计入** 1302 配额，8 swap + 3 pick 在 50 步窗口下最大约 960、余量仅约 25%；
33 步窗口把这段从 464 压到 **328**。`VideoUnmaskSwap` 第一个任务是 `static` 且 `demonstration=True`
⇒ 那段**不计入**配额，余量极大。
...
| `native.cubes.min_gap_factor` | 间距系数 | `2` ⇒ `min_gap=0.04` | **不动**（B5） |
...
**要做**：clutter、pick times `[4,6]`、block 颜色任意、swap `[8,12]`（A3）。

> **为什么从 medium 派生**：hard 是 5 轮 × 红蓝绿 = 15 块的特殊结构、`swap=0`、连 `cube` 键都没有；
> medium 是"3 块 + swap `[2,3]`"的常规结构，`swap [8,12]` 接得上。
...
- **注入分支依赖颜色名字符串**：`idx = [item["name"] for item in options].index(spec["objects"]["color"])`
  ⇒ 颜色连续化会打断这条路径。
- easy/medium/hard 不动（口径 7），hard 那套 `cube_{color}_{idx}` 命名与 `hard_round_order`
  规格路径**不受影响**。
...
| `native.color.xhard` | 场上方块数（= color 值，`cubes_per_color=1`） | `3` | 不变 ⇒ **场上 3 块，够演示 2 块** |
...
- **"更近"与现有判据的张力**：`build_peg` 的几何是 head 占 `[-0.025,+0.025]`、tail 中心在 `x=-0.05`
  ...
  ⇒ **现行参数下两根杆本来就可能互相穿插**，靠物理弹开。B6 定的是**不放松间距**
...
| `decision.length.xhard` | **节点数**（不是段数） | `[4,8]` | **`[20,24]`**（K1：原定 `[20,25]`，25 实测基本生成不出）；注入 `actions.path_nodes` |
...
```text
   ...
   ⚠ 整根杆跨度 0.1 m > 最小间距 0.075 m ⇒ **现在就可能穿插**，B6 定的是不放松、也不修复
...
环境侧的接口只有三个 kwargs，三步都靠它们：`sampling_config`（往哪个范围抽）、
`native_episode_spec`（这一局的规格；不传就是抽签）、`difficulty`。
环境内部每个抽随机数的地方都长这样（记录器 `utils/episode_spec.py::SpecRecorder`，一个环境实例一份）：

```python
self.dynamic = self._spec.value("layout.dynamic", torch.randint(...))
```

- **没传规格**：`value()` 把抽到的数按路径 `layout.dynamic` 记进规格，原样返回。
- **传了规格**：`torch.randint(...)` **照样执行**，但 `value()` **返回规格里那个数**；当场抽到的数只记下来做对账。

为什么还要照样抽：后面的随机数取决于前面抽了几次，少抽一次就全平移了（红线 R8）。
为什么不能"抽到相同就用抽的"：那样规格没被真用，只是碰巧一样，对拍就失去意义。

#### ① 抽签怎么调（新入口 `scripts/parity/<抽签入口>`，每条候选一次）

```python
env = gym.make(task,
               sampling_config=xhard_ranges[task],   # 来自 scripts/configs/newtask-v4/sampling_config.json
               native_episode_spec=None,             # 不传 ⇒ 记录器进"记"模式
               difficulty="xhard",
               seed=seed)                            # seed_layout 公式算出
env.reset()                                          # 只 reset；失败就捕获、归类
spec = env.unwrapped._spec.to_dict()                 # 这局抽到的全部数 ⇒ 一条规格
env.close()
drafts.append({"identity": ..., "reset_ok": ..., "fail_class": ..., "spec": spec})
```
...
```python
write_specs(drafts="artifacts/newtask-v4/<run-id>/draft/drafts.jsonl",
            sampling_config="scripts/configs/newtask-v4/sampling_config.json",
...
```python
sampling_by_task, specs_by_identity = load_specs("scripts/configs/newtask-v4/<run-id>/specs.jsonl")
#   sampling_by_task[task]              ← header 里内嵌的配置（不再读磁盘 config）
#   specs_by_identity["task/episode"]   ← 只含 selected=true 的 48 行
```

这两个 dict 就是 `train_split_runner.py::_submit` 本来就在用的那两个，往下一行都不改：

```python
# train_split_worker.py::run_one，V3 现成代码
env = gym.make(task,
               sampling_config=sampling_by_task[task],
               native_episode_spec=specs_by_identity[f"{task}/{episode}"],   # 传了 ⇒ 记录器进"用"模式
               difficulty="xhard", seed=seed, ...)
record_env = RobommeRecordWrapper(env, ...)   # 录 h5 + 视频
record_env.reset()                            # 每个取值点用规格里的数
planner 跑演示 → h5 落盘
spec_replay.json ← env.unwrapped._spec 的对账结果（规格里有没有缺、有没有没用到、哪些不等）
...
| `SpecRecorder.__init__` 校验 `spec_kind` | 只认 `native-parity/1` | 增开 `native-newvalue/1`（3.3③） |
...
- **N2 录像器冻结。** `RecordWrapper.py` 不改不覆盖；验证 `git diff --quiet HEAD -- src/robomme/env_record_wrapper/RecordWrapper.py`。**唯一例外（I1，用户 2026-09-22 授权）**：`fail_safe_limit` 2000→5000，其余一字不动。
