...
| 阶段 | 状态 | 已有证据 | 下一步 |
| --- | --- | --- | --- |
| RouteStick 白球尾迹再减半 20→10 专项重出 `20260912-contract-v3-09`（四档各 5 条，`10.71`） | 完成 | `RouteStick.step` 存活步数 20→10（规则 11 获批，用户原话「再次降低从+20到+10」）、快照重导出一致；09 `PLAN=OK elapsed_s=335.3`、`CHECK=PASS elapsed_s=660.3`、与 07 `OLD_GROUPS_EQUIVALENCE=PASS differences=0`；smoke 1 条后正式 20 条墙钟 97.4 秒 `RUN=PASS` `DELIVERY=PASS`，timestep 与 07/08 逐条一致；四档 ep0 游程差中位对 07 = 30.0、对 08 = 10.0（`TRAIL_HALVED=PASS` ×8，[TRAIL_CHECK.md](docs/validation/newtask-v2/20260912-contract-v3-09/TRAIL_CHECK.md)）；09 小产物 34 个入库 | RouteStick 代码现状为 10 步尾迹；07 全量 30 条与 08 的 20 步数据均为旧尾迹，如需统一按 09 runbook 重出 |
