...
`plots` 行统计4张图，速率单位为图/秒；其余行统计episode。
...
采样窗口按 UTC 阶段时间与 GPU 日志的主机时区对齐。利用率为样本均值；原始间隔统计保存在 FINAL.json，不把名义500ms当作硬件计数器分辨率。
...
原始证据：[run_context.json](run_context.json)、[phase_timings.tsv](phase_timings.tsv)、[main.log](main.log)、[gpu_samples.csv](gpu_samples.csv)。FINAL.json保留原始纳秒时间、ICL_TIMING原文和所有控制文件摘要。
