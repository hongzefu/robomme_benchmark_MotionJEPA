"""v4.2 网格 mask（Wan VAE latent 对齐）的逻辑测试（纯合成数据，毫秒级，不碰 h5）。
...
2. **参数守卫**：min_pixels/cell_size 的合法区间、拒绝 float、from_fraction 的
   有理数精确换算（不依赖浮点比较）。
3. **两条结构性质**：min_pixels=1 时网格是像素 mask 的严格超集（打破 v4.2
   「只减不增」单调性——这是刻意口径，见 grid_mask.py docstring）；对阈值单调收缩。
4. **65 行累计表的后缀和 == 直算**：grid_sweep 的全部逐阈值指标都从逐计数档位
...
def _load_v42_modules():
    """隔离加载 v4.2 的 grid_mask / arm_mask_v4。

    grid_mask 目前只在 v4.2 目录存在且只 import numpy，冲突风险为零，但仍按
    test_arm_mask_v4_2.py 的 sys.modules 隔离体例写——将来若有人给 grid_mask.py
    加一行 `from color_model import ...`，没有隔离就会按导入顺序静默拿到
    v4 / v4.1 目录的错误实现。
    """
    names = ("grid_mask", "arm_mask_v4", "color_model")
    ...
    return grid_mask, arm_mask


grid_mask_mod, arm_mask_v4 = _load_v42_modules()
...
def test_累计表后缀和等于直算():
    ...
    grid_sweep.py 的全部逐阈值指标都建立在这条推导上，这里用小规模合成数据锁死。
    """
    rng = _rng()
    frames = [
    ...
    cell_area = C * C
    # 累计表：与 grid_sweep 同法——bincount 按格计数档位累计
