"""两分布 + 臂颜色模型与形态学标定的逻辑测试（合成数据，秒级，不碰任何 h5 产物）。

覆盖五件事：

1. **GT 映射**：`robot_link` 归机械臂、`background_prop` 加 seg_id 0
   归背景、`segmentation_objects` 归物体、`kind == "tcp"` 归机械臂；未覆盖 seg id 归物体并被
   单独计数。
...
5. **四条形态学规则**：开运算 / 触顶连通域 / 时间多数表决 / 保守收缩的行为契约。
"""
...
arm_masks_for_episode = arm_mask_mod.arm_masks_for_episode
arm_masks_for_segment = arm_mask_mod.arm_masks_for_segment
_keep_top_entering = arm_mask_mod._keep_top_entering
_temporal_majority = arm_mask_mod._temporal_majority


def _setup_group(handle: h5py.File, excluded, objects) -> h5py.Group:
    """按真实 h5 的结构造一个 setup group。"""
    setup = handle.create_group("setup")
    group = setup.create_group("segmentation_excluded")
    ...
    group = setup.create_group("segmentation_objects")
