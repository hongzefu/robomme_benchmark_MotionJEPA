"""mask 生产入口（make_mask）的逻辑测试（合成 h5，秒级，不碰真实数据）。

覆盖四件事：

1. **帧迭代器**：按 timestep 数字序（h5py 原生是字典序）、不读 segmentation
   （mask 生产全程不看 GT，两种数据源零分叉）、缺 front_rgb/is_video_demo/
   is_completed 逐一 fail-loud。
...
# 重构后同名模块只剩一份（历史上 v4/v4.1/v4.2 三份同名，需要整组 sys.modules 隔离），
# 因此这里直接按目录加载即可。
for _path in (str(ARM_MASK_DIR), str(GT_DATA_DIR)):
    if _path not in sys.path:
        sys.path.insert(0, _path)

make_mask = importlib.import_module("make_mask")
...
MaskParams = arm_mask_mod.MaskParams
cell_counts = grid_mask_mod.cell_counts

CanaryThresholds = make_mask.CanaryThresholds
annotate_episode_arrays = make_mask.annotate_episode_arrays
canary_verdict = make_mask.canary_verdict
iter_frames = make_mask.iter_frames
pick_preview_frames = make_mask.pick_preview_frames
read_sidecar_episode = make_mask.read_sidecar_episode
verify_sidecar = make_mask.verify_sidecar
write_sidecar = make_mask.write_sidecar

FRAME_PIXELS = make_mask.FRAME_PIXELS
...
def test_K13常量与拍板口径():
    assert make_mask.GRID_MIN_PIXELS == 13


def test_demo_prefix前缀与非前缀():
    assert make_mask._demo_prefix(np.array([True, True, False, False])) == 2
    assert make_mask._demo_prefix(np.array([False, False])) == 0
    assert make_mask._demo_prefix(np.array([True, False, True])) == -1
    assert make_mask._phase_segments(np.array([True, False, True])) == 3
    assert make_mask._phase_segments(np.array([True, True])) == 1
...
def test_sidecar写读回环与attrs键集(tmp_path):
    ...
    assert set(loaded["root_attrs"]) == set(make_mask.ROOT_ATTR_KEYS)
    assert set(loaded["attrs"]) == set(make_mask.EPISODE_ATTR_KEYS)
