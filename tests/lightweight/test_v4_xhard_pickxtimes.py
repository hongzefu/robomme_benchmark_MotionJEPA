...
* xhard 新值：num [6,15]、color 3、corner_bias、独立圆盘区域、三个干扰色；
* 守卫放行 xhard 条目的收窄（口径 14 端点单测、corner_bias 对比用），拒绝改原三档；
...
def test_decision_xhard_entries() -> None:
    ...
    assert set(xhard) == {"target_cube_position_policy", "goal_position_policy", "distractor"}
    cube = xhard["target_cube_position_policy"]
    assert cube["region_center"] == [-0.1, 0] and cube["region_half_size"] == 0.2
    assert 0.0 <= cube["corner_bias"] <= 1.0
...
def test_native_blocks_are_independent_copies() -> None:
    ...
    first["xhard"]["target_cube_position_policy"]["corner_bias"] = 0.123
    second, _ = MOD.native_blocks(CLS)
    assert second["xhard"]["target_cube_position_policy"]["corner_bias"] != 0.123
...
@pytest.mark.parametrize("mutate", [
    ...
    lambda d: d["xhard"]["target_cube_position_policy"].__setitem__("corner_bias", 0.0),
])
