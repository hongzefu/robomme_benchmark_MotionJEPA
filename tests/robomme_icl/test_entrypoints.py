...
def entry_modules(monkeypatch):
    ...
    return {name: importlib.import_module(name) for name in (
        "prepare_suite", "generate_dataset", "replay_dataset", "plot_distribution")}
...
def test_recertification_cannot_change_configs(entry_modules):
    with pytest.raises(SystemExit):
        entry_modules["prepare_suite"].main([
            "--source-suite", "old.json", "--task-config", "new.json", "--output-dir", "unused"])
...
def test_phase_preserves_identity_and_allows_worker_change(entry_modules, local_root):
    ...
    for workers in (1, 4):
        with common.run_phase(output, "prepare", {"configs": ({"a": 1}, {"b": 2})}, {"workers": workers}):
            pass
...
def test_phase_preserves_completed_hdf5_after_video_error(entry_modules, local_root):
    ...
    with pytest.raises(RuntimeError, match="编码失败"):
        with common.run_phase(output, "generate", {"suite_hash": "fixed"}, {}) as (root, result):
    ...
    assert common.read_json(output / "run_summary.json")["phases"]["generate"]["hdf5_count"] == 1
    with common.run_phase(output, "generate", {"suite_hash": "fixed"}, {}) as (_, result):
        result.update(hdf5_count=1, video_count=1)
    assert common.read_json(output / "run_summary.json")["status"] == "passed"


def test_summary_retains_other_phase_failure_until_that_phase_recovers(entry_modules, local_root):
    common = importlib.import_module("_icl.common")
    output = local_root / "run"
    with pytest.raises(RuntimeError, match="编码失败"):
...
@pytest.mark.parametrize("first,second", [
    ("prepare", "replay"), ("generate", "replay"),
    ("replay", "prepare"), ("replay", "generate"),
])
def test_replay_and_generation_roots_cannot_mix_roles(entry_modules, local_root, first, second):
    common = importlib.import_module("_icl.common")
    output = local_root / "run"
...
def test_generation_checks_batch_manifest_before_entering_run(entry_modules, local_root):
    configs = load_configs()
    slots = plan_slots(*configs, tasks=["BinFill"], episodes_per_task=2)
    specs = [candidate_for_slot(slot, 0) for slot in slots]
    certificates = {spec.spec_hash: {"passed": True, "repeat_equal": True} for spec in specs}
    local = save_suite(local_root / "run/suite/suite.json", [specs[0]], configs,
                       {specs[0].spec_hash: certificates[specs[0].spec_hash]})
    external = save_suite(local_root / "external/suite.json", [specs[1]], configs,
...
def test_unknown_output_and_reference_paths_are_rejected(entry_modules, local_root):
    ...
    with pytest.raises(ValueError, match="缺少运行身份"):
        with common.run_phase(local_root, "generate", {}, {}):
            pass
    for path in (repository_root() / "data/robomme_data_h5/new", repository_root() / "src/new"):
...
def test_replay_discovers_only_published_inputs(entry_modules, local_root):
    workflow = importlib.import_module("_icl.workflow")
    spec = candidate_for_slot(plan_slots(*load_configs(), tasks=["BinFill"], episodes_per_task=1)[0], 0)
    frames = [{"observation": {"base_rgb": np.zeros((2, 2, 3), dtype=np.uint8)},
               "joint_action": np.zeros(8), "info": {"success": True, "fail": False}}]
...
def test_replay_scan_root_blocks_nested_output_but_allows_batch_replay(entry_modules, local_root, directory_name):
    workflow = importlib.import_module("_icl.workflow")
...
def test_replay_raw_directory_and_single_file_have_explicit_scan_roots(entry_modules, local_root):
    workflow = importlib.import_module("_icl.workflow")
    raw = local_root / "raw"
    raw.mkdir()
