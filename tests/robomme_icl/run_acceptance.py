...
def _run_reset(job: dict) -> dict:
    ...
    assert_identical(certification["runtime_fingerprint"], runtime_fingerprint(), path="runtime_fingerprint")
...
def _run_item(job: dict) -> dict:
    ...
    from robomme_icl.io.hdf5 import assert_records_identical
    ...
    if mode in {"generate", "resume"}:
        ...
        result = replay_episode(data_path, replay_path)
        record = _check_record(replay_path, spec, certification)
        assert_records_identical(data, record)
    else:
        raise ValueError(f"不支持的模式：{mode}")
    return {**result, "passed": True, "frame_count": len(record.frames), "content_hash": record.content_hash}
...
def _validate_completed(row: dict, job: dict, context: dict) -> None:
    ...
    assert_identical(context["runtime_fingerprint"], row.get("runtime_fingerprint"), path="runtime_fingerprint")
...
def main(argv=None) -> int:
    ...
    context = {"schema_version": 1, "mode": args.mode, "workers": args.workers,
               "reverse": args.reverse, "suite_hash": suite["suite_hash"],
    ...
    for spec in specs:
        ...
        assert_identical(fingerprint, certification["runtime_fingerprint"], path="runtime_fingerprint")
    ...
    for row in rows:
        ...
        _validate_completed(row, jobs[key], context)
    ...
    executor = (ProcessPoolExecutor(max_workers=args.workers, mp_context=mp.get_context("spawn"), max_tasks_per_child=1)
                if args.mode == "reset" else ThreadPoolExecutor(max_workers=args.workers))
    try:
        with segment.open("x", encoding="utf-8", buffering=1) as stream:
            operation = _run_reset if args.mode == "reset" else _run_item
            futures = {executor.submit(operation, job): job for job in pending}
            for future in as_completed(futures):
                ...
                if row["passed"]:
                    ...
                    print(f"[{len(completed)}/{len(specs)}] {args.mode} {spec.task_kind} seed={spec.seed} 帧数={result['frame_count']} resumed={result.get('resumed', False)}", flush=True)
                else:
