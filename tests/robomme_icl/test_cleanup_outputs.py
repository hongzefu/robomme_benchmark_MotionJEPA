...
def scenario(monkeypatch):
    ...
    with tempfile.TemporaryDirectory(
        prefix="cleanup-fake-only-", dir=cache
    ) as directory:
        root = Path(directory)
        ...
        (root / "FIXTURE_ONLY.md").write_text(
            "仅测试用假规格、假媒体及假验收，不能作为正式验收证据。\n"
        )
        suite = {
            "episodes": [],
            "certification": {},
            "configs": {"test_fixture_only": True},
            "suite_hash": "fake-new-suite",
        }
        verified = []
        for task_index, task in enumerate(cleanup.TASKS):
            for episode in range(24):
                ...
                spec = {
                    "task_kind": task,
                    "seed": seed,
                    "difficulty": ("easy", "medium", "hard")[episode // 8],
                    "spec_hash": spec_hash,
                    "schedule": {"control_freq": 20},
                    "test_fixture_only": True,
                }
                certification = {
                    "content_hash": f"fake-content-{seed}",
                    "frame_count": 1,
                    "render_gpu": seed % 2,
                ...
                for stage in ("cert0", "cert1", "generate", "replay"):
                    base = (
                        ...
                        else (run if stage == "generate" else run / "replay")
                    )
                    h5 = base / "hdf5_files" / task / f"seed_{seed}.h5"
                    h5.parent.mkdir(parents=True, exist_ok=True)
                    with h5py.File(h5, "w") as handle:
                        handle.attrs.update(
                            complete=True, content_hash=certification["content_hash"]
                        )
                        handle.create_group("setup").create_dataset(
                            "episode_spec", data=json.dumps(spec)
                        )
        ...
        (distribution / "distribution_summary.json").write_text(
            json.dumps({"suite_hash": suite["suite_hash"], "episodes_total": 96})
        )
        verification = {
            "schema_version": 1,
            "status": "passed",
            "test_fixture_only": True,
            "run_root": str(run),
            "suite": str(run / "suite/suite.json"),
            "suite_hash": suite["suite_hash"],
            "source_suite_hash": source["suite_hash"],
            "episodes": 96,
            "hdf5_verified": 384,
            "videos_verified": 192,
            "source_specs_unchanged": True,
            "configs_unchanged": True,
            "strict_original_frames_equal": True,
            "episodes_verified": verified,
            "frames_per_pass": 96,
            "figures": {
                "image_count": 4,
                "actual_counts_equal": True,
                "position_coverage_equal": True,
                "figures": images,
            },
        }
        ...
        def save_verification(value):
            ...
            (report / "verification.json").write_text(
                json.dumps(cleanup._signed(value))
            )
...
def test_apply_only_deletes_listed_artifacts_without_following_symlinks(scenario):
    ...
    result = cleanup.cleanup(
        scenario.inventory, scenario.run, scenario.report, apply=True
    )
    ...
    assert {Path(row["path"]).name for row in receipt["deletions"]} == {
        "old-fixture",
        "old.h5",
        "reference-link",
    }
    ...
    assert (
        sum(path.is_file() for path in (scenario.run / "hdf5_files").rglob("*.h5"))
        == 192
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "artifacts/generated",
        "artifacts/generated/robomme-icl/scripts-v1",
        "artifacts/reports",
        ".cache/uv",
        ".cache/maniskill",
        "src",
...
def test_protected_data_new_batch_ancestors_and_dependencies_are_rejected(
    scenario, relative
):
...
def test_staging_hardlinks_are_excluded_but_unknown_published_hdf5_blocks_cleanup(
    scenario,
):
...
@pytest.mark.parametrize(
    "key,value", [("status", "failed"), ("episodes", 95), ("videos_verified", 191)]
)
...
def test_unsigned_verification_blocks_cleanup(scenario):
    ...
    (scenario.report / "verification.json").write_text(
        json.dumps(scenario.verification)
    )
