...
from robomme_icl.workflows import recertify as module
...
def _frames():
    ...
    return [
        {
            "observation": {
                "base_rgb": np.arange(12, dtype=np.uint8).reshape(2, 2, 3),
...
def scenario(monkeypatch):
    ...
    with tempfile.TemporaryDirectory(prefix="recertify-", dir=cache) as directory:
        ...
        slots = plan_slots(
            *configs, tasks=["BinFill", "VideoRepick"], episodes_per_task=2
        )
        specs = [candidate_for_slot(slot, 0) for slot in slots]
        current = {
            "icl_source_hash": "new-source",
            "legacy_source_hash": "same-legacy",
            "uv_lock_hash": "same-lock",
        ...
        for spec in specs:
            ...
            paths = [
                write_episode(
                    root / "source" / f"{spec.seed}-{index}.h5",
                    spec,
                    _frames(),
        ...
        source = save_suite(
            root / "source" / "suite.json", specs, configs, certification
        )
        calls = []
        ...
        def runner(spec, destination, *, render_gpu, timeout_seconds):
            ...
            return write_episode(
                ...
                spec,
                _frames(),
                runtime_fingerprint=fingerprint(render_gpu=render_gpu),
        ...
        monkeypatch.setattr(module, "new_manager", _Manager)
        monkeypatch.setattr(
            module,
            "run_jobs",
            lambda function, jobs, workers, stop: [function(job) for job in jobs],
        )
        monkeypatch.setattr(module, "run_episode_process", runner)
...
def test_source_change_recertifies_same_full_specs_gpu_and_frames(scenario):
    ...
    for spec in scenario.specs:
        ...
        assert (
            report["content_hash"]
            != old["certification"][spec.spec_hash]["content_hash"]
        )
...
@pytest.mark.parametrize(
    "key,value",
    [
        ("uv_lock_hash", "changed-lock"),
        ("gpu_devices", ["replacement-gpu"]),
        ("legacy_source_hash", "changed-legacy"),
...
def test_filter_preserves_source_order_episode_numbers_and_seeds(scenario):
    manifest = module.recertify_suite(
        scenario.source,
        scenario.root / "new",
        tasks=["VideoRepick"],
        episodes_per_task=1,
    ...
    selected = [
        spec.to_dict() for spec in scenario.specs if spec.task_kind == "VideoRepick"
    ][:1]
...
def test_any_new_frame_or_spec_difference_stops_and_preserves_record(
    ...
    def changed(spec, destination, *, render_gpu, timeout_seconds):
        ...
        return write_episode(
            ...
            spec,
            frames,
            runtime_fingerprint=scenario.fingerprint(render_gpu=render_gpu),
        )

    monkeypatch.setattr(module, "run_episode_process", changed)
    ...
    def failing_once(spec, destination, **kwargs):
        ...
        if len(attempts) == 1:
            Path(destination).write_bytes(
                "模拟基础设施失败留下的不完整文件".encode("utf-8")
            )
            raise OSError("模拟设备短暂失败")
        return scenario.runner(spec, destination, **kwargs)

    monkeypatch.setattr(module, "run_episode_process", failing_once)
    module.recertify_suite(
        scenario.source, scenario.root / "new", episodes_per_task=1, tasks=["BinFill"]
    )
...
def test_second_run_task_failure_is_reproducibility_failure(scenario, monkeypatch):
    ...
    monkeypatch.setattr(module, "run_episode_process", second_failure)
...
def test_unknown_or_broken_resume_records_are_preserved(scenario):
    output = scenario.root / "new"
    module.recertify_suite(
        scenario.source, output, tasks=["BinFill"], episodes_per_task=1
    )
...
def test_geometry_rejection_never_searches_replacement(scenario, monkeypatch):
    def rejected(*args, **kwargs):
        raise CandidateRejected("原版真实碰撞")

    monkeypatch.setattr(module, "run_episode_process", rejected)
    with pytest.raises(ReproducibilityError, match="禁止修改 seed 或更换候选"):
        module.recertify_suite(scenario.source, scenario.root / "new")
    assert not scenario.calls
...
def test_output_must_not_touch_source_or_reuse_different_selection(scenario):
    ...
    with pytest.raises(ReproducibilityError, match="恢复上下文"):
        module.recertify_suite(
            scenario.source, output, tasks=["VideoRepick"], episodes_per_task=1
        )


@pytest.mark.parametrize("state", ["missing", "incomplete", "truncated", "complete"])
def test_interrupted_bound_attempt_resumes_same_spec_without_overwriting(
    scenario, monkeypatch, state
):
    output = scenario.root / "new"
    attempted = []
    ...
    monkeypatch.setattr(module, "run_episode_process", interrupted)
    ...
    monkeypatch.setattr(module, "run_episode_process", scenario.runner)
    manifest = module.recertify_suite(
        scenario.source, output, tasks=["BinFill"], episodes_per_task=1
    )
...
def test_interruption_after_first_success_only_runs_second_repeat(
    ...
    monkeypatch.setattr(module, "run_episode_process", second_interrupted)
    with pytest.raises(KeyboardInterrupt):
        module.recertify_suite(
            scenario.source, output, tasks=["BinFill"], episodes_per_task=1
        )
...
def test_completion_receipt_prevents_treating_lost_complete_record_as_interruption(
...
def test_explicit_task_failure_is_latched_before_future_retries(
    scenario, monkeypatch, error_type
):
    output = scenario.root / "new"
    ...
    monkeypatch.setattr(module, "run_episode_process", failure)
...
def test_frame_difference_latches_and_cannot_be_retried_as_crash(scenario, monkeypatch):
    ...
    monkeypatch.setattr(module, "run_episode_process", difference)
...
def test_complete_but_tampered_attempt_without_receipt_is_never_resampled(
    scenario, monkeypatch
):
    output = scenario.root / "new"
    ...
    monkeypatch.setattr(module, "run_episode_process", interrupted_after_corruption)
    with pytest.raises(KeyboardInterrupt):
        module.recertify_suite(
            scenario.source, output, tasks=["BinFill"], episodes_per_task=1
        )
...
def test_interrupted_attempt_requires_identity_and_known_files(
    scenario, monkeypatch, damage
):
    output = scenario.root / "new"
