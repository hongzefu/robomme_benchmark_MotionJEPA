...
def test_generation_mismatch_stops_and_preserves_evidence(local_dir):
    ...
    with pytest.raises(ReproducibilityError, match="认证内容不同"):
        generate_one(FakeSpec(), path,
                     runner=lambda spec, destination: write_episode(destination, spec, FakeEnv().frames()),
...
def test_prepare_rejects_geometry_then_certifies_same_slot_candidate(local_dir, monkeypatch):
    ...
    monkeypatch.setattr(geometry, "validate_spec_geometry", lambda spec: {
        "ok": spec.seed != 1900000001, "reasons": ["模拟碰撞"], "min_clearance": 0.01,
    })
    calls = []
    ...
    assert (manifest.parent / "certification/BinFill-easy-0/candidate_0000/rejected.json").exists()
...
def test_runtime_fingerprint_is_part_of_hdf5_integrity(local_dir):
    path = write_episode(local_dir / "episode.h5", FakeSpec(), FakeEnv().frames(),
                         runtime_fingerprint={"driver": "固定版本"}, source_commit="首次提交")
...
def test_partial_generation_retries_same_seed_in_new_staging_and_publishes_only_complete(local_dir):
    spec, calls = FakeSpec(), []
...
def test_prepare_resume_skips_rejected_candidate_and_completed_repeat(local_dir, monkeypatch):
    ...
    monkeypatch.setattr(geometry, "validate_spec_geometry", geometry_check)
    calls = []
...
def test_prepare_resume_rejects_complete_repeat_with_wrong_fingerprint(local_dir, monkeypatch):
    ...
    def interrupted(spec, path, **_):
        write_episode(path, spec, FakeEnv().frames(), runtime_fingerprint={"testing": "old"})
