...
def test_native_single_episode(task):
    ...
    for candidate_index in range(16):
        ...
        try:
            actors = getattr(env.unwrapped, "all_cubes", getattr(env.unwrapped, "spawned_cubes", []))
            for actor in actors:
                assert np.array_equal(actor.pose.p.cpu().numpy(), actor.initial_pose.p.cpu().numpy())
            if len(actors) > 1:
                assert len({tuple(actor.pose.p[0].tolist()) for actor in actors}) == len(actors)
            ...
            print(f"{task}: candidate={candidate_index}，{len(frames)}操作，{frames[-1]['info']['step']}物理步，原版任务完成")
            return
        finally:
            env.close()
