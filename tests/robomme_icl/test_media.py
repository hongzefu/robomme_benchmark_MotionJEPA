...
def synthetic_record(local_dir):
    ...
    spec = {"seed": 2_000_000_000, "env_id": "RoboMME-ICL/BinFill-v0", "schedule": {"control_freq": 20}}
    spec["spec_hash"] = tree_hash(spec)
    frames = []
    for index in range(8):
        frames.append({"observation": {"base_rgb": np.full((256, 256, 3), (30 + index, 80, 100), dtype=np.uint8),
                                       "wrist_rgb": np.full((256, 256, 3), (100, 100 + index, 40), dtype=np.uint8)},
                       "joint_action": np.arange(8, dtype=np.float64),
                       "info": {"is_demonstration": index < 4, "step": index, "success": index == 7}})
    return write_episode(local_dir / "source.h5", spec, frames)


def test_compose_frame_has_red_border_without_mutating_source():
    frame = {"observation": {"base_rgb": np.full((256, 256, 3), 80, dtype=np.uint8),
...
def test_real_video_export_verify_resume_and_missing_video_rebuild(local_dir, synthetic_record):
    ...
    assert first["verification"] == {"frames": 8, "fps": 20, "width": 512, "height": 256, "decodable": True}
    assert first["resumed"] is False
    assert json.loads(target.with_suffix(".json").read_text())["source_content_hash"] == read_episode(synthetic_record).content_hash
    ...
    decoded = subprocess.run([video.FFMPEG, "-v", "error", "-threads", "1", "-i", str(target),
...
def test_video_failure_keeps_hdf5_and_allows_retry(local_dir, synthetic_record, monkeypatch):
    before = video._sha256(synthetic_record)
...
def synthetic_suite(local_dir):
    ...
    specs = [candidate_for_slot(slot, 0) for slot in plan_slots(*configs, episodes_per_task=1)]
    certification = {spec.spec_hash: {"passed": True, "repeat_equal": True, "testing": "媒体轻量测试伪认证"} for spec in specs}
    return save_suite(local_dir / "suite", specs, configs, certification)


def test_real_distribution_images_summary_resume_and_missing_plot(local_dir, synthetic_suite):
    ...
    assert all(sum(dimension["index_counts"].values()) == 1 for dimension in dimensions.values())
