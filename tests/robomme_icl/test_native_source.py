...
def test_native_subgoal_callables_are_not_rewritten(task):
    ...
    path = f"src/robomme/robomme_env/{task}.py"
    original = subprocess.run(["git", "show", f"{NATIVE_REFERENCE}:{path}"], cwd=ROOT,
...
@pytest.mark.parametrize("filename", ["object_generation.py", "statechange.py", "subgoal_evaluate_func.py",
                                     "subgoal_planner_func.py", "task_goal.py", "planner_fail_safe.py"])
