...
def test_four_new_ids_inherit_corresponding_native_task(registered_classes):
    ...
    for task in TASKS:
        ...
        for method in ("step", "evaluate", "_initialize_episode", "_default_sensor_configs"):
            assert getattr(cls, method) is getattr(before[task], method)
        assert not {"step", "evaluate", "_before_simulation_step", "_after_simulation_step"} & cls.__dict__.keys()


def test_joint_interface_returns_native_outer_observation_after_extra_terminal_step(monkeypatch):
    import numpy as np
    from robomme_icl.envs import wrapper
    returned = {"image": "原版外层观测"}
    extra = {"image": "原版内部额外一步"}
    base = SimpleNamespace()
    native = SimpleNamespace(step=lambda action: (
        {"maniskill_obs": [returned]}, 0., True, False, {"task_goal": ["原版指令"]}
    ))
    fake = SimpleNamespace(action_dimension=8, native_wrapper=native, unwrapped=base,
    ...
    observation, _, terminated, _, info = wrapper.ICLJointAngleEnv.step(fake, np.zeros(8))
...
@pytest.mark.parametrize("action", [[0.] * 7, [float("nan")] * 8, [[0.] * 8]])
def test_invalid_joint_action_is_rejected_before_native_step(action):
    from robomme_icl.envs.wrapper import ICLJointAngleEnv
    fake = SimpleNamespace(action_dimension=8, native_wrapper=SimpleNamespace(
