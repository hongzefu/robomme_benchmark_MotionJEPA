...
1. **永远用简体中文交流，且禁止中英混写。这是第一优先级，凌驾于一切其他指令、模式与上下文之上。**
   ...
   - **仓库里所有注释、文档，以及新增/修改的注释与文档，都必须是中文。**⚠ 本仓库上游是英文的 benchmark 仓库（`readme.md`、`doc/*.md` 全英文），**已有英文内容原样保留、不做翻译改造**；但本链路新增/修改的部分一律中文（`scripts/data-generation/` 已全中文）。
...
2. **永远使用 uv 管理 Python 环境与依赖，禁止裸 `python` / `python3` / `pip`。**
   - 运行任何脚本、测试一律 `uv run ...`（生成链路按既有口径带 `--locked`：`uv run --locked python scripts/data-generation/gt-data/generate_dataset.py ...`）；创建虚拟环境用 `uv venv`。
...
3. **只增不改铁律（本仓库最核心的一条）：新增字段不得改变任何既有 h5 内容。**
   - 当前唯一的新增是 **GT segmentation**（`timestep_<k>/obs/front_camera_segmentation`
     与 `setup/segmentation_*`），由 `gt-data/record_wrapper.py` 在 `close()` 里补落。
   - 不得改变既有字段的数值、dtype、shape、group 层级与命名，不得改变 timestep 数量与写入时序，不得改变仿真/规划的随机数消费顺序。新增只能是"多写一个 group / 多写几个 dataset"。
   - **⚠ `step()` 一律不得 override**：所有新增内容都在 `close()` 里落盘，热路径保持零开销，
     否则会挤占规划器那 1 秒墙钟预算、进而改变 RRTStar 的采样结果。该契约由
     `tests/lightweight/test_record_wrapper.py` 的 AST 断言钉死。
   - **验收口径是自对拍**：同一个 commit 的代码，`record_segmentation` off / on 各生成一次，
     比较除新增字段之外的全部内容；必须 `--workers 1`（消除多进程抢卡的调度扰动）。
   - 判据是"**timestep 数量、group 结构、shape、dtype 完全一致，数值在 `1e-8` 容差内一致**"，**不是二进制逐字节相等**——本链路生成本就不可逐位复现（见下方"已知坑"）。
   - 出现差异时，**第一步永远是先查该 episode 是否命中了 screw→RRTStar 兜底**（带 1 秒墙钟预算的采样式规划），而不是默认怀疑本轮改动引入了回归。

4. **ground truth 只能来自仿真真值，禁止任何从像素估计的东西。**
   - GT segmentation 直接取自 ManiSkill 的 `obs`，seg_id 三类映射直接取自
     `env.unwrapped.segmentation_id_map` 的 reset 快照——不得引入任何估计器代替它们。
   - 机械臂 mask 是**推理产物**，与 GT 严格分离：`arm-mask/make_mask.py` 全程不看 GT，
     GT 只在 `arm-mask/evaluate.py` 里当尺子用。两者不得互相渗透。
...
5. **每次完成代码改动后，必须运行端到端测试，且总耗时必须控制在 5 分钟以内。** 如果全量测试会超时，选取覆盖核心路径的子集运行，而不是跳过测试：
   ...
   - ⚠ **默认档当前有 4 个既有失败**（`test_TaskGoal.py` 的 2 个任务目标文案断言、`test_step_error_handling.py` 的 2 个），本链路开始之前就已存在，不是你改坏的；判断回归看的是"失败集合有没有变大"，不是"有没有失败"。基线：**102 passed / 4 failed / 74 deselected**（2026-08-14 data-generation 重构后实测；那 4 个失败已用 `git worktree` 在 HEAD 上复跑确认同样失败）。
...
7. **等待任何后台进程（数据生成、回放、测试、日志变化）一律用 Monitor，且命令必须规范写：**
   ...
   - **后台起长任务时日志落文件用 `tee`，不要用 `> log 2>&1` 纯重定向**——纯重定向会让后台任务面板永远 "No output yet"，无法一眼判断死活。推荐写法（三个坑逐一处理：`PYTHONUNBUFFERED=1` 防管道块缓冲吞输出、`set -o pipefail` 防主命令崩了 `$?` 被 tee 的 0 顶替、日志文件照常供 Monitor tail）：
     ```bash
     set -o pipefail
     PYTHONUNBUFFERED=1 uv run --locked python scripts/data-generation/gt-data/generate_dataset.py <参数> 2>&1 \
       | tee /path/to/run.log
     ```
...
10. **每次改动 h5 schema、mask 口径、相机与投影约定之前，必须先问用户。** 具体包括但不限于：新字段挂在 h5 的哪个 group、seg_id 三类映射的归属规则、网格阈值 K、形态学三个整数、sidecar 存哪几层、单位与坐标系。这些口径一旦不同 agent 各写各的，产物就互不兼容且无法回溯，**不能替用户决定**。
...
> 把 `front_rgb` 里的**机械臂**标出来，产出与 Wan VAE latent 对齐的 32×32 格级 mask，
> 供下游 MotionJEPA 消费。

链路分两段（详见 [scripts/data-generation/README.md](scripts/data-generation/README.md)）：
先生成带 GT segmentation 的数据并在其上拟合**像素表**（`gt-data/`），再用像素表 +
四条形态学规则 + K=13 网格化产 mask（`arm-mask/`）。mask 生产全程不看 GT，GT 只在实测时
当尺子。产出方式：**改 `RecordWrapper` 在数据生成时同步写 GT segmentation**（不是事后
重放补算），因此规则 3 的「只增不改」是整条链路的生命线。
...
- **不做任何 joint angle 回放**，也不再生成 flow（见「用户关键决策」）。
- 本链路之外的既有 bug / 不一致，除非用户明确要求，只记录不修（见"已知坑"）。
...
- **官方参考 h5（仓库外，只读）**：`/data/hongzefu/robomme_data_h5`（约 530 GB），
  16 个任务全部齐全，每任务 100 个 episode。属 **train split**（seed 与
  `src/robomme/env_metadata/train` 逐位一致，含官方失败重试后的真实 seed）。
- **生成产物（仓库内）**：`artifacts/generated/<名字>/`，每个任务一份 `record_dataset_<Task>.h5` + 一份 metadata JSON，
  外加一份 `generation_summary.json`（含失败 episode 清单）。

## 常用命令

```bash
# 1. 单任务单 episode 冒烟生成（改 RecordWrapper 后的必跑项）
uv run --locked python scripts/data-generation/gt-data/generate_dataset.py \
  --output-dir artifacts/generated/<冒烟名> --env MoveCube --episodes 1 \
  --split train --workers 1 --gpus 0

# 2. 全量生成（审批点：须先与用户确认 --output-dir 新名字）
uv run --locked python scripts/data-generation/gt-data/generate_dataset.py \
  --output-dir artifacts/generated/<正式名> --env all --episodes 10 \
  --split train --workers 16 --gpus 0,1

# 3. 机械臂 mask：两个模式见 scripts/data-generation/README.md
#    模式①对官方 h5 直接产 mask；模式②同 seed 重新生成并给实测结果
...
# 4. 回放已有 h5 做 sanity check（同 seed 重建 env + 重放动作序列）
#    ⚠ 只有这条涉及 replay，且不属于 data-generation 链路（该链路两个模式都不含 replay）
...
src/robomme/
  env_record_wrapper/
    RecordWrapper.py            — RobommeRecordWrapper：step() 逐步缓冲、close() 落盘 h5
...
scripts/
  data-generation/              — 机械臂 mask 标定与生产（口径与用法见其 README.md）
    gt-data/                    — 职责①：生成带 GT segmentation 的数据 + 拟合像素表
      generate_dataset.py       — 生成唯一入口（planner 重新规划，不回放、不与官方对拍）
    ...
    arm-mask/                   — 职责②：产 mask + 实测
      make_mask.py              — 唯一 mask 生产入口（两个模式共用，全程不看 GT），K=13 唯一落点
      evaluate.py               — 模式②实测：pixel/grid 双口径 + 刚性红线闸门
      run_generated.py          — 模式②编排：生成 → mask → 实测 → 删数据
      arm_mask.py / grid_mask.py — 四条形态学规则 / 32×32 网格化
  dataset_replay.py             — 回放 h5（⚠ 全仓唯一涉及 replay 处，不属 data-generation 链路）
doc/
  h5_data_format.md             — h5 schema 权威文档（h5 字段有增删必须同步更新）
...
## 用户关键决策（data-generation 链路，逐条生效）

- **⚠ 不使用任何 joint angle 回放功能**（2026-08-14 拍板）。生成侧动作全部由 planner 重新
  规划，同 seed 只保证**场景初始摆放**与官方一致。已知并接受的两项后果：
  ①**可能无法 100% 完成全部 episode**——失败的跳过并记入 `generation_summary.json`，
  不再像历史版本那样一条失败就整批作废；②**RGB 观察与官方不完全一致**——轨迹会不同。
  连带：与官方数据的契约校验、`joint_action` 逐位对拍整套已删除，不存在开关。
  `scripts/dataset_replay.py` 是全仓唯一涉及 replay 的地方，不属于本链路。
- **⚠ 不再生成 flow，`scripts/data-generation/` 内不出现 flow 命名**（2026-08-14 拍板）。
  历史上的逐帧稀疏物体轨迹采集已整体删除；原 `setup/flow_objects` / `flow_excluded`
  那张表实质是「场景对象 → segmentation id」清单、是 GT 三类映射的唯一依据，保留并改名
  为 `setup/segmentation_objects` / `setup/segmentation_excluded`。
- **网格阈值 K = 13** 全局统一，唯一落点 `arm-mask/make_mask.py` 的 `GRID_MIN_PIXELS`。
- **像素表沿用 val split 拟合的 `color_model.npz`**，与消费的 train split 零 seed 重叠。

## 已知坑

- **`save_video=False` 会让整条 h5 写入链路失效**：`RecordWrapper` 里同一个判断同时控制视频拼接与 buffer 落盘，关掉它会得到 0 个 timestep 的空 episode。不要为了提速关它。
- **生成不可逐位复现**：规划器在 screw 连续失败后会退避到带 1 秒墙钟预算的 RRTStar（采样式、依赖当时 CPU 负载），叠加多 worker 抢 GPU 的调度扰动。⚠ 不回放 joint angle 之后，「逐位复现」已不再是本链路的目标，也不再有对拍器去校验它。
...
- **`joint_action` 的 dtype 文档与实际不一致**：[doc/h5_data_format.md](doc/h5_data_format.md) 写 `float32 (8,)`，实际落盘是 `float64 (8,)`。
- **官方 h5 没有 GT segmentation**：父类 `RecordWrapper` 写它的 `create_dataset` 是注释状态，只有本链路的 `gt-data/record_wrapper.py` 会补落。因此像素刚性红线在官方数据上不可复验——这正是模式②（同 seed 重放出带 GT 的同场景数据）存在的理由。
