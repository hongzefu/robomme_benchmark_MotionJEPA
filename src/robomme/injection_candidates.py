"""候选封套读取器——`scripts/injection/candidates/io.py` 的同源副本。

本文件存在的唯一理由：`pyproject.toml` 的
`[tool.hatch.build.targets.wheel] packages = ["src/robomme"]` 不包含 `scripts/`，
所以外部仓库（策略侧评测）装了 robomme 之后 import 不到那份读取器。
`scripts/INJECTION.md` 规定 `candidates.jsonl` 是起环境的唯一数据入口，
策略侧要遵守同一读取契约，就必须能拿到这段校验逻辑。

**除本 docstring 外，两份文件的正文必须逐字节相同。** 复制时源文件 sha256：
3397ac440c0182676d712b941781ea036e9d6912f6fbc16977c3f050ad3a9f4d
主仓库 `scripts/evaluation/verify_sources.py` 会断言「两份文件各自剥掉首个模块 docstring 之后的
正文字节一致」，漂移在评测前置闸即被拦下。
改动请改 `scripts/injection/candidates/io.py` 再同步过来，不要只改一边。
"""
