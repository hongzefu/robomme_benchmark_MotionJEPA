...
def plan_slots(task_config, position_config, tasks=None, episodes_per_task=None):
    ...
    if episodes_per_task is not None:
        ...
        quotas = {name: episodes_per_task // 3 + (index < episodes_per_task % 3)
...
def distribution_summary(slots):
    ...
    return {"episodes": len(slots), "by_task_difficulty": {
        f"{task}/{difficulty}": count for (task, difficulty), count in sorted(counts.items())
    }}
