...
class NativePlacements:
    ...
    def fit_actor(actor, point, bounds):
        ...
        for index, axis in enumerate(("x", "y")):
            ...
            low_offset = min(box.center[index] - box.radius_on(direction) for box in parts) - center[index]
            high_offset = max(box.center[index] + box.radius_on(direction) for box in parts) - center[index]
    ...
    def build_board(self, env, **kwargs):
        ...
        kwargs.update(position=[*self.center(point, bounds), 0.],
    ...
    def spawn_cube(self, env, **kwargs):
        ...
        kwargs.update(region_center=provisional, region_half_size=[half_size + 1e-9] * 2,
                      avoid=[], include_existing=False, include_goal=False, min_gap=0., max_trials=1)
        cube = spawn_random_cube(env, **kwargs)
        angle = math.radians(point["yaw_degrees"]) / 2
        cube.set_pose(sapien.Pose([*provisional, half_size],
    ...
    def spawn_bin(self, env, **kwargs):
        ...
        provisional = [sum(bounds[axis]) / 2 for axis in ("x", "y")]
        actor = build_bin(env, callsign=kwargs.get("name_prefix", "bin"),
