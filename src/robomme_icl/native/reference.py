...
def load_reference(reference_root):
    ...
    listing = subprocess.run(["git", "ls-tree", "-r", "-z", NATIVE_REFERENCE, "--", "src/robomme"],
                             cwd=repository_root(), check=True, capture_output=True).stdout
    count = 0
...
def _fixed_sampling(module, spec):
    ...
    def center(point, bounds):
        return [bounds[axis][0] + point[f"{axis}_fraction"] * (bounds[axis][1] - bounds[axis][0])
                for axis in ("x", "y")]
    ...
    def fit_reference_actor(actor, point, bounds):
        ...
        for axis, key in enumerate(("x", "y")):
            ...
            lower_offset = min(box.center[axis] - box.radius_on(direction) for box in boxes) - position[axis]
            upper_offset = max(box.center[axis] + box.radius_on(direction) for box in boxes) - position[axis]
    ...
    if spec.task_kind in ("BinFill", "VideoRepick"):
        ...
        def cube(env, **kwargs):
            ...
            kwargs.update(region_center=provisional, region_half_size=[size + 1e-9, size + 1e-9],
            ...
            actor.set_pose(sapien.Pose([*provisional, size],
    ...
    if spec.task_kind == "BinFill":
        ...
        def board(env, **kwargs):
            ...
            half_angle = math.radians(point["yaw_degrees"]) / 2
            kwargs.update(position=[*center(point, bounds), 0.],
    ...
    if spec.task_kind == "VideoUnmaskSwap":
        object_generation = importlib.import_module("robomme.robomme_env.utils.object_generation")

        def container(env, **kwargs):
            point, bounds = next_point("containers")
            provisional = [sum(bounds[axis]) / 2 for axis in ("x", "y")]
            actor = object_generation.build_bin(env, callsign=kwargs["name_prefix"],
                                               position=[*provisional, .002],
...
def make_reference_env(spec, render_gpu):
    ...
    if spec.task_kind == "BinFill":
        configs[spec.difficulty] = dict(color=params["scene_color_count"],
        ...
        configs[spec.difficulty] = dict(length=[params["walk_steps"]] * 2,
                                       backtrack=params["allow_backtracking"])
    elif spec.task_kind == "VideoUnmaskSwap":
        configs[spec.difficulty] = dict(bin=params["container_count"], pick_min=params["pick_count"],
                                       pick_max=params["pick_count"], swap_min=params["swap_count"],
                                       swap_max=params["swap_count"])
    else:
        configs[spec.difficulty] = dict(cube=params["spawn_count"], swap_min=params["swap_count"],
                                       swap_max=params["swap_count"])
    cls.configs = configs
    try:
        with _fixed_sampling(module, spec):
            raw = gym.make(spec.task_kind, seed=spec.seed, difficulty=spec.difficulty, num_envs=1,
                           obs_mode="rgb+depth+segmentation", control_mode="pd_joint_pos", render_mode="rgb_array",
                           sim_backend="physx_cpu", render_backend=f"cuda:{render_gpu}", reward_mode="dense")
    finally:
    ...
    if spec.task_kind == "RouteStick":
        ...
        for index, target in enumerate(raw.unwrapped.buttons_grid):
            ...
            position = [x * math.cos(angle) - y * math.sin(angle), x * math.sin(angle) + y * math.cos(angle)]
            target.set_pose(sapien.Pose([*position, .01 if index % 2 == 0 else -.01], target.pose.q[0].cpu().numpy()))
            if index % 2:
                obstacle = raw.unwrapped.target_cubes[index]
                obstacle.set_pose(sapien.Pose([*position, .05], obstacle.pose.q[0].cpu().numpy()))
    return raw


def run_reference_episode(spec, render_gpu):
    ...
    wrapper = DemonstrationWrapper(recorder, max_steps_without_demonstration=10000, gui_render=False,
    ...
    try:
        ...
        options = dict(debug=False, vis=False, base_pose=raw.unwrapped.agent.robot.pose,
        ...
        policy = OraclePlannerDemonstrationWrapper(wrapper, env_id=spec.task_kind, gui_render=False)
