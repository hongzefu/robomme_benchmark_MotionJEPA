...
from .compiler import validate_configs
from .spec import EpisodeSpec, canonical_json, content_hash
...
def _check_manifest(value: dict) -> list[EpisodeSpec]:
    if value.get("schema_version") != 1 or value.get("status") != "certified":
        raise ValueError("只能加载已认证的 schema_version=1 清单")
