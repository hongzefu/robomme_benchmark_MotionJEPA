...
def validate_configs(task_config: dict, position_config: dict) -> None:
    if (
        task_config.get("schema_version") != 1
        or position_config.get("schema_version") != 2
    ):
    ...
    if task_config["task_order"] != list(TASKS) or set(task_config["tasks"]) != set(
        TASKS
    ):
    ...
    for value in [
        *quotas.values(),
        task_config["compiler_seed"],
        task_config["episode_seed_start"],
        position_config["compiler_seed"],
    ]:
    ...
    if (
        type(clearance) not in (float, int)
        ...
        for difficulty, parameters in task_config["tasks"][task].items():
            ...
            for key, values in parameters.items():
                if (
                    ...
                    raise ValueError(
                        f"{task}.{difficulty}.{key} 必须为不重复的非空列表"
                    )
                for value in values:
            ...
            if (
                task == "BinFill"
                and max(
                    parameters["scene_color_count"] + parameters["target_color_count"]
                )
                > 3
            ):
                raise ValueError("BinFill 仅有三个原色")
            if task == "VideoUnmaskSwap" and (
                not set(parameters["container_count"]) <= {3, 4}
                or max(parameters["pick_count"]) > 2
            ):
                raise ValueError("原版仅支持3/4容器和1/2次抓取")
            if (
                task in ("VideoUnmaskSwap", "VideoRepick")
                and max(parameters["swap_count"]) > 3
            ):
            ...
            if task == "VideoRepick":
                if parameters["spawn_count"] != ([15] if difficulty == "hard" else [3]):
                    raise ValueError(
                        "VideoRepick 原版 easy/medium 为3方块，hard为15方块"
                    )
...
def load_configs(task_path=None, position_path=None) -> tuple[dict, dict]:
    task_path = (
        Path(task_path) if task_path else CONFIG_DIRECTORY / "task_distribution.json"
    )
    position_path = (
        Path(position_path)
        if position_path
        else CONFIG_DIRECTORY / "position_distribution.json"
    )
