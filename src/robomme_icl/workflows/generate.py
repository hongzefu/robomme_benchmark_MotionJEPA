...
def generate_one(
    ...
    *,
    ...
    render_gpu = (
        _render_gpu_from_fingerprint(expected_runtime_fingerprint)
        if expected_runtime_fingerprint is not None
        else 0
    )
    operation = runner or (
        lambda candidate, destination: run_episode_process(
            candidate,
            destination,
...
def generate_spec_job(job: dict) -> dict[str, Any]:
    ...
    try:
        ...
        result = generate_one(
            spec,
            path,
            expected_content_hash=certification["content_hash"],
            expected_runtime_fingerprint=fingerprint,
            runner=run,
            timeout_seconds=job["timeout_seconds"],
        )
        print(
            f"{'已复用' if result['resumed'] else '已生成'} {spec.task_kind} seed={spec.seed} GPU={render_gpu}",
            flush=True,
        )
...
def generate_suite(
    ...
    *,
    ...
    with new_manager() as manager:
        ...
        gpus = sorted(
            {
                _render_gpu_from_fingerprint(
                    suite["certification"][spec.spec_hash]["runtime_fingerprint"]
                )
                for spec in specs
            }
        )
