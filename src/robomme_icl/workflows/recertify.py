...
from robomme_icl.errors import (
...
from robomme_icl.io.state import (
    _bind_context,
    _exclusive_json,
    _read_json,
    _validate_record,
)
from robomme_icl.workflows.generate import _render_gpu_from_fingerprint
from robomme_icl.workflows.workers import (
    gpu_limits,
    new_manager,
    run_jobs,
    retry_same_spec,
...
def _select_specs(
    suite: dict, tasks: Sequence[str] | None, episodes_per_task: int | None
) -> list:
...
def _source_record_paths(certification: dict, source_dir: Path) -> list[Path]:
    ...
    result = [
        output_path(Path(path) if Path(path).is_absolute() else source_dir / path)
        for path in paths
    ]
...
def _assert_cross_version(old_record: Any, new_record: Any) -> None:
    ...
    assert_identical(
        old_record.episode_spec, new_record.episode_spec, path="跨版本 episode_spec"
    )
...
def _check_receipt(path: Path, identity: dict) -> dict:
    ...
    assert_identical(
        identity,
        {key: value.get(key) for key in identity},
        path=f"恢复凭证/{path.name}",
    )
    return value
...
def _check_failure_latch(job: dict, spec: EpisodeSpec) -> None:
    ...
    if path.exists():
        ...
        raise ReproducibilityError(
            f"该冻结场景已有锁存失败，禁止重新执行：{failure['error_type']}：{failure['error']}"
        )
...
def _latch_failure(job: dict, spec: EpisodeSpec, error: BaseException) -> None:
    ...
    if path.exists():
        ...
        _exclusive_json(
            path, {**identity, "error_type": type(error).__name__, "error": str(error)}
        )


def _repeat_record(
    job: dict, spec: EpisodeSpec, repeat: int, old_record: Any
) -> tuple[Path, Any]:
    ...
    if published_report.exists():
        published_paths = {
            Path(path) for path in _read_json(published_report)["record_paths"]
        }
        if any(
            not path.is_file()
    ...
    for attempt in attempts:
        ...
        assert_identical(
            {"seed": spec.seed, "spec_hash": spec.spec_hash}, identity, path="重试身份"
        )
        ...
        if failure_path.exists():
            ...
            assert_identical(
                identity,
                {key: failure.get(key) for key in identity},
                path="基础设施失败身份",
            )
        ...
        if complete or completion_path.exists() or record_path in published_paths:
            ...
            _exclusive_json(
                interrupted_path,
                {**_recovery_identity(job, spec), "reason": interrupted_reason},
            )
    ...
    next_index = [
        max((int(path.name.removeprefix("attempt_")) for path in attempts), default=-1)
        + 1
    ]
    ...
    def run(candidate: EpisodeSpec):
        ...
        try:
            with job["gpu_limit"]:
                ...
                result = run_episode_process(
                    candidate,
                    record_path,
                    render_gpu=job["render_gpu"],
            ...
            if isinstance(exc, FileExistsError):
                raise
            _exclusive_json(
                attempt / "infrastructure_error.json",
                {
                    **identity,
        ...
        _validate_record(
            record, candidate, fingerprint=job["fingerprint"], path=record_path
        )
        _assert_cross_version(old_record, record)
...
def _recertify_job(job: dict) -> tuple[dict, dict]:
    ...
    try:
        ...
        assert_identical(
            job["fingerprint"],
            runtime_fingerprint(render_gpu=job["render_gpu"]),
        ...
        for path, record in zip(paths, records):
            _validate_record(
                record,
                spec,
                fingerprint=old["runtime_fingerprint"],
        ...
        initial = next(
            frame["info"]
            for frame in results[0][1].frames
            if frame["info"].get("operation") == "reset_complete"
        )
        geometry = initial["geometry_report"]
        if geometry.get("ok") is not True:
            raise ReproducibilityError("原版真实初态几何未通过")
        from robomme_icl.io.scene_metadata import scene_metadata
        from robomme_icl.io.hdf5 import tree_hash
        ...
        report = {
            ...
            "content_hash": results[0][1].content_hash,
            "frame_count": len(results[0][1].frames),
            "geometry": geometry,
            "runtime_fingerprint": job["fingerprint"],
        ...
        print(
            f"已重新认证 {spec.task_kind} seed={spec.seed} GPU={job['render_gpu']}：两次新进程与旧帧逐位相同",
            flush=True,
        )
        return spec.to_dict(), report
    except BaseException as exc:
        explicit_failure = isinstance(
            exc, (CandidateRejected, SceneRejected, SharedReproducibilityError)
        )
...
def recertify_suite(
    ...
    *,
    ...
    for spec in specs:
        ...
        if (
            old.get("fresh_process") is not True
            or not old.get("content_hash")
            or not old.get("source_commit")
        ):
    ...
    _bind_context(
        output / "recertify_state.json",
        {
            "schema_version": 2,
            "source_suite": str(source),
            "source_suite_hash": suite["suite_hash"],
            "episodes": [spec.to_dict() for spec in specs],
            "runtime_fingerprint": fingerprint,
            "gpu_fingerprints": {
                str(gpu): value for gpu, value in gpu_fingerprints.items()
            },
        },
    )
    unknown = {path.name for path in output.iterdir()} - {
        "recertify_state.json",
        "certification",
        "suite.json",
    }
    if unknown:
    ...
    if certificate_root.exists():
        ...
        for task_dir in certificate_root.iterdir():
            ...
            expected_seeds = {
                f"seed_{spec.seed}" for spec in specs if spec.task_kind == task_dir.name
            }
    ...
    if manifest.exists():
        ...
        assert_identical(
            [spec.to_dict() for spec in specs], published["episodes"], path="恢复规格"
        )
    commit = source_commit()
    with new_manager() as manager:
        stop = manager.Event()
        limits = gpu_limits(manager, sorted(gpu_fingerprints), workers)
        jobs = []
        for spec in specs:
            old = suite["certification"][spec.spec_hash]
            gpu = old["render_gpu"]
            jobs.append(
                {
                    ...
                    "directory": str(
                        output / "certification" / spec.task_kind / f"seed_{spec.seed}"
                    ),
                    "fingerprint": gpu_fingerprints[gpu],
                    "render_gpu": gpu,
        ...
        results = run_jobs(_recertify_job, jobs, workers, stop)
    ...
    return save_suite(
        manifest,
        specs,
        (suite["configs"]["task"], suite["configs"]["position"]),
        certification,
    )
