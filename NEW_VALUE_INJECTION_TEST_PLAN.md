...
### 一、生成 100 条，两阶段验证后全部实跑

一句话方案：**在原难度和空间约束内均衡生成 11 组、每组 100 条固定规格，画图并静态检查全部 1100 条；先做冒烟、串行参考与双卡负载校准，再把 1100 条全部注入实跑，逐条给出通过／失败／碰撞的结果和视频状态。**
...
│  第一阶段 轻量验证：4 条冒烟 + 串行参考 16 条 × 2 遍（32 次）+ 双卡负载阶梯 160 条 × 3 档（480 次） │  约 1 小时
│    负载阶梯：双卡每卡 12 → 16 → 20 worker，每档同一份 160 条清单，按「成功且视频完整的条数 ÷ 墙钟」选档   │
│  第二阶段 全部实跑：1100 条逐条注入执行 ──▶ 每条一个结果 + 视频状态 + 跑后三类图 + 失败位置图        │  耗时以校准吞吐为准
└──────────────────────────────────────────────────────────────────────────────────────┘
...
| 任务 | 生成固定规格 | 第二阶段实跑 |
|---|---:|---:|
| `BinFill` | easy、medium、hard 各 100 条 | 每档 100 条 |
| `RouteStick` | easy、medium、hard 各 100 条 | 每档 100 条 |
| `VideoUnmaskSwap` | easy、medium、hard 各 100 条 | 每档 100 条 |
| `VideoRepick` | easy、medium 各 100 条 | 每档 100 条 |
| **合计** | **1100 条** | **1100 条** |

**`VideoRepick hard` 放弃：本轮不生成规格、不画图、不实跑。**

第一阶段只动 4 条冒烟、16 条固定校准样本和 160 条负载清单（都取自 1100 条之内），目的是证明注入链路与双卡高并发可信；第二阶段把 1100 条全部跑完，每组给出七类任务结果的计数、执行状态与视频状态，碰撞拒绝再分接触／穿入、数值边界、无法证明安全三项，并列出每条失败发生在哪个环节。碰撞初筛与运行时碰撞检查都覆盖视频任务的 5 组、500 条。固定诊断案例另册保留，不占用上述分母。

#### 1.1 已定死的口径

下面每条都在后面的某一节展开，读者按节号跳读即可：

1. 只覆盖四任务的 11 个难度组；`VideoRepick hard` 完全排除。1100 条全部实跑，物理可行性以实跑结果为准，不以静态检查代替（本节、第五节）。
...
6. 验收分五问单独回答：规格是否均匀、碰撞是否全部排除、注入是否生效、1100 条各自能否执行、并行是否与串行逐位一致；最终逐项列出，不用一个总成功率替代（第五节）。
7. 本轮实跑采用双 GPU 对拍，直接测每卡 12／16／20 worker 三档：先用单卡单 worker 跑两遍 16 条固定样本建立串行参考，再每档用同一份 160 条清单一次调用混跑，16 条固定样本逐位对串行参考、其余档间互比；按「成功且视频完整的条数 ÷ 墙钟」选档并另报最大稳定档；12 档不通过则向下补测 8、4 两档，仍无合格双卡档就报告双 GPU 要求未满足并停止，不自动改单卡全量（第六节）。
8. 用户已目视确认的四容器三例持久保留；重跑使用新目录，分别复核轨迹重算和视频重渲染，不能把旧视频或单姿态交叉验证当作新值执行通过（第二部分第 8.4 节及第二部分第四、七节）。
9. 1100 条每条都登记视频状态：录像器 `RobommeRecordWrapper` 冻结，不改源文件、不子类覆写、不打补丁；视频就是它现有逻辑的产出（主相机、腕部相机、原始分割、目标分割、目标标记，上下两排规划与在线判断，任务文字；`NO RECORD` 阶段按设计跳过、不补 reset 帧；成功名／`FAILED_` 前缀，落 `videos/`）。生成入口在 `close()` 后按命名规则找文件、用 ffprobe 数帧、算散列，成功局要求帧数等于 HDF5 的 timestep 数；视频缺失或帧数不符不改变任务结果，但单列视频判定失败，样本不移出分母（第 5.9 节）。
...
中途曾选定「每卡 12／16／20 三档、每批每任务 40 条、固定样本逐位比、其余档间互比」；上述原话决定了本计划第 5.9 节与第六节的口径：录像器不改、不覆盖，`NO RECORD` 跳过是设计而非缺陷。
...
每组保存**跑前、跑后两套图**，版式相同：跑前按冻结的规格画，用来核对分配是否均匀；跑后按第二阶段实跑结果画，用来看失败和碰撞落在哪里。草图中的 `n`、`θ=1.23`、`+3.1 mm` 等数字均为占位。
...
读法：每条一个俯视小图，标出对象编号、目标和动作。跑前附初态／连续路径检查状态与最危险对象对；跑后在同一小图上标出结果类别、失败环节、对象对与判定值。不能只画中心点掩盖旋转边界。
...
跑后另附一张**每组结果汇总表**：七类结果计数（合计 100）、碰撞拒绝的三个分项、每条失败样本的 episode、失败环节（初始化／第几次交换／子步／规划／超时）、对象对与判定值。验收以计数表为依据，图用于直观复核。
...
- **录像器冻结**：`RobommeRecordWrapper` 不改源文件、不子类覆写、不打补丁；生成入口 `_worker` 仍按现状构造它并传 `save_video=True`。视频核验只在 `close()` 之后由生成入口完成（第 5.9 节）。
...
**第一阶段 轻量验证（约 1 小时）**：证明注入链路和双卡高并发可信；只动 1100 条之内的 4 条冒烟、16 条固定校准样本与 160 条负载清单。

| 步骤 | 内容 | 判定 |
|---|---|---|
| 1a | 代码短测（≤ 5 分钟） | 定向 pytest 通过 |
| 1b | 关闭注入时与修改前逐位对照 | `DEFAULT_PARITY` |
| 1c | 每任务 1 条冒烟，共 4 条，单卡单 worker；带观察器开关对照；核对视频文件存在、`ffprobe` 帧数等于 HDF5 timestep 数 | `SMOKE` |
| 1d | 串行参考：16 条固定校准样本（四任务各前 4 条，前三任务 hard、`VideoRepick` medium）单卡单 worker 跑两遍 `S0a`、`S0b`，共 32 次；两遍完整 HDF5 逐位一致才成为参考 | `SERIAL_REFERENCE`、`INJECTION_BINDING`（16 条） |
| 1e | 双卡负载阶梯：每卡 12、16、20 worker 三档（`--gpus 0,1 --workers 24／32／40`），每档同一份 160 条清单（四个校准组各 episode 0～39，含上面 16 条），一次调用、一套双卡池、四任务混跑，共 480 次；16 条固定样本逐位对 `S0a`，其余 144 条相邻档互比；12 档不通过时向下补测 8、4 两档 | `PARALLEL_CONTENT`、`PARALLEL_OVERLAP`、`PARALLEL_SCALE`、`VIDEO_INDEX`、`VIDEO_DECODE`（校准范围） |

**第二阶段 全部实跑**：1100 条逐条注入执行，用 `PARALLEL_SCALE` 选出的双卡档；没有合格双卡档就停止并报告双 GPU 要求未满足，不自动改单卡全量。每条样本走下面的路径，任一环节失败就记为对应类别并保留失败位置：

```text
一条实跑样本（任务/难度/episode k，k∈0～99）
...
| 检查 | 静态（1100 条，跑前） | 第一阶段（冒烟 4 + 固定 16 = 20 条；负载清单 160 条） | 第二阶段（1100 条） |
|---|---|---|---|
| 难度、数量、索引、动作合法性 | 检查 | — | — |
| 均匀配额、位置范围、几何静态初筛 | 检查 | — | — |
| 视频对象初态与全部预定交换的连续几何 | 500 条检查 | 视频任务的冒烟与校准样本 | 500 条从实际起态重新检查 |
| 创建出的对象、位姿与规格一致 | — | 20 条 | 1100 条 |
| 两次初始化及实际动作绑定一致 | — | 20 条 | 1100 条 |
| 视频对象完整位姿更新及物理子步前后无碰撞 | — | 视频任务样本 | 500 条，接触／数值边界／无法证明安全分项记录 |
| 抓取对象、交换双方、路线和方向 | — | 按实际执行事件 | 按实际执行事件 |
| 最终成功及 HDF5 契约 | — | 20 条 | 1100 条 |
| 视频文件存在、帧数等于 HDF5 timestep 数、可解码 | — | 冒烟 4 条 + 校准每档 160 条 | 1100 条，成功与 `FAILED_` 都查 |
| 多卡与单卡逐位一致、真实并发 | — | 16 条 × 3 档对串行参考，144 条 × 相邻档互比 | — |
...
| 视频状态 | `complete`／`frame_mismatch`／`missing`／`no_close`（进程未到 `close()`） | 第 5.9 节核验 |

每组任务结果计数合计 100，全部合计 1100；"全部有结果"（执行状态全部登记）与"全部成功"（任务结果为通过）分别报告，不用一个总成功率替代。没有 HDF5 的失败也保留规格、日志和失败位置。产物按运行编号／任务／难度分目录，规格与结果用散列关联，历史证据只读。
...
| `COVERAGE_QUOTA` | 均匀 | 重算全量与每批配额、分层计数、条件频数和缺口 | `COVERAGE_QUOTA=PASS groups=11 batches=10 quota_gaps=0` |
...
| `COLLISION_RUNTIME` | 碰撞 | 500 个视频样本的两次初始化、每段实际起态、完整位姿更新、子步前后与 `separation`；拒绝数是结果不是门槛 | `COLLISION_RUNTIME=PASS unique=500 checked=500 missing_checks=0 rejected=<实际数>` |
...
| `PLOT_EVIDENCE` | 均匀 | 每组跑前、跑后各一套（100 小图 + 三类图 + 失败位置图），图与计数表的规格散列一致 | `PLOT_EVIDENCE=PASS groups=11 before=11 after=11 thumbnails=1100` |
| `DEFAULT_PARITY` | 注入生效 | 修改前后关闭注入、同 seed／配置，原调用流、对象动作、完整 HDF5 逐位相同 | `DEFAULT_PARITY=PASS cases=<实际数> differences=0` |
| `SMOKE` | 注入生效 | 每任务 1 条、单 GPU、单 worker：注入、终态、HDF5、视频；观察器开关对照 | `SMOKE=PASS episodes=4 attempt=0 observer_differences=0` |
| `INJECTION_BINDING` | 注入生效 | 创建前输入 vs 创建后位姿、两次初始化、所有目标／方向／交换事件 | 第一阶段 `INJECTION_BINDING=PASS unique=16 mismatches=0`；第二阶段 `INJECTION_BINDING=PASS unique=1100 mismatches=0` |
| `SERIAL_REFERENCE` | 并行一致 | 16 条固定校准样本单卡单 worker 跑两遍，`compare_h5` 全字段逐位一致；两遍同失败的条记 `both_failed` 并从可比数中扣除 | `SERIAL_REFERENCE=PASS unique=16 comparable=<n> both_failed=<n> differences=0` |
| `PARALLEL_CONTENT` | 并行一致 | 每档 160 条里的 16 条固定样本各与 `S0a` 比较完整 HDF5 与对象动作证据；其余 144 条相邻档两两互比，只作附加证据 | `PARALLEL_CONTENT=PASS unique=16 configs=<实跑档数> pairs=<档数×可比数> differences=0 cross_tier_pairs=<n> cross_tier_differences=0` |
| `PARALLEL_OVERLAP` | 并行一致 | 每档按 start／end 事件扫描 step 时间线：不同 PID 峰值 ≥ 总 worker 数、每卡峰值 ≥ 每卡 worker 数、双卡共同窗口 > 0；只有排队／导入重叠不算 | `PARALLEL_OVERLAP=PASS mode=P01x<n> gpus=2 workers_per_gpu=<n> peak_distinct_pids=<2n>`，每档一行 |
| `PARALLEL_SCALE` | 并行一致 | 每档吞吐 = 成功且视频完整的条数 ÷ 批次墙钟（含生成、编码、关闭、落盘），同时报失败数；守卫：本任务进程组 RSS 峰值 < 300 GB、`free` 可用 ≥ 40 GB、swap 不增长、无 OOM／超时、磁盘余量足够；两个最快候选各复测一次，取吞吐最高且复测稳定的档，另报最大稳定档 | `PARALLEL_SCALE=PASS chosen=P01x<n> workers_per_gpu=<n> delivered_per_min=<值> failed=<n> peak_rss_gb=<值> max_stable=P01x<n> highest_failed=<档或 none>` |
| `VIDEO_INDEX` | 可执行 | 1100 行每行有视频状态，`missing`／`no_close` 必须带原因（无文件、进程未到 `close()`、编码异常日志）；成功与 `FAILED_` 都查 | `VIDEO_INDEX=PASS rows=1100 complete=<n> frame_mismatch=<n> missing=<n> no_close=<n> untraceable=0` |
| `VIDEO_DECODE` | 可执行 | 成功局 `ffprobe` 读出的帧数等于 HDF5 timestep 数；`FAILED_` 视频帧数 > 0 且可解码到最后一帧；散列留档 | `VIDEO_DECODE=PASS success_rows=<n> decoded_eq_timesteps=<n> failed_videos=<n> mismatches=0` |
| `FEASIBILITY` | 可执行 | 1100 条全部登记执行状态、任务结果、视频状态；成功条用 `inspect_episode_terminal` 契约核验；成功数是报告结果，不是门槛 | `FEASIBILITY=PASS unique=1100 executed=1100 unclassified=0 succeeded=<实际数> attempt=0`，另一行 `RESULT_COVERAGE all_recorded=1100 all_success=<实际数>` |
| `DELIVERY` | 可执行 | 离线重算清单散列；所有规格、1100 行三字段结果、每组七类计数与碰撞三分项、视频状态计数、命令、退出码、固定案例保留清单 | `DELIVERY=PASS specs=1100 result_rows=1100 missing=0` |
...
| 阶段 | 步骤 | 内容 | 判据 |
| --- | --- | --- | --- |
| 0 | — | 核对授权、代码／配置／环境；冻结实现基线 | 第二部分前置红线满足，记录未验证项 |
| 一 轻量验证 | 1.1 | 实现规格生成、真实碰撞盒与连续路径检查、固定案例复现、跑前三类图 | `COLLISION_GEOMETRY`、`COLLISION_REPRODUCE`、`SPEC_SCOPE`、`SPEC_REPRODUCIBLE`、`COVERAGE_QUOTA`、`STATIC_GEOMETRY`、`COLLISION_SWEEP`、`PLOT_EVIDENCE`（跑前） |
| | 1.2 | 接入规格、碰撞拒绝、运行时子步检查与事件证据；关闭态对照；每任务 1 条冒烟 | `DEFAULT_PARITY`、`SMOKE`；短验证累计 ≤ 5 分钟，失败不放大 |
| | 1.3 | 串行参考：16 条固定样本 × `S0a`、`S0b` = 32 次 | `SERIAL_REFERENCE`、`INJECTION_BINDING`（16）；两遍不一致即停，不换规格 |
| | 1.4 | 双卡负载阶梯：160 条清单 × 每卡 12／16／20 三档 = 480 次（12 档失败时向下补测 8、4）；两个最快候选各复测一次 | `PARALLEL_CONTENT`、`PARALLEL_OVERLAP`、`PARALLEL_SCALE`、`VIDEO_INDEX`／`VIDEO_DECODE`（校准范围）；没有合格双卡档则停止并报告 |
| 二 全部实跑 | 2.1 | 1100 条全部执行（用 `PARALLEL_SCALE` 选出的双卡档）；每组七类计数、碰撞三分项、逐条失败环节、视频状态 | `INJECTION_BINDING`（1100）、`COLLISION_RUNTIME`、`FEASIBILITY`、`VIDEO_INDEX`、`VIDEO_DECODE` |
| | 2.2 | 跑后三类图与失败位置图；轻量证据、独立离线复核、更新说明与账本并提交 | `PLOT_EVIDENCE`（跑后）、`DELIVERY` |

执行次数预算：冒烟 4 + 串行参考 32 + 负载阶梯 480（补测与复测另计，最多再加 480）+ 全量 1100 = 1616～2096 次。⚠ 耗时与容量按原值实测折算只作参考：`S0` 约 47 秒一条，32 次约 25 分钟；三档各 160 条若线性约 3～5 分钟一档，含编码拖尾按翻倍估约 25 分钟；第一阶段约 1 小时。第二阶段 1100 条在 40 worker 下若线性约 20 分钟，CPU 超订后按 30～60 分钟估，实际以校准吞吐为准。单条 HDF5 0.32～0.72 GB、视频 8～23 MB，校准约 0.26 TB、全量约 0.55 TB，合计约 0.8 TB，`/data` 剩余 2.6 TB；每档开跑前核一次余量。超过 5 分钟的步骤用 detached `tmux`。

#### 5.7 实施后实测追加区

**正式新值两阶段尚未执行。** 定稿前碰撞诊断（编号 `20260909-bin-contact-v2`）已发生的结果如下，不回写目标判据以适配结果：
...
**原值校准检查已完成，完整 16 条资格未满足，工具结论整体未通过；正式新值两阶段仍未执行。**
...
判定行 `VIDEO_INDEX`、`VIDEO_DECODE` 见第 5.5 节；冒烟与校准阶段先在小范围跑通，第二阶段对 1100 条全部登记。视频文件存在不等于已人工观看；有目视时记录查看对象与结论。

### 六、GPU 与进程：双 GPU 正确性校准与持续负载选档

**本轮采用双 GPU 对拍，直接测每卡 12／16／20 worker 三档，每档同一份 160 条清单，一次调用、一套双卡池、四任务混跑。** 运行配置：`S0a`／`S0b`（GPU 0／1 worker，串行参考两遍）、`P01x12`／`P01x16`／`P01x20`（GPU 0、1／每卡 12、16、20，`--gpus 0,1 --workers 24／32／40`）；12 档不通过时向下补测 `P01x8`、`P01x4`。资源上限（本机实测）：主机内存 377 GB（可用约 328 GB，swap 已满）、32 逻辑核、两张 46 GB 显卡、`/data` 剩余 2.6 TB。原值 `P01` 单 worker 峰值 RSS 3.5～5.8 GB（含内存里的视频帧），40 worker 正常约 240 GB，长失败样本可达 12 GB 一个；每 worker 一个 Python 线程加一个 x264 线程，32 核在每卡 16 以上必然超订，吞吐会在某档平台化，只能实测。生成器每 GPU 一个 `spawn` 进程池，`per_gpu = max(1, workers // len(gpu_ids))`（`generate_dataset_newseed.py::_run_jobs`），`--workers` 是总数；x264 线程由 `--affinity per-gpu` 的 CPU 亲和压住。
...
1. **冒烟（`S0`）**：每任务 1 条、共 4 条，单 worker，`SMOKE` 通过（含视频帧数等于 timestep 数）后才进入下一步。
2. **串行参考**：16 条固定校准样本（四任务各前 4 条，前三任务 hard、`VideoRepick` medium）单卡单 worker 跑 `S0a`、`S0b` 两遍，共 32 次；`SERIAL_REFERENCE` 两遍逐位一致才有参考资格，同失败的条记 `both_failed` 扣出可比数。
3. **负载阶梯**：160 条清单（四个校准组各 episode 0～39，含 16 条固定样本）依次跑 `P01x12 → P01x16 → P01x20`，每档一次调用、`--gpus 0,1`、清单混跑；档内 16 条固定样本对 `S0a` 逐位，144 条与相邻档互比。守卫：进程组 RSS 峰值 < 300 GB、`free` 可用 ≥ 40 GB、swap 不增长、无 OOM／超时、磁盘余量足够；跌破即中止该档、保留已完成档。12 档不通过则向下补测 `P01x8`、`P01x4`。
4. **判定**：`PARALLEL_CONTENT`（固定样本逐位 + 档间互比）+ `PARALLEL_OVERLAP`（同一主机单调时钟，按 start／end 事件扫描 `[first_step, last_step]` 窗口：不同 PID 峰值 ≥ 总 worker 数、每卡峰值 ≥ 每卡 worker 数、跨卡共同窗口 > 0；只有排队／导入重叠不算）。
5. **选档（`PARALLEL_SCALE`）**：每档算"成功且视频完整的条数 ÷ 批次墙钟"，同时报失败数，避免把快速失败误当加速；两个最快候选各复测一次，取吞吐最高且复测稳定的档，另列最大稳定档。
6. **第二阶段 1100 条全部实跑**：用选出的双卡档（40 worker 若线性约 20 分钟，CPU 超订后按 30～60 分钟估，以校准吞吐为准）。没有任何双卡档通过时停止并报告双 GPU 要求未满足，并行三项保持 `FAIL`，不自动改单卡全量、不换规格、不放宽容差。
7. **留档**：保留每档逐条结果、墙钟、吞吐、显存与内存峰值、并发峰值、完整差异与视频状态计数；超过 5 分钟的阶段用 detached `tmux`。
...
2. **范围：**第一部分第一节的 11 组是唯一规格集合；1100 条全部是第二阶段实跑样本，第一阶段只取其中 4 条冒烟、16 条固定校准样本与 160 条负载清单；`VideoRepick hard` 不生成、不画图、不实跑。
...
| 串行参考与三档负载阶梯 | 16 条固定样本 `S0a` 对 `S0b`；每档 160 条中的固定样本对 `S0a`，其余相邻档互比 | 少产物、不同 timestep、数值／图像差异、规划回退、只有排队重叠、每卡 PID 峰值不足、OOM 被记成通过、把快速失败算成高吞吐、跳档 | `SERIAL_REFERENCE`、`PARALLEL_CONTENT`、`PARALLEL_OVERLAP`、`PARALLEL_SCALE` |
...
| 最终样本与交付 | 1100 个唯一规格、每次执行记录、每组七类计数与碰撞三分项、文件清单 | 无 HDF5 的失败被丢弃、未运行被记成功、仅保存通过标记、失败环节缺失 | `FEASIBILITY`、`DELIVERY` |

七类最终状态互斥：通过、规格拒绝、碰撞拒绝、实际对象／动作不符、规划失败、超时、未运行；对应计数之和为 1100，每组 100。碰撞的几何确认、数值边界与无法证明安全分项统计，不能把 `uncertified` 当作已确认穿入。基础设施或代码错误另有 `error_type`／`failure_class`／阶段明细，阻塞受影响样本并计入未完成项，不冒充物理不可行。串行参考与各档负载校准的重复执行按档单独列逐条结果；唯一规格的可行性结论与跨配置一致性结论分开，不能用重复成功次数扩大分子。执行状态、任务结果、视频状态三字段分别计数，系统错误计入执行状态而不冒充物理不可行。冻结前候选拒绝数与固定的 9 个诊断案例另册统计，不进入 1100 行分母。
...
拟新增最小冒烟入口如下。选择固定 `BinFill hard` episode 0，属于 1100 条之内；第一阶段冒烟每任务 1 条、共 4 条，其余三条命令同型只换任务与难度；观察器对照使用另一个子目录，不能覆盖首条产物。冒烟失败停止矩阵，保留该规格：
...
拟新增 `run --phase calibration`：先跑 `S0a`、`S0b`（`--gpus 0 --workers 1`，16 条固定样本，按任务／难度拆目录），再按 `P01x12 → P01x16 → P01x20`（`--gpus 0,1 --workers 24／32／40`）各一次调用、传 160 条清单混跑，每档独立输出根；12 档失败向下补测 8、4；每档采样进程组 RSS、`free`、swap、显存与磁盘，守卫跌破即中止该档。`run --phase feasibility` 读取校准报告中 `PARALLEL_SCALE` 选出的档，一次调用传 11 组清单执行全部 1100 条，每组独立输出根，逐条写执行状态、任务结果、失败环节与视频状态；没有合格双卡档时拒绝启动。`compare` 独立检查原始数据；`report` 无论此前通过与否都保存完整状态，不得因某条失败遗失剩余样本。
...
| 视频帧攒在内存导致高档 OOM | 每档采样进程组 RSS、`free` 可用、swap 增量；单 worker 峰值按 3.5～5.8 GB、长失败样本 12 GB 估 | 守卫跌破即中止该档并退回已通过档；不改录像器，不放宽守卫 |
| 32 核被 40 个 worker 加 x264 线程超订 | 每档吞吐曲线与 CPU 占用采样 | 吞吐平台化即视为最大稳定档；`--affinity per-gpu` 保持 |
...
- 第一阶段只对 20 条做注入验证、160 条做负载校准；1100 条的物理可行结论以第二阶段实跑为准，跑前的静态与连续几何通过不代表抓取、运行时最近邻或实际物理轨迹可执行。
...
- 串行参考只覆盖 16 条固定样本；负载清单其余 144 条的档间互比只证明三档之间一致，不证明与串行一致。三档之外的 worker 数没有测过。
...
轻量包拟放 `docs/validation/newtask-v2/<运行编号>/`：中文 `README.md`、11 份规格及规范化散列、全量／每批计数表、静态拒绝清单、跑前与跑后两套图、1100 行唯一样本结果（执行状态、任务结果、视频状态与失败环节）、每组七类计数与碰撞三分项、视频状态计数、4 次冒烟、32 次串行参考与每档负载校准记录（含吞吐、内存峰值、并发峰值）、命令与退出码、HDF5 全字段聚合指纹、动作绑定摘要、资源／时间区间、环境与源码指纹、完整清单散列。轻量不表示只留成功条目；没有 HDF5 的失败也必须能定位到规格与错误阶段。
