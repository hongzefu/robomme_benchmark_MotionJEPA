...
> **权威与状态：**本文件按 [AGENTS.md](AGENTS.md) 强制规则第 10 条组织，承接 `cfca77d`（`10.17`）保存的已确认范围及 `f11548c`（`10.19`）的两部分结构。容器碰撞机制与固定案例已于 `dc0b03b`（`10.20`）补入；第一部分已按用户指定顺序重排并内置草图，本次改为逐字段精确约定并压缩第三、五节；本轮实跑采用多 GPU 对拍，依据第 5.8 节原值并行校准 `20260909-schema3-parallel-v2` 的 15/16 逐位一致（第六节）。本次再按用户确认把第五节改为两阶段：第一阶段轻量验证（4 条冒烟 + 16 条四配置校准），第二阶段 1100 条全部实跑并给出失败／碰撞分类；图改为跑前、跑后两套。生产检测、新值注入接口、规格生成与一键复现均待实施。第二部分第 8.4 节和第 5.7 节单列定稿前已完成的诊断证据，不作为完整新值任务验收。每个实施阶段须单独获批；后续授权明确覆盖多个阶段时按该授权执行。
>
...
│  第一阶段 轻量验证：每任务 1 条冒烟（4 条）+ 16 条 × 4 种运行配置校准（64 次）  │  约 40 分钟
│  第二阶段 全部实跑：1100 条逐条注入执行 ──▶ 每条一个结果 + 跑后三类图 + 失败位置图 │  双卡约 5 小时
└──────────────────────────────────────────────────────────────────────────┘
...
7. 本轮实跑采用多 GPU 对拍：第一阶段先对 16 条新值样本做四种运行配置校准，通过后第二阶段 1100 条用双卡、每卡 2 worker；校准不通过则用单卡单 worker 完成并保留失败结论（第六节）。
...
| 1d | 16 条新值 × 4 种运行配置 = 64 次校准 | `PARALLEL_CONTENT`、`PARALLEL_OVERLAP`、`INJECTION_BINDING`（16 条） |
...
| | 1.3 | 16 条新值规格 × 四配置 = 64 次校准 | `PARALLEL_CONTENT`、`PARALLEL_OVERLAP`、`INJECTION_BINDING`（16）；失败按第六节串行回退，不换规格 |
| 二 全部实跑 | 2.1 | 1100 条全部执行（校准通过用 `P01`，否则 `S0`）；每组七类计数、碰撞三分项、逐条失败环节 | `INJECTION_BINDING`（1100）、`COLLISION_RUNTIME`、`FEASIBILITY` |
| | 2.2 | 跑后三类图与失败位置图；轻量证据、独立离线复核、更新说明与账本并提交 | `PLOT_EVIDENCE`（跑后）、`DELIVERY` |

执行次数预算：冒烟 4 + 校准 64 + 全量 1100 = 1168 次。⚠ 耗时按原值实测折算只作参考：`P01` 约 16 秒一条、1100 条约 5 小时；`S0` 约 47 秒一条、约 14.5 小时；第一阶段约 40 分钟。新布局的实际耗时以实跑为准。超过 5 分钟的步骤用 detached `tmux`。
...
4. **第二阶段 1100 条全部实跑**：校准通过用 `P01`（按原值实测约 5 小时）；不通过则用 `S0`（约 14.5 小时），并行两项保持 `FAIL`，不换规格、不放宽容差。
...
七类最终状态互斥：通过、规格拒绝、碰撞拒绝、实际对象／动作不符、规划失败、超时、未运行；对应计数之和为 1100，每组 100。碰撞的几何确认、数值边界与无法证明安全分项统计，不能把 `uncertified` 当作已确认穿入。基础设施或代码错误另有 `error_type`／`failure_class`／阶段明细，阻塞受影响样本并计入未完成项，不冒充物理不可行。64 次校准重复执行单独列逐轮结果；唯一规格的可行性结论与跨配置一致性结论分开，不能用重复成功次数扩大分子。冻结前候选拒绝数与固定的 9 个诊断案例另册统计，不进入 1100 行分母。
...
拟新增 `run --phase calibration` 固定 16 条和四种配置，按任务／难度／配置拆目录并调用原生产入口；`run --phase feasibility` 先读取校准报告，再以 `P01`（校准未通过则 `S0`）执行全部 1100 条，按任务／难度拆目录，逐条写结果类别与失败环节。`compare` 独立检查原始数据；`report` 无论此前通过与否都保存完整状态，不得因某条失败遗失剩余样本。
...
轻量包拟放 `docs/validation/newtask-v2/<运行编号>/`：中文 `README.md`、11 份规格及规范化散列、全量／每批计数表、静态拒绝清单、跑前与跑后两套图、1100 行唯一样本结果（含结果类别与失败环节）、每组七类计数与碰撞三分项、4 次冒烟与 64 次校准记录、命令与退出码、HDF5 全字段聚合指纹、动作绑定摘要、资源／时间区间、环境与源码指纹、完整清单散列。轻量不表示只留成功条目；没有 HDF5 的失败也必须能定位到规格与错误阶段。
