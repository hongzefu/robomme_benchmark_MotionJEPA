...
│  步骤 1～4 校准：短测 + 4 条冒烟 + 串行参考 16 条 × 2 遍（32 次）+ 双卡负载阶梯 120 条 × 3 档（360 次） │  约 50 分钟
│    负载阶梯：双卡每卡 12 → 16 → 20 worker，每档同一份 120 条清单，按「成功且视频完整的条数 ÷ 墙钟」选档   │
...
步骤 3 串行参考：16 条固定样本 × S0a、S0b 两遍 ─── SERIAL_REFERENCE ─── 两遍不一致：停
        │
步骤 4 双卡负载阶梯：120 条清单 × 每卡 12 → 16 → 20
        │   固定 16 条逐位对 S0a，其余 104 条相邻档互比；守卫跌破即停该档；12 不过补 8、4
        │   吞吐 = 成功且视频完整 ÷ 墙钟；两个最快档复测；选档 + 报最大稳定档
        │                                      PARALLEL_CONTENT / OVERLAP / SCALE
        │                                      没有合格双卡档：停止报告，不自动改单卡
        ▼
步骤 5 实跑 330 条（选出的档，一次调用 11 组清单，每组 episode 0～29）
...
- **做什么**：120 条清单（四个校准组各 episode 0～29，含步骤 3 的 16 条）依次在 `P01x12 → P01x16 → P01x20`（双卡每卡 12／16／20 worker，`--gpus 0,1 --workers 24／32／40`）下跑，每档一次调用、一套双卡池、四任务混跑，共 360 次。每档跑完：16 条固定样本对 `S0a` 逐位，其余 104 条与相邻档互比；按 start／end 事件扫描 step 时间线核并发峰值。守卫（数字见第六节）跌破即中止该档、保留已完成档；12 档不通过则向下补测 `P01x8`、`P01x4`。
- **选档**：每档算"成功且视频完整的条数 ÷ 批次墙钟"（含生成、编码、关闭、落盘），同时报失败数，避免把快速失败误当加速；两个最快候选各复测一次，取吞吐最高且复测稳定的档，另报最大稳定档。
- **判定**：`PARALLEL_CONTENT`、`PARALLEL_OVERLAP`、`PARALLEL_SCALE`，以及校准范围内的 `VIDEO_INDEX`、`VIDEO_DECODE`。
- **不过怎么办**：没有任何双卡档通过时停止并报告"双 GPU 要求未满足"，并行三项保持 `FAIL`；不自动改单卡全量、不换规格、不放宽容差。
...
| 4 | `PARALLEL_CONTENT` | 并行一致 | 每档 120 条里的 16 条固定样本各与 `S0a` 比较完整 HDF5 与对象动作证据；其余 104 条相邻档两两互比，只作附加证据 | `PARALLEL_CONTENT=PASS unique=16 configs=<实跑档数> pairs=<档数×可比数> differences=0 cross_tier_pairs=<n> cross_tier_differences=0` |
| 4 | `PARALLEL_OVERLAP` | 并行一致 | 每档按 start／end 事件扫描 step 时间线：不同 PID 峰值 ≥ 总 worker 数、每卡峰值 ≥ 每卡 worker 数、双卡共同窗口 > 0；只有排队／导入重叠不算 | `PARALLEL_OVERLAP=PASS mode=P01x<n> gpus=2 workers_per_gpu=<n> peak_distinct_pids=<2n>`，每档一行 |
| 4 | `PARALLEL_SCALE` | 并行一致 | 每档吞吐 = 成功且视频完整的条数 ÷ 批次墙钟（含生成、编码、关闭、落盘），同时报失败数；守卫：本任务进程组 RSS 峰值 < 300 GB、`free` 可用 ≥ 40 GB、swap 不增长、无 OOM／超时、磁盘余量足够；两个最快候选各复测一次，取吞吐最高且复测稳定的档，另报最大稳定档 | `PARALLEL_SCALE=PASS chosen=P01x<n> workers_per_gpu=<n> delivered_per_min=<值> failed=<n> peak_rss_gb=<值> max_stable=P01x<n> highest_failed=<档或 none>` |
...
执行次数预算：冒烟 4 + 串行参考 32 + 负载阶梯 360（补测与复测另计，最多再加 360）+ 实跑 330 = 726～1086 次。⚠ 耗时与容量按原值实测折算只作参考：`S0` 约 47 秒一条，32 次约 25 分钟；三档各 120 条若线性约 2～4 分钟一档，含编码拖尾按翻倍估约 20 分钟；步骤 1～4 约 50 分钟。步骤 5 的 330 条在 40 worker 下若线性约 6 分钟，CPU 超订后按 10～20 分钟估，实际以校准吞吐为准。单条 HDF5 0.32～0.72 GB、视频 8～23 MB，校准约 0.2 TB、实跑约 0.17 TB，合计约 0.4 TB，`/data` 剩余 2.6 TB；每档开跑前核一次余量。超过 5 分钟的步骤用 detached `tmux`。
...
| 配置 | `--gpus` | `--workers` | 每卡 worker | 清单 | 用途 |
|---|---|---:|---:|---|---|
| `S0a`、`S0b` | `0` | 1 | 1 | 16 条固定样本 | 步骤 3 串行参考，两遍 |
| `P01x12` | `0,1` | 24 | 12 | 120 条负载清单 | 步骤 4 第一档 |
| `P01x16` | `0,1` | 32 | 16 | 同上 | 步骤 4 第二档 |
| `P01x20` | `0,1` | 40 | 20 | 同上 | 步骤 4 第三档 |
| `P01x8`、`P01x4` | `0,1` | 16、8 | 8、4 | 同上 | 仅 12 档不通过时向下补测 |
| 全量 | `0,1` | 选出的档 | 选出的档 | 11 组清单各 episode 0～29 共 330 条 | 步骤 5 |

生成器每 GPU 一个 `spawn` 进程池，`per_gpu = max(1, workers // len(gpu_ids))`（`generate_dataset_newseed.py::_run_jobs`），`--workers` 是总数；全部配置固定 `--max-attempts 1 --max-tasks-per-child 8 --affinity per-gpu`，x264 线程由 CPU 亲和压住。
...
**守卫（任一跌破即中止当前档，保留已完成档）**

| 守卫 | 阈值 |
|---|---|
| 本任务进程组 RSS 峰值 | < 300 GB |
| `free` 可用内存 | ≥ 40 GB |
| swap | 不增长 |
| OOM／超时 | 0 |
| `/data` 余量 | 开跑前按 0.6 GB × 本档条数核一次 |
...
| 串行参考与三档负载阶梯 | 16 条固定样本 `S0a` 对 `S0b`；每档 120 条中的固定样本对 `S0a`，其余相邻档互比 | 少产物、不同 timestep、数值／图像差异、规划回退、只有排队重叠、每卡 PID 峰值不足、OOM 被记成通过、把快速失败算成高吞吐、跳档 | `SERIAL_REFERENCE`、`PARALLEL_CONTENT`、`PARALLEL_OVERLAP`、`PARALLEL_SCALE` |
...
拟新增 `run --phase calibration`：先跑 `S0a`、`S0b`（`--gpus 0 --workers 1`，16 条固定样本，按任务／难度拆目录），再按 `P01x12 → P01x16 → P01x20`（`--gpus 0,1 --workers 24／32／40`）各一次调用、传 120 条清单混跑，每档独立输出根；12 档失败向下补测 8、4；每档采样进程组 RSS、`free`、swap、显存与磁盘，守卫跌破即中止该档。`run --phase feasibility` 读取校准报告中 `PARALLEL_SCALE` 选出的档，一次调用传 11 组清单执行每组 episode 0～29 共 330 条，每组独立输出根，逐条写执行状态、任务结果、失败环节与视频状态；没有合格双卡档时拒绝启动。`compare` 独立检查原始数据；`report` 无论此前通过与否都保存完整状态，不得因某条失败遗失剩余样本。
...
- 串行参考只覆盖 16 条固定样本；负载清单其余 104 条的档间互比只证明三档之间一致，不证明与串行一致。三档之外的 worker 数没有测过。
