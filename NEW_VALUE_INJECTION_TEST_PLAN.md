...
> **权威与状态：**本文件按 [AGENTS.md](AGENTS.md) 强制规则第 10 条组织，承接 `cfca77d`（`10.17`）保存的已确认范围及 `f11548c`（`10.19`）的两部分结构。容器碰撞机制与固定案例已于 `dc0b03b`（`10.20`）补入；第一部分已按用户指定顺序重排并内置草图，本次改为逐字段精确约定并压缩第三、五节；本轮实跑采用多 GPU 对拍，依据第 5.8 节原值并行校准 `20260909-schema3-parallel-v2` 的 15/16 逐位一致（第六节）。本次再按用户确认把第五节改为两阶段：第一阶段轻量验证（4 条冒烟 + 16 条 × 8 种运行配置校准，每卡最多 10 worker），第二阶段 1100 条全部实跑并给出失败／碰撞分类；全量用的每卡 worker 数由校准实测确认；图改为跑前、跑后两套。生产检测、新值注入接口、规格生成与一键复现均待实施。第二部分第 8.4 节和第 5.7 节单列定稿前已完成的诊断证据，不作为完整新值任务验收。每个实施阶段须单独获批；后续授权明确覆盖多个阶段时按该授权执行。
>
...
一句话方案：**在原难度和空间约束内均衡生成 11 组、每组 100 条固定规格，画图并静态检查全部 1100 条；先用 20 条做轻量验证，再把 1100 条全部注入实跑，逐条给出通过／失败／碰撞的结果。**
...
│  第一阶段 轻量验证：每任务 1 条冒烟（4 条）+ 16 条 × 8 种运行配置校准（128 次）  │  约 1 小时
│    配置阶梯：单卡 1 → 单卡 2 → 双卡各 2、4、6、8、10 worker，选实测通过的最大档     │
│  第二阶段 全部实跑：1100 条逐条注入执行 ──▶ 每条一个结果 + 跑后三类图 + 失败位置图 │  双卡各 2 worker 约 5 小时，更多 worker 按实测
└──────────────────────────────────────────────────────────────────────────┘
...
第一阶段只跑 20 条，目的是证明注入链路与多卡并行可信；第二阶段把 1100 条全部跑完，每组给出七类结果的计数，碰撞拒绝再分接触／穿入、数值边界、无法证明安全三项，并列出每条失败发生在哪个环节。碰撞初筛与运行时碰撞检查都覆盖视频任务的 5 组、500 条。固定诊断案例另册保留，不占用上述分母。
...
7. 本轮实跑采用多 GPU 对拍，且每卡 worker 数不锁死：第一阶段对 16 条新值样本按阶梯校准（单卡 1、单卡 2、双卡各 2／4／6／8／10 worker），第二阶段 1100 条用实测通过的最大档；全部档都不通过则用单卡单 worker 完成并保留失败结论（第六节）。
...
| 连续位置与角度 | 分层采样；将 100 条组织成 10 个均衡小组，每批 10 条都分散覆盖各区间 |
...
**第一阶段 轻量验证（约 40 分钟）**：证明注入链路和多卡并行可信，只跑 20 条。

| 步骤 | 内容 | 判定 |
|---|---|---|
| 1a | 代码短测（≤ 5 分钟） | 定向 pytest 通过 |
| 1b | 关闭注入时与修改前逐位对照 | `DEFAULT_PARITY` |
| 1c | 每任务 1 条冒烟，共 4 条，单卡单 worker；带观察器开关对照 | `SMOKE` |
| 1d | 16 条新值 × 8 种运行配置 = 128 次校准：`S0`、`S1`、`P0`、`P01`（每卡 2）、`P01x4`、`P01x6`、`P01x8`、`P01x10`（每卡 4／6／8／10，双卡共 8／12／16／20 worker）；按阶梯从低到高跑，某档不通过就停在该档 | `PARALLEL_CONTENT`、`PARALLEL_OVERLAP`、`PARALLEL_SCALE`、`INJECTION_BINDING`（16 条） |

**第二阶段 全部实跑**：1100 条逐条注入执行，校准通过用双卡各 2 worker，否则单卡单 worker。每条样本走下面的路径，任一环节失败就记为对应类别并保留失败位置：
...
| 检查 | 静态（1100 条，跑前） | 第一阶段（20 条） | 第二阶段（1100 条） |
|---|---|---|---|
| 难度、数量、索引、动作合法性 | 检查 | — | — |
| 均匀配额、位置范围、几何静态初筛 | 检查 | — | — |
...
| 最终成功及 HDF5 契约 | — | 20 条 | 1100 条，保存视频 |
| 多卡与单卡逐位一致、真实并发 | — | 16 条 × 4 配置 | — |

正式实跑固定 `attempt=0`、`--max-attempts 1`；失败保留原条目，不换 seed、不补位。七类最终状态互斥：通过、规格拒绝、碰撞拒绝（再分接触／穿入、数值边界、无法证明安全）、实际对象／动作不符、规划失败、超时、未运行；每组计数合计 100，全部合计 1100。没有 HDF5 的失败也保留规格、日志和失败位置。产物按运行编号／任务／难度分目录，规格与结果用散列关联，历史证据只读。
...
| `PARALLEL_CONTENT` | 并行一致 | `S1`、`P0`、`P01`、`P01x4`～`P01x10` 每档各与同规格 `S0` 比较完整 HDF5 与对象动作证据，每档 16 对，最多 112 对 | `PARALLEL_CONTENT=PASS unique=16 configs=<实跑档数> pairs=<档数×16> differences=0` |
| `PARALLEL_OVERLAP` | 并行一致 | 每档核对卡身份、不同 PID、真实 step 时间区间重叠 > 0；双卡需跨卡共同窗口；每卡 n 个 worker 要看到 n 个不同 PID 同时在 step | `PARALLEL_OVERLAP=PASS mode=<档> gpus=2 workers_per_gpu=<n>`，每档一行 |
| `PARALLEL_SCALE` | 并行一致 | 选第二阶段配置：取通过 `PARALLEL_CONTENT`、`PARALLEL_OVERLAP`、无 OOM／超时、主机内存峰值低于 300 GB、且墙钟不比上一档差的最大档；记录每档墙钟、显存与内存峰值 | `PARALLEL_SCALE=PASS chosen=<档> workers_per_gpu=<n> wall_s=<秒> peak_rss_gb=<值> highest_failed=<档或 none>` |
| `FEASIBILITY` | 可执行 | 1100 条全部执行并落入七类之一；成功条用 `inspect_episode_terminal` 契约与视频核验；成功数是报告结果，不是门槛 | `FEASIBILITY=PASS unique=1100 executed=1100 unclassified=0 succeeded=<实际数> attempt=0` |
| `DELIVERY` | 可执行 | 离线重算清单散列；所有规格、1100 行结果、每组七类计数与碰撞三分项、命令、退出码、固定案例保留清单 | `DELIVERY=PASS specs=1100 result_rows=1100 missing=0` |
...
| | 1.3 | 16 条新值规格 × 8 档配置 = 128 次校准，阶梯从低到高，某档失败即停 | `PARALLEL_CONTENT`、`PARALLEL_OVERLAP`、`PARALLEL_SCALE`、`INJECTION_BINDING`（16）；全部档失败按第六节串行回退，不换规格 |
| 二 全部实跑 | 2.1 | 1100 条全部执行（用 `PARALLEL_SCALE` 选出的档，全部档失败则 `S0`）；每组七类计数、碰撞三分项、逐条失败环节 | `INJECTION_BINDING`（1100）、`COLLISION_RUNTIME`、`FEASIBILITY` |
| | 2.2 | 跑后三类图与失败位置图；轻量证据、独立离线复核、更新说明与账本并提交 | `PLOT_EVIDENCE`（跑后）、`DELIVERY` |

执行次数预算：冒烟 4 + 校准最多 128 + 全量 1100 = 最多 1232 次。⚠ 耗时按原值实测折算只作参考：`P01`（每卡 2）约 16 秒一条、1100 条约 5 小时；每卡 10 worker 若线性加速约 1 小时，实际以校准墙钟为准；`S0` 约 47 秒一条、约 14.5 小时；第一阶段约 1 小时。新布局的实际耗时以实跑为准。超过 5 分钟的步骤用 detached `tmux`。
...
**本轮采用多 GPU 对拍，每卡 worker 数由实测确认，不锁死在 2。** 八档运行配置：`S0`（GPU 0／1 worker）、`S1`（GPU 1／1 worker）、`P0`（GPU 0／2 workers）、`P01`（GPU 0、1／每卡 2）、`P01x4`／`P01x6`／`P01x8`／`P01x10`（GPU 0、1／每卡 4、6、8、10，共 8、12、16、20 workers）。资源上限（本机实测）：主机内存 377 GB、32 核、两张 46 GB 显卡；原值 `P01` 每 worker 约 5.3 GB 内存、显存约 0.4 GB，20 个 worker 预计约 107 GB 内存，历史 No-Patch 全量曾用单卡 20 workers。生成器每 GPU 一个 `spawn` 进程池，`per_gpu = max(1, workers // len(gpu_ids))`（`generate_dataset_newseed.py::_run_jobs`），`--workers` 是总数。
...
1. **冒烟（`S0`）**：每任务 1 条、共 4 条，单 worker，`SMOKE` 通过后才进入下一步。
2. **校准阶梯**：16 条新值规格（四任务各前 4 条，前三任务 hard、`VideoRepick` medium）× 八档 = 最多 128 次；按 `S0 → S1 → P0 → P01 → P01x4 → P01x6 → P01x8 → P01x10` 从低到高跑，某档不通过（逐位差异、并发不成立、OOM、超时）即停在该档，记录失败档。
3. **判定**：`PARALLEL_CONTENT`（每档 16 对逐位对 `S0`）+ `PARALLEL_OVERLAP`（同一主机单调时钟，不同 PID 的 `[first_step, last_step]` 窗口 `min(end_a,end_b) − max(start_a,start_b) > 0`；`P01` 还需跨卡共同窗口；只有排队／导入重叠不算）。
4. **选档（`PARALLEL_SCALE`）**：取通过两项判定、无 OOM／超时、主机内存峰值低于 300 GB、且墙钟不比上一档差的最大档。
5. **第二阶段 1100 条全部实跑**：用选出的档（每卡 2 约 5 小时，每卡 10 若线性约 1 小时，以校准墙钟为准）；全部档不通过则用 `S0`（约 14.5 小时），并行三项保持 `FAIL`，不换规格、不放宽容差。
6. **留档**：保留最多 128 次逐轮结果与每档的墙钟、显存与内存峰值、重叠时长、完整差异；超过 5 分钟的阶段用 detached `tmux`。
...
2. **范围：**第一部分第一节的 11 组是唯一规格集合；1100 条全部是第二阶段实跑样本，第一阶段只取其中 4 条冒烟与 16 条校准；`VideoRepick hard` 不生成、不画图、不实跑。
...
| 八档校准阶梯 | 同一 16 规格的 `S0` 对 `S1/P0/P01/P01x4/P01x6/P01x8/P01x10` | 少产物、不同 timestep、数值／图像差异、规划回退、只有排队重叠、OOM 被记成通过、跳档 | `PARALLEL_CONTENT`、`PARALLEL_OVERLAP`、`PARALLEL_SCALE` |
| 最终样本与交付 | 1100 个唯一规格、每次执行记录、每组七类计数与碰撞三分项、文件清单 | 无 HDF5 的失败被丢弃、未运行被记成功、仅保存通过标记、失败环节缺失 | `FEASIBILITY`、`DELIVERY` |

七类最终状态互斥：通过、规格拒绝、碰撞拒绝、实际对象／动作不符、规划失败、超时、未运行；对应计数之和为 1100，每组 100。碰撞的几何确认、数值边界与无法证明安全分项统计，不能把 `uncertified` 当作已确认穿入。基础设施或代码错误另有 `error_type`／`failure_class`／阶段明细，阻塞受影响样本并计入未完成项，不冒充物理不可行。最多 128 次校准重复执行按档单独列逐轮结果；唯一规格的可行性结论与跨配置一致性结论分开，不能用重复成功次数扩大分子。冻结前候选拒绝数与固定的 9 个诊断案例另册统计，不进入 1100 行分母。
...
短测、上述核心端到端与视频碰撞链路冒烟合计不超过 300 秒；预计超出时选取覆盖关键拒绝分支与成功链的定向子集，剩余预算再补轻量全量。不以旧案例回放或语法检查替代任务冒烟，也不因五分钟限制跳过；任一冒烟失败停止放大，不换规格。完整八档校准阶梯、全部固定案例复现和可行性执行按已获批阶段组织，预计超过五分钟使用 tmux。

拟新增 `run --phase calibration` 固定 16 条和八档配置（`--workers` 依次 1、1、2、4、8、12、16、20，`--gpus` 按档取 `0`／`1`／`0,1`），按任务／难度／配置拆目录并调用原生产入口，阶梯从低到高、某档失败即停；`run --phase feasibility` 先读取校准报告中 `PARALLEL_SCALE` 选出的档（全部档未通过则 `S0`）执行全部 1100 条，按任务／难度拆目录，逐条写结果类别与失败环节。`compare` 独立检查原始数据；`report` 无论此前通过与否都保存完整状态，不得因某条失败遗失剩余样本。
...
- 第一阶段只跑 20 条；1100 条的物理可行结论以第二阶段实跑为准，跑前的静态与连续几何通过不代表抓取、运行时最近邻或实际物理轨迹可执行。
...
轻量包拟放 `docs/validation/newtask-v2/<运行编号>/`：中文 `README.md`、11 份规格及规范化散列、全量／每批计数表、静态拒绝清单、跑前与跑后两套图、1100 行唯一样本结果（含结果类别与失败环节）、每组七类计数与碰撞三分项、4 次冒烟与最多 128 次校准记录、命令与退出码、HDF5 全字段聚合指纹、动作绑定摘要、资源／时间区间、环境与源码指纹、完整清单散列。轻量不表示只留成功条目；没有 HDF5 的失败也必须能定位到规格与错误阶段。
