...
| PatternLock/RouteStick 长度校准到 30±5 s | **两者现在都偏短**：PatternLock ep0 演示 649 帧（21.6 s）；RouteStick ep6 演示 600 帧（20.0 s）。用户定「不改布局，尽可能长」。RouteStick 每段恰好 50 帧，L 改为 [15,21] 得 25.0～35.0 s（或按对齐口径收成 [16,17]，待答 L37）；PatternLock 5×5 上不重访路径最多 25 节点、每段平均 34.25 帧 ⇒ 上限约 27.4 s，取节点 [24,25] 并把搜索预算提到 20000、耗尽抛错，预计 26～27.5 s，**到不了 30 s** | 2.10 / 2.11 |
...
| L37 | RouteStick 的 L 范围 | [15,21]（25.0～35.0 s） / [16,20]（26.7～33.3 s） | [15,21] | 待答：[15,21]（围绕 30 s）/ [16,17]（与 PatternLock 的 26～27 s 对齐） |
| L38 | RouteStick 的 L 范围是否冻进 decision 与规格 header（现在从类属性读、未冻结） | 是 / 否 | 是 | 建议是，未反对即按是 |
| L39 | 「30 s」怎么量 | h5 中 `is_video_demo` 帧数 ÷ 30（A2） / 物理仿真时间 | A2 | 建议 A2，未反对即按 A2 |
...
| 字段 | 含义 | V4 xhard 现值 | V5 新值 / 注入什么 |
|---|---|---|---|
| `config_xhard.length` | **段数 L**（节点数 = L+1） | `[12, 15]` | **`[15, 21]`**（25.0～35.0 s，均值 30.0 s）或 **`[16, 17]`**（26.7～28.3 s，与 PatternLock 对齐）——L37 待用户定；注入 `objects.L` |
