...
| L52（新，P6 后） | InsertPeg 可达极限：要不要收窄 xhard 杆区的 x 上界 | 不收（接受约 16% 的局抓取点 > 0.78 m，实测 2/10 因此失败）/ x 上界 0.2 → 0.15（> 0.78 m 的比例降到 5%，布局失败仍 0）/ 0.2 → 0.1（降到 0） | 0.15：只改 xhard 的 x 采样上界，不动 y；离线布局失败仍为 0 | 待答 |
