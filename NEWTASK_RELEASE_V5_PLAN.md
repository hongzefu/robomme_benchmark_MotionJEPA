...
| L52（新，P6 后） | InsertPeg 可达极限：要不要收窄 xhard 杆区的 x 上界 | 不收（接受约 16% 的局抓取点 > 0.78 m，实测 2/10 因此失败）/ x 上界 0.2 → 0.15（> 0.78 m 的比例降到 5%，布局失败仍 0）/ 0.2 → 0.1（降到 0） | 0.15：只改 xhard 的 x 采样上界，不动 y；离线布局失败仍为 0 | **x 上界 0.2 → 0.1**（2026-09-24 用户选「缩到 x 最大 0.1（区域变 30 × 60 cm）」；只改 xhard 的 x 上界，y 不动） |
| L53（新，P6 后） | InsertPeg 插入不到位（3/10，抓尾插头时插入端离孔板中心 0.051～0.074 > 0.05）：要不要修 `insert_peg` 求解器 | 不修（正式生成按约 40% 通过率留余量，靠 H4 递补）/ 只在 xhard 修求解器的插入末段（属 `src/robomme` 求解器改动，须出 md 报告） | 本轮不修，另立事项：它是求解器问题，与 V5 的采样议题无关 | **不修**（2026-09-24 用户答「l52不修」，按选项内容理解为 L53；L52 另问） |
...
| L46 | 是否把 PickXtimes xhard 的方块区域扩到半宽 0.25 | 是 / 否 / 先做演示探针再定 | 先探针再定 | **0.25**（2026-09-24「l46 0.25」；P1 探针 13/13） |
...
| L54（新，P4 后） | VideoRepick 交换路径压过按钮底座（最终规则下 66.2% 的局至少一段，演示中可见方块单帧 5～6.5 cm 跳变）：扫掠过滤要不要把按钮底座算作障碍 | (a) 把按钮底座作为静止障碍纳入 reset 规划的扫掠过滤（`check_swap_sweep` 的 bystanders 加按钮 OBB），压过按钮的候选搭档视为不可行 / (b) 接受现状，人工看片 | (a)：规划期过滤只增加候选级拒绝，不改机制；实施后要重测 plan_fail | **(a)** 按钮底座纳入 reset 规划的扫掠障碍（2026-09-24「l54 a」） |
...
| xhard 杆区 x 上界（L52，新增） | 四根杆的采样区 | `x ∈ [-0.2, 0.2]`、`y ∈ [-0.3, 0.3]` | **`x ∈ [-0.2, 0.1]`**（P6：远边 x≈0.2 的杆抓取点离基座 0.82～0.83 m 超本机极限约 0.81 m，2/10 因此失败；x 上界收到 0.1 后 > 0.78 m 的比例由 16% 降到 0，布局失败仍 0）；y 不动；原三档不变 |
...
| 字段 | 含义 | V4 xhard 现值 | V5 新值 / 注入什么 |
|---|---|---|---|
| `config_xhard.length` | **节点数** | `[20, 24]` | **`[25, 25]`**（5×5 不重访路径的上限；P3 探针后由 [24,25] 收成固定 25，因为区间搜索 86% 只得 24 节点）；注入 `actions.path_nodes` |
...
| 运行记录 | 实际演示帧数 | ep0/3/6 实测 649 / 683 / 757 帧 = 21.6 / 22.8 / 25.2 s | P3 实测每段平均 35.06 帧，25 节点局 857～872 帧 ≈ **28.6～29.1 s**；记 `demo_frames` |
...
| `XHARD_DECISION.target_cube_position_policy.region_half_size` 与 `distractor.region_half_size` | 有色与干扰方块的采样区半宽 | `0.2` | **`0.25`**（L46；P1 探针：10 cm 三块团 33.5% → 10.2%，演示 13/13 含远角 seed）；圆盘区域不变 |
...
| 按钮作扫掠障碍（L54，新增） | 交换路径是否允许压过按钮底座 | D5 只查方块之间，不查按钮（V4 8.9% 的局路径经过按钮，最终规则下 66.2%） | **reset 规划时把按钮底座 OBB 作为静止 bystander 放进 `check_swap_sweep`**，会压按钮的搭档视为不可行；规划失败率会从 9.04% 上升，实施后重测 |
...
- **实测**（离线 1500～2400 局；最小距 0.12、余量 5 mm、**k = 2 且目标每 3 次一轮**——与最终定的 k%6 轮转 + 3 个最近不同，下表只作参考，3.5 的探针按最终规则重跑）：
  ...
  | 单局同一对最多重复 | 5.77 | 2.92 |
  | 不同对的个数 | 2.84 | 5.62 |
  | D5 拒绝 | 演示期约 35～45% | reset 期 8.4%（k=2 口径；最终规则 P4 实测 9.04%）；运行时残余 0（P4 演示 0/101） |
...
- **P3 PatternLock（已完成，2026-09-24）**：搜索层各 1000 次试验，24 / 25 节点在 20000 次预算内命中率都是 1.000（1000 次预算下分别 0.905 / 0.365，5000 次 0.871，10000 次 0.984）；搜索墙钟 p95 0.14 s、最大 0.38 s，对抽签 30 次上限无影响。演示 8/8 成功（含 oracle 执行），实测每段平均 **35.06 帧**（计划写 34.25）；24 节点局平均 26.7 s，25 节点局平均 28.8 s，最短一局 735 帧 = 24.5 s（seed 5500900，低于 750 帧下限）。**发现**：按 [24,25] 搜到第一条在区间内的路径就停，86.3% 的局只有 24 节点。**实施方决定（按口径 14 例外自决）**：节点数固定 **25**（`length=[25,25]`，20000 次命中率 1.000），演示预计 28～29 s，能把带外风险压到最低。
...
- **P6 InsertPeg 演示（已完成，2026-09-24）**：v4-01 的 10 个 seed 本机演示 **4/10**（V4 同组 2/10）。间隔全部成立：两两最小轮廓间隔 0.0304 > 0.03、离孔板最小 0.0165 > 0.01，静置 20 步最大横移 0.001 mm，演示中无邻杆被抬起；10 局布局与 V5 离线原型逐位一致。间隔规则负责的三类失败（目标重叠 / 邻杆在夹爪闭合区 / 压孔板）从 V4 的 3/2/1 **全部清零**；剩余 6 局失败与间隔无关：可达极限 2（抓取点离基座 0.832 / 0.815 m，都在杆区远边 x≈0.2，本机极限约 0.80～0.81 m）、插入不到位 3（都是抓尾插头，插入端离孔板中心 0.051～0.074 > 判据 0.05，邻杆与孔板全程未动，是 `insert_peg` 求解器的问题）、规划失败 1（抓取规划失败未报错、空转到 5000 步）。离线估计（每种 400 局）：抓取点 > 0.78 m 的比例，当前杆区 16%，x 上界收到 0.15 → 5%，收到 0.1 → 0，只收 y 无用。**由此新增两项待决 L52 / L53**（见 1.4）。
  判定行：`INSERTPEG_DEMO=REPORT ok=4/10 min_pair_gap=0.0304 min_box_gap=0.0165 by_cause={目标重叠:0, 邻杆在夹爪闭合区:0, 压孔板:0, 可达极限:2, 规划失败:1, 插入不到位:3}`。报告 `plan-probes-r2/insertpeg_demo/REPORT.md`。
- **P4 VideoRepick 最终规则（已完成，2026-09-24）**：副本对 v4-01 的 10 行逐位一致，10 局演示落点与副本一致 10/10。离线 2400 局（V4 / 旧设计 k=2 / **最终规则**）：≥3 块挤同一六分之一区域 26.9% / 3.8% / 3.8%；6 块全部参与 17% / 100% / 100%；目标块被换次数均值 5.84 / 4.91 / 3.66（最终规则下 9.8% 的局目标只换 2 次）；同一对最多重复 5.77 / 2.95 / 2.81；每局最长段均值 0.16 / 0.29 / 0.30 m；reset 规划失败率 **9.04%**（2.15 原写 8.4%，以此为准）。演示 **10/10 成功**，运行时 D5 拒绝 0/101。**新发现按钮问题**：D5 只查方块之间、不查按钮；最终规则下 66.2% 的局至少有一段交换路径让方块中心压进按钮底座（V4 8.9%），演示里 4 局出现单帧 5～6.5 cm 跳变、2 局轻微跳变，与离线预测一一对应；原因是间距拉开后方块被推向区域边缘而按钮在区域内。速度：最长段峰值约 V4 的 1.5 倍、p95 约 2.1 倍，须人工看片（跳变局 4900100 / 4900400 / 4900500 / 4900700）。**新增待决 L54**。
  判定行：`VR_FINAL=REPORT place_ok=100.0% plan_fail=9.04% all6=100.0% path_mean=0.196 path_max=0.516 demo_ok=10/10 d5_runtime=0/101`。报告 `plan-probes-r2/videorepick_final/REPORT.md`。
- **P1 PickXtimes 区域半宽（已完成，2026-09-24）**：副本对 v4-01 逐位一致 20/20。离线每组 3000 局：两组 reset 失败都是 0，< 8 cm 的对都是 0；10 cm 三块团 **33.5%（0.2）→ 10.2%（0.25）**；目标圆盘离基座最大 0.715 → 0.778 m，0.25 下 3.1% 的局目标 > 0.75 m。本机演示：半宽 0.2 **10/10**，半宽 0.25 **13/13**（含 3 个专挑目标 > 0.75 m 的远角 seed，最远 0.765 m），同 seed 两组耗时几乎一致。**建议 L46 取 0.25**，待用户定。
  判定行：`P1_PICK_REGION=REPORT hw0p20: reset_ok=3000/3000 demo_ok=10/10 | hw0p25: reset_ok=3000/3000 demo_ok=13/13 far_target_max=0.765`。报告 `plan-probes-r2/pickxtimes_region/REPORT.md`。

六项探针全部完成。汇总：P1/P2/P3/P4/P5 的最终规则都可行且演示成功率高（10/10、19/24、8/8、10/10、6/6），P6 InsertPeg 4/10 但间隔类失败清零。由探针新增的待决项：L46（区域半宽）、L52（InsertPeg 杆区 x 上界）、L53（插入求解器）、L54（VideoRepick 按钮纳入障碍）。
