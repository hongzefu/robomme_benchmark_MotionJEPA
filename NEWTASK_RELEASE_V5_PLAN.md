...
| L53（新，P6 后） | InsertPeg 插入不到位（3/10，抓尾插头时插入端离孔板中心 0.051～0.074 > 0.05）：要不要修 `insert_peg` 求解器 | 不修（正式生成按约 40% 通过率留余量，靠 H4 递补）/ 只在 xhard 修求解器的插入末段（属 `src/robomme` 求解器改动，须出 md 报告） | 本轮不修，另立事项：它是求解器问题，与 V5 的采样议题无关 | 待答 |
...
| L54（新，P4 后） | VideoRepick 交换路径压过按钮底座（最终规则下 66.2% 的局至少一段，演示中可见方块单帧 5～6.5 cm 跳变）：扫掠过滤要不要把按钮底座算作障碍 | (a) 把按钮底座作为静止障碍纳入 reset 规划的扫掠过滤（`check_swap_sweep` 的 bystanders 加按钮 OBB），压过按钮的候选搭档视为不可行 / (b) 接受现状，人工看片 | (a)：规划期过滤只增加候选级拒绝，不改机制；实施后要重测 plan_fail | 待答 |
