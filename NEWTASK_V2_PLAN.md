...
交付清单见 [docs/validation/newtask-v2/DELIVERY.md](docs/validation/newtask-v2/DELIVERY.md)，
逐项对应第六步要求的内容。核对结论：

- **完成**：第一至第五步；②③④ 15 格通过、⑤ 两路通过；③.4 的合并产物比较已补上
  （A 路用基线的原合并脚本、B/C 用平铺主文件的 `--merge-only`，三路 0 差异）；
  清理后抽样复验八项 0 差异、定向检查清单跑完；离线复验由三项轻量测试覆盖。
- **未完成并如实登记**：① 的 `BinFill` 六格目视（138 张只看了 1 张，用户中途要求停止目视）；
  RRT\* 回退局一次都没触发，4.0 的容差退出路径未被启用也未验证；
  其余 11 格未在清理后重跑（抽样口径）；③.4 只在一格上比过。
- 因此本次**不宣称**方案第六步「15 格五项重要对拍逐格登记完成」的全部条件已满足：
  ① 有 6 格停在「待目视」。②③④⑤ 与全部定向检查已完成。

首个实现提交按 `AGENTS.md` 的「改完即提交」规则拆成了 `10.0`–`10.4`，
标题与正文见各自提交；方案原文要求的单个 `10.0` 提交口径已在 10.0 一节说明。
