...
1. **数量**：分别固定颜色数、总生成数、投入颜色数和投入总数的取值规则。每局记下：每色生成数、每色投入数、选中的颜色池和实际投入总量。
2. **布局**：固定方块和按钮在哪里生成、怎样旋转，以及方块何时出现；开局全出现与布局密集程度分开。每局记下：dynamic 值、全部方块创建顺序／颜色／位姿、按钮位姿；不只存最终计数。
3. **投入对象**：固定孔板的位置、姿态和尺寸，以及抓哪块、投入哪里。每局记下：孔板完整姿态、每一步 `pick` 的方块 ID 与 `put_in` 目标；任务执行必须实际使用该顺序。
4. **生命周期**：固定每次初始化的抽样顺序、方块出现顺序和失败恢复动作。每局记下：两次初始化分别保存颜色排列、出现次序、恢复注入动作；不能两次共用一个排列。

**再说现在的值**

1. **数量**：easy：颜色 1 种，生成 4～6 块，投入颜色 1 种，投入 1～3 块；medium：颜色 2 种，生成 8～10 块，投入颜色 1～2 种，投入 2～4 块；hard：颜色 3 种，生成 10～12 块，投入颜色 2～3 种，投入 3～5 块。选中的目标颜色允许分得 0 块。
...
1. **颜色与次数**：分别固定颜色数、每色块数和抓放循环次数 `number`；循环次数不是方块数。每局记下：实际颜色列表、全部方块、`num_repeats`。
2. **布局**：分别固定目标方块 `target_cube`、放置圆盘 `target` 和按钮的生成区域与姿态。每局记下：方块／圆盘／按钮完整位姿，以及最终目标方块 ID。
3. **角色**：固定怎样挑目标方块、哪些块属于非目标；每个对象的颜色和角色分开。每局记下：`target_cube_id`、`non_target_ids`；保留先抽目标颜色又被方块选择覆盖的随机消费。
4. **动作**：固定反复抓放同一目标块的动作顺序，以及结束时的按钮动作。每局记下：完整有序动作列表与恢复动作索引。

**再说现在的值**

1. **颜色与次数**：easy：颜色 1、循环 1～3；medium：颜色 3、循环 1～3；hard：颜色 3、循环 4～5；选中颜色各 1 块。
...
1. **轮数**：固定颜色数和摆动轮数；一轮由左右两个方向的动作组成。每局记下：实际轮数及左右共几次，不把轮数写成方块数。
2. **布局**：分别固定方块、两处摆动目标和按钮的位置与姿态。每局记下：全部对象、颜色、目标方块、两个落点与按钮位姿。
3. **选择**：固定目标方块和干扰方块的选择规则；干扰方块不参加目标动作。每局记下：最终目标与非目标 ID 列表。
4. **动作**：固定怎样区分左右目标、先去哪里、摆多高和怎样判定到位。每局记下：按右→左展开的完整循环及左右对象绑定。

**再说现在的值**

1. **轮数**：easy：颜色 1 种，摆动 1～3 轮；medium：颜色 3 种，摆动 1～2 轮；hard：颜色 3 种，摆动 3 轮。一轮按右→左各一次，`max_swings=2*num_repeats`。
...
1. **速度与次数**：分别固定运动速度档和在第几次经过目标时停止。每局记下：选中的速度档、实际 stop_time。
2. **路线**：固定方块运动路线、停止目标、按钮的位置及方块颜色。每局记下：起终点、目标、按钮、RGB 和完整姿态。
3. **运动段**：固定整条路线分成哪些往返段、每段朝哪个方向运动。每局记下：每段实际方向、起止步及停止所在段。
4. **时间**：固定开始准备、按按钮和停止运动之间的时间关系。每局记下：每个停止／准备窗口，以及抽样后又被覆盖的时间值和对应随机消费。
...
1. **数量**：分别固定容器总数和拾取数量。每局记下：实际容器数、拾取数、目标容器顺序。
2. **布局**：固定容器的生成区域、朝向、间距和位置重抽规则。每局记下：每个容器和藏块的完整位姿、创建顺序、空容器列表。
3. **藏物**：固定哪些容器藏方块、藏什么颜色、哪些容器为空。每局记下：颜色→方块→容器的完整映射。
4. **动作**：固定展示哪些容器、何时升降、先后抓哪个容器。每局记下：展示对象列表、每次抓取的容器及其中方块 ID。

**再说现在的值**

1. **数量**：easy：3 个容器，拾取 1 次；medium：5 个容器，拾取 1 次；hard：15 个容器，拾取 2 次。
...
1. **数量**：分别固定容器总数和拾取数量。每局记下：容器数与抓取顺序。
2. **布局**：固定容器和按钮在哪里生成、怎样摆放，以及对按钮的避让。每局记下：全部容器、藏块、按钮位姿和创建顺序。
3. **藏物**：固定颜色、藏块、容器和拾取目标之间的对应关系。每局记下：具体颜色／藏块／容器映射、空容器与非目标集合。
4. **动作**：固定按按钮、展示容器和拾取的先后顺序。每局记下：按钮动作和抓取对象的有序列表、真实展示窗口。
...
1. **数量**：分别固定容器数、交换次数和拾取次数的取值规则。每局记下：`n_bins/n_swaps/n_picks` 和本局按顺序要拾取的目标。
2. **布局**：固定容器怎样摆放和旋转，以及颜色、藏块和空容器怎样分配。每局记下：每个容器位姿、颜色顺序、藏物／空容器、目标选择。
3. **交换绑定**：固定谁发起交换、怎样选搭档，以及藏块跟随哪个容器。每局记下：每次 `initiator_id/partner_id`、当时位置与距离、藏块跟随映射。
4. **时间**：固定每次交换何时开始和结束，以及交换的移动方式。每局记下：完整窗口和轨迹输入；实际帧边界另留证据。

**再说现在的值**

1. **数量**：easy：3 个容器，交换 1～2 次，拾取 1～2 次；medium：4 个容器，交换 1～2 次，拾取 1 次；hard：4 个容器，交换 2～3 次，拾取 2 次。现行 xhard 不进入原 train。
...
1. **数量**：分别固定容器数、交换次数和拾取次数的取值规则。每局记下：实际容器数、交换数、抓取数。
2. **布局**：分别固定两个按钮和各容器的位置、朝向与布局抽样规则。每局记下：两按钮与容器位姿；所有原随机偏移，包括抽取后未使用的布局分支。
3. **绑定**：固定藏块和容器的对应关系、交换发起者及搭档选择规则。每局记下：容器、藏块、目标、每次实际交换双方。
4. **动作时间**：固定两个按钮的操作顺序、交换时间表和拾取顺序。每局记下：两按钮动作、全部交换窗口、拾取顺序。

**再说现在的值**

1. **数量**：easy：3 个容器，交换 1～2 次，拾取 1～2 次；medium：4 个容器，交换 1～2 次，拾取 1 次；hard：4 个容器，交换 2～3 次，拾取 2 次。
...
1. **数量**：分别固定场上方块数和要高亮的方块数。每局记下：实际生成数量、有序高亮目标列表。
2. **外观与位置**：固定每块方块的颜色、生成区域、姿态和按钮位置。每局记下：每块颜色／姿态、按钮位姿与创建顺序。
3. **对象选择**：固定哪些块依次高亮、依次抓放，以及哪些块不能抓。每局记下：高亮 ID、抓取／放下 ID、非目标集合。
4. **时间**：固定高亮何时开始、何时结束。每局记下：每个高亮事件与动作边界。

**再说现在的值**

1. **数量**：easy：生成 3 块，高亮 1 块；medium：生成 4 块，高亮 2 块；hard：生成 6 块，高亮 3 块。
...
1. **模式与数量**：分别固定布局模式、是否交换、场上块数和交换次数。每局记下：实际物体列表与交换数；hard 必须有完整规格。
2. **次数**：固定同一目标块重复抓放多少次；与交换次数分开。每局记下：实际重复抓放次数、每次目标 ID。
3. **颜色与布局**：固定每块颜色、摆放位置、按钮位置和目标选择。每局记下：逐块颜色／姿态、按钮、创建顺序、目标方块与非目标列表。
4. **动作**：固定每次谁和谁交换、什么时候交换，以及重复抓放的动作顺序。每局记下：完整交换双方、动作序列和两次初始化结果。

**再说现在的值**

1. **模式与数量**：easy：3 块、swap 1～2；medium：3 块、swap 2～3；hard：红蓝绿各 5 块，共 15 块、swap 0。
...
1. **数量**：分别固定方块、目标台和演示对象的数量，并固定这些对象的位置与颜色。每局记下：每块方块、目标台、按钮和 `goal_site` 的位姿、颜色及目标方块身份。
2. **任务语义**：固定任务问的是按按钮之前还是之后、答案指向哪个台。每局记下：具体答案、前后台绑定。
3. **演示**：固定演示操作哪块方块、经过哪些台、什么时候按按钮。每局记下：完整演示列表和执行目标。
4. **交换与重置**：固定是否交换目标台、交换哪两个台，以及演示后物体放在哪里；出生原位与放置终点分开。每局记下：交换台 ID、窗口、每块演示开始位姿及原重置位置。

**再说现在的值**

1. **数量**：easy：1 块、3 台；medium：3 色各 1 块、4 台；hard：3 色各 1 块、4 台；三档 `additional_place=False`。布局：方块和目标台中心 `[0,0]`、半边长 0.2，方块随机 yaw；按钮中心 `[0.1,0]`、扰动 `[0.05,0.3]`、scale 1.5；`goal_site` 中心 `[-0.1,0]`、半边长 0.1、半径 `3*cube_half_size`。
...
1. **数量**：分别固定方块与目标台数量，以及方块、台、按钮和放置终点的位置。每局记下：方块／目标台／按钮／goal_site 列表与位姿。
2. **次序**：固定演示经过哪些台、经过顺序，以及任务询问其中第几个台。每局记下：台 ID 序列、`which_in_subset`。
3. **演示**：固定演示操作哪块方块、按钮动作插在哪里、最后放在哪里。每局记下：目标块 ID、`button_task_index`、完整演示与执行动作。
4. **交换与重置**：固定是否交换目标台、交换哪两个台，以及每块方块的原位和重置位置。每局记下：交换台 ID、窗口、每块演示开始位姿和原重置位置。
...
1. **两套布局**：分别固定演示场景和执行场景，两套布局各自记录。每局记下：`demo_layout` 与 `execution_layout` 的杆／cube／goal 全部位姿。
2. **区域**：分别固定杆、方块、目标区域在哪里生成，以及位置的避让规则。每局记下：位置输入、最终接受位置和拒绝序列。
3. **姿态与方法**：固定杆的尺寸和朝向，以及使用杆推、夹爪推还是抓起放下。每局记下：两套杆完整姿态、实际 way、目标对象和执行动作。
4. **随机流**：固定每次初始化和场景切换时的抽样顺序，保留没有改变最终数值的随机调用。每局记下：原抽样消费、每次初始化与 reset 切换身份。
...
1. **对象**：分别固定杆的总数、目标杆身份、尺寸及颜色规则。每局记下：每根杆的 ID、目标杆身份、共享头色 RGB 和互补尾色；保留被覆盖的抽样。
2. **孔与布局**：固定孔板和每根杆的位置与朝向，按每次初始化分别保存。每局记下：每次 `_initialize_episode` 的孔与全部杆的完整位姿，不只保存构造临时位置。
3. **距离**：固定杆与孔板、杆与杆之间的距离限制和位置重抽规则。每局记下：实际距离、接受顺序和全部拒绝消费。
4. **生命周期**：固定每次重置的对象顺序、任务对应关系和完整随机消费。每局记下：随机源状态、每次重置的杆列表和任务绑定。
...
1. **网格**：固定网格的行列数、位置和节点间距。每局记下：所有节点位姿与创建编号。
2. **路线**：固定怎样选起终点和路径、允许哪些连接、路径多长；长度按节点数计。每局记下：起终点、完整节点序列、抽样／拒绝过程、是否用最后一次兜底。
3. **动作**：固定演示与执行共用哪条路径、每段经过哪个目标。每局记下：各段对象绑定、高亮和阶段边界。
4. **时间**：固定演示的动作与等待规则，并记录实际视频覆盖的时间。每局记下：实际演示帧范围、录制帧数、写出 fps；不由节点数臆算秒数。

**再说现在的值**

1. **网格**：easy：3×3；medium：4×4；hard：5×5；中心 `[-0.1,0]`、间距 0.1。
...
1. **路线**：固定运动段数、是否允许回退，以及每段经过哪些节点；段数与节点数分开。每局记下：实际段数、按顺序排列的路径节点和槽位，以及是否回退。
2. **几何**：固定网格位置、间距、整体朝向和障碍柱颜色。每局记下：实际节点／柱位姿、整体旋转、柱颜色。
3. **方向**：固定每段绕行方向怎样抽取，以及演示和执行怎样复用方向。每局记下：L 个顺／逆时针方向，演示与执行的绑定；不做跨 episode 配额。
4. **事件**：固定曲线运动、高亮和尾迹等事件，并记录实际发生时间。每局记下：完整路线事件、实际段边界与帧数；原 train 恢复核验不排除尾迹。

**再说现在的值**

1. **路线**：easy：2～3 段、不主动回退；medium：4～5 段、不主动回退；hard：4～7 段、允许回退；端点强制反向。段数为 L 时，节点数为 L+1。
